...
**Hero Image**:
- Generate a **context-specific, detailed** hero image prompt (use 6-part structure: Subject, Style, Composition, Colors, Elements, Constraints)
...
- Identify sections in the content that benefit from visual explanation
- For each identified section, either:
  ...
  - **Download from official sources**: When the blog references official documentation with relevant screenshots/diagrams:
    - Download the image directly from the official source URL
    - Save to `src/assets/blog/[slug]-[descriptive-name].[ext]`
    - Add proper attribution in alt text or caption
    - Use `curl` or `node` to download
- **Goal**: Every blog post should have **at least 2-3 images** (hero + 1~2 in-content)
