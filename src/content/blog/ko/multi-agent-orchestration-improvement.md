...
블로그 자동화 시스템은 약 2개월간 점진적으로 확장되면서 다음과 같은 문제들이 누적되었다:
...
48개 파일을 한 AI가 통째로 읽으면 중간쯤부터 맥락이 흐려진다. 그래서 영역별로 4개 전문 에이전트에게 나눠 맡겼다.
...
분석 단계에서 얻은 가장 중요한 발견은 <strong>문제가 고립되지 않고 서로 연결되어 있다</strong>는 점이었다:
...
이 문서는 `.claude/` 디렉토리의 모든 에이전트, 커맨드, 스킬 중 <strong>실제로 구현된 것</strong>과 <strong>이론적 계획만 있는 것</strong>을 명확히 구분한다.
...
이 패턴이 만능은 아니다. 48개 파일을 직접 쪼개 보면서, 오히려 단일 에이전트가 나았을 상황도 분명히 있었다. 도입 전에 아래 기준으로 한 번 걸러 보길 권한다.

### 적합한 경우

- <strong>작업 범위가 자연스럽게 나뉠 때</strong>: agents, commands, scripts, guidelines처럼 영역이 독립적이고 서로 겹치는 컨텍스트가 적으면, 각 에이전트가 자기 영역만 보고도 충분히 판단할 수 있다.
- <strong>파일·이슈 수가 단일 컨텍스트를 넘길 때</strong>: 한 에이전트가 통째로 읽으면 중간부터 맥락이 흐려지는 규모(대략 30개 파일 이상)에서 분할의 이득이 분명해진다.
- <strong>반복 검증이 품질을 좌우할 때</strong>: 리뷰 → 수정 → 재검증 루프를 여러 번 돌려야 하는 문서·코드 정리 작업에 잘 맞다.
- <strong>같은 종류의 작업을 계속 돌릴 때</strong>: 공식 서브에이전트 문서가 권장하듯, 동일한 지시를 반복하는 워커는 전용 에이전트로 고정하는 편이 효율적이다.

### 피해야 하는 경우

- <strong>작업이 강하게 얽혀 있을 때</strong>: 한 파일의 결정이 다른 파일 다섯 개를 연쇄로 바꾸는 구조라면, 에이전트 간 조율 비용이 분할 이득을 잡아먹는다. 차라리 단일 컨텍스트에서 순차 처리하는 게 빠르다.
- <strong>규모가 작을 때</strong>: 파일 서너 개를 고치는 일에 오케스트레이터를 세우는 건 과설계다. 오케스트레이션 자체의 오버헤드가 더 크다.
...
직접 해보면서 정리한 순서다. 그대로 따라가도 좋고, 본인 프로젝트 규모에 맞게 줄여도 된다.
...
이 개선 프로세스는 일회성이 아니다. 지속적인 개선을 위해:
