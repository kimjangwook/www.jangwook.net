...
LLM을 활용한 코딩 에이전트를 운영하다 보면 반드시 부딪히는 벽이 있다. <strong>컨텍스트 윈도우의 한계</strong>다. 128K 토큰이든 200K 토큰이든, 대규모 코드베이스를 다루다 보면 모델이 중요한 정보를 놓치기 시작한다. 이른바 "Context Rot" — 컨텍스트가 길어질수록 성능이 급격히 떨어지는 현상이다.

MIT에서 발표한 <strong>Recursive Language Models(RLM)</strong> 논문(arXiv:2512.24601)은 이 문제에 대한 근본적인 해법을 제시한다. 그리고 최근 Tenobrus라는 개발자가 이 아이디어를 [Claude Code](/ko/blog/ko/claude-managed-agents-production-deployment-guide)에 직접 구현해서 화제가 되었다. 코딩 에이전트 안에서 RLM을 "스킬"로 구현한 것이다.

AI로 시스템을 만드는 입장에서 이 접근법이 왜 중요한지, 그리고 실무에서 어떻게 활용할 수 있는지 분석한다.

## RLM이란 무엇인가

### 핵심 아이디어

RLM의 핵심은 단순하다. <strong>LLM이 자기 자신을 재귀적으로 호출</strong>할 수 있게 하는 것이다.

기존 방식에서는 긴 프롬프트를 그대로 모델에 넣었다. 당연히 컨텍스트 윈도우를 넘으면 잘리고, 넘지 않더라도 중간 정보를 놓치는 Context Rot이 발생한다.
...
특히 주목할 점은 <strong>RLM-Qwen3-8B</strong>라는 네이티브 재귀 모델을 학습시킨 결과, 기본 Qwen3-8B 대비 평균 28.3% 향상되었고, 일부 태스크에서는 바닐라 GPT-5에 근접하는 성능을 보였다는 것이다.

## 코딩 에이전트에서의 RLM 구현

### Tenobrus의 실험

Tenobrus는 Claude Code 안에서 RLM을 "스킬"로 구현했다. 핵심 아이디어는:
...
이것은 사실상 "코딩 에이전트 안에서 코딩 에이전트가 자기 자신을 재귀적으로 호출하는" 구조다.

### 왜 코딩 에이전트에 RLM이 필요한가

실무에서 코딩 에이전트를 운영하면 다음과 같은 상황이 빈번한다:
...
단일 컨텍스트 윈도우로는 이런 작업을 제대로 수행하기 어렵다. RLM 패턴을 적용하면 에이전트가 스스로 작업을 분할하고, 각 부분을 재귀적으로 처리한 뒤, 결과를 합성할 수 있다.

### 구현 아키텍처

코딩 에이전트에서 RLM을 구현하는 기본 구조는 다음과 같다:
...
최근 AI 업계에서는 단일 모델의 한계를 극복하기 위해 [<strong>멀티 에이전트 시스템</strong>](/ko/blog/ko/multi-agent-orchestration-routing)이 주목받고 있다. 여러 모델이 협업하는 방식이다.

RLM은 이와 다른 접근이다. <strong>같은 모델이 자기 자신을 재귀적으로 호출</strong>하는 것이기 때문에, 모델 간 커뮤니케이션 오버헤드가 없고, 일관된 "사고 방식"을 유지할 수 있다.
...
EM으로서 팀을 운영하는 관점에서 보면, RLM과 멀티 에이전트는 <strong>양자택일이 아니라 상호보완적</strong>이다.

- <strong>RLM</strong>: 단일 에이전트 내부에서 대규모 컨텍스트를 효율적으로 처리
- <strong>멀티 에이전트</strong>: 서로 다른 전문성을 가진 에이전트 간의 협업

실제로 가장 효과적인 아키텍처는 <strong>멀티 에이전트 시스템의 각 에이전트가 내부적으로 RLM 패턴을 사용</strong>하는 것이다.
...
Tenobrus의 접근법을 참고하면, 다음과 같은 패턴으로 RLM을 구현할 수 있다:
...
조직에서 RLM 패턴을 도입할 때는 다음과 같은 단계를 권장한다:
...
에이전트 아키텍처 최적화의 심층 분석은 [딥 에이전트 아키텍처 최적화 가이드](/ko/blog/ko/deep-agents-architecture-optimization)에서 확인하면 된다.

## 논문의 시사점: 향후 전망

RLM 논문이 시사하는 바는 명확하다:
...
RLM은 단순히 학술적 아이디어가 아니다. Tenobrus의 실험이 보여주듯, <strong>지금 당장 코딩 에이전트에 구현할 수 있는 실용적인 패턴</strong>이다.

단일 모델의 한계를 느끼고 있다면, 멀티 에이전트 시스템을 구축하기 전에 먼저 RLM 패턴을 시도해 보십시오. 같은 모델로 훨씬 더 많은 것을 할 수 있다는 사실에 놀라게 될 것이다.

AI로 시스템을 만드는 입장에서, RLM은 "모델을 바꾸지 않고 아키텍처로 성능을 끌어올리는" 엔지니어링의 본질을 보여주는 사례다. 더 큰 모델을 기다리는 것이 아니라, 지금 있는 모델로 더 영리한 구조를 만드는 것 — 이것이 엔지니어링 매니저가 주목해야 할 방향이다.
