...
description: 'Node.js 22.5.0부터 npm install 없이 쓸 수 있는 node:sqlite 모듈. DatabaseSync, StatementSync, 트랜잭션, 사용자 정의 함수까지 실제 실행한 코드와 로그로 정리했다.'
pubDate: '2026-06-09'
heroImage: '../../../assets/blog/node-sqlite-builtin-practical-guide-2026/node-sqlite-builtin-practical-guide-2026-hero.png'
tags: ['Node.js', 'SQLite', '내장 모듈']
relatedPosts:
...
- 동기(sync) 전용이라는 점은 같다
- `db.transaction()` 래퍼가 없다 — 이게 가장 큰 차이다 (아래 섹션에서 자세히 다룸)
...
- `db.transaction()` 래퍼 없음 — 코드가 더 verbose해짐
...
`node:sqlite`의 API 표면을 전부 두드려봤을 때 든 생각: **생각보다 완성도가 높다.** `DatabaseSync`, `StatementSync`, 사용자 정의 함수, 집계 함수, WAL 모드, BigInt 지원 — 내부 툴링에서 필요한 건 거의 다 있다.

`better-sqlite3`의 `db.transaction()` 같은 편의 기능이 빠진 건 아쉽다. 하지만 그게 없어서 쓸 수 없다는 수준은 아니다. `withTransaction` 헬퍼 하나 만들면 되니까.

Node.js 생태계가 외부 의존성을 줄이는 방향으로 움직이고 있다는 신호로 읽힌다. `fetch`, `WebCrypto`, `test runner`에 이어 이제 `sqlite`까지. 언제쯤 `postgresql` 내장을 기대해볼 수 있을까 — 라고 쓰면 너무 욕심인가.
