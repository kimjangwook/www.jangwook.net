...
결론부터 말하면, **git worktree + Claude Code 조합은 생각보다 훨씬 자연스럽게 작동한다**. 설정도 복잡하지 않다. 이 글에서 다루는 내용은 모두 1차 출처로 확인할 수 있다. Git 공식 문서의 [git-worktree](https://git-scm.com/docs/git-worktree) 페이지와 Claude Code 공식 문서의 [Run parallel sessions with worktrees](https://code.claude.com/docs/en/worktrees) 항목, 그리고 일상 작업 레시피를 모은 [Common workflows](https://code.claude.com/docs/en/common-workflows) 가이드가 그것이다.

## Git Worktree가 뭔지 먼저

git worktree는 하나의 git 저장소에서 여러 브랜치를 동시에 서로 다른 디렉토리에 체크아웃할 수 있는 기능이다. 2016년부터 git에 내장되어 있는데, 생각보다 잘 알려지지 않았다. [공식 문서](https://git-scm.com/docs/git-worktree)의 설명을 그대로 옮기면 "하나의 저장소에 연결된 여러 작업 트리를 관리한다(Manage multiple working trees attached to the same repository)"는 기능이다. 각 작업 트리는 자기만의 `HEAD`와 인덱스를 갖는다.
...
Claude Code 기본 사용법과 Best Practices를 이미 익혔다면, 이 패턴은 바로 적용할 수 있다.

### `--worktree` 플래그: 한 번에 처리하기

위처럼 `git worktree add`로 디렉토리를 만들고 `cd`로 들어가 `claude`를 실행하는 수동 방식 외에, Claude Code는 worktree 생성과 세션 시작을 한 명령으로 묶는 내장 플래그를 제공한다. 공식 문서([Worktrees](https://code.claude.com/docs/en/worktrees))에 따르면 `--worktree`(또는 `-w`)를 넘기면 저장소 루트의 `.claude/worktrees/<이름>/` 아래에 worktree를 만들고 `worktree-<이름>` 브랜치로 그 안에서 Claude를 띄운다.

```bash
# worktree 생성 + 세션 시작을 한 줄로
claude --worktree feature-auth

# 다른 터미널에서 다른 이름으로 두 번째 격리 세션
claude --worktree bugfix-123

# 이름을 생략하면 bright-running-fox 같은 이름을 자동 생성
claude --worktree
```

기본 동작은 원격의 기본 브랜치(`origin/HEAD`)에서 분기하므로 항상 깨끗한 트리에서 시작한다. 한 가지 주의점은, worktree는 새 체크아웃이라 `.env`나 `.env.local` 같은 추적되지 않는 파일이 따라오지 않는다는 것이다. 프로젝트 루트에 `.worktreeinclude` 파일(`.gitignore` 문법)을 두면 매번 그 파일들을 자동으로 복사해준다. 그리고 `.claude/worktrees/`는 `.gitignore`에 넣어두는 게 좋다.

세션을 끝낼 때 변경 사항이 없으면 worktree와 브랜치가 자동으로 정리되고, 커밋이나 미저장 변경이 남아 있으면 유지할지 삭제할지 물어본다. 수동으로 만든 worktree는 이 자동 정리 대상에서 빠지므로 직접 `git worktree remove`로 지워야 한다.
...
Claude Code Hook으로 자동화된 리뷰 시스템을 구성해두면, worktree 전환 시 자동으로 컨텍스트를 업데이트하는 hook도 달 수 있다.
...
이 패턴을 무조건 권하지는 않는다. 실제로 써 보면 효과가 분명한 상황과 오히려 손해인 상황이 갈린다.

**병렬 worktree가 유리한 경우:**

- 작업들이 **서로 다른 파일·디렉토리**를 건드릴 때. OAuth는 `src/auth/`, 버그 수정은 `src/routes/`처럼 영역이 갈리면 머지 충돌이 거의 없다.
- 긴 작업 중간에 **긴급 핫픽스**가 들어올 때. stash나 브랜치 전환 없이 다른 터미널에서 바로 고치고, 끝나면 원래 작업으로 돌아온다.
- 같은 코드를 **여러 접근으로 실험**하고 싶을 때. 디렉토리를 두 개 띄워 두고 어느 쪽이 나은지 비교한다.
- 서브에이전트나 백그라운드 작업을 **격리**해 메인 체크아웃을 깨끗하게 유지하고 싶을 때.

**병렬 worktree를 피하는 게 나은 경우:**

- 두 작업이 **같은 컴포넌트나 같은 파일**을 모두 수정해야 할 때. 이때는 worktree가 충돌을 줄이기는커녕 늘린다. 순차적으로 처리하는 편이 낫다.
- 세션이 **3개를 넘어갈 때**. 어느 세션이 어디까지 했는지 추적하는 오버헤드가 작업 시간을 잡아먹기 시작한다. 둘부터 시작하라.
