...
- 닫을 때 포커스가 모달을 연 트리거로 돌아오는지 확인한다(APG 요구사항). 돌아온 포커스가 스티키 헤더 아래에 가려 보이지 않는 경우는 [스티키 헤더가 삼킨 키보드 포커스](/ko/blog/ko/focus-not-obscured-sticky-header-scroll-padding-2026/)에서 따로 쟀다.
