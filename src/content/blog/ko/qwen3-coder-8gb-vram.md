...
80B 파라미터의 코딩 특화 모델을 8GB VRAM 노트북 GPU에서 실행할 수 있다면 어떨까요? Reddit의 r/LocalLLaMA 커뮤니티에서 <strong>nalexand</strong>라는 개발자가 이를 실현한 프로젝트를 공개했다. Qwen3-Coder-Next 80B를 RTX 3070Ti(8GB VRAM)에서 <strong>1.2 tokens/s</strong>로 동작시키는 데 성공한 것이다.

이 글에서는 해당 프로젝트의 핵심 기술인 FP8 양자화, 전문가 레이지 로딩, 캐시 최적화 전략을 분석하고, 소비자 GPU에서의 대형 LLM 실행이 갖는 실용적 의미와 한계를 살펴본다.

## 핵심 과제: 80B 모델을 8GB에 우겨넣기

### 왜 불가능해 보이는가

Qwen3-Coder-Next는 80B 파라미터 모델이다. FP8 양자화 상태에서도 모델 크기는 약 <strong>80GB</strong>에 달한다. 8GB VRAM + 32GB RAM 환경에서는 전체 모델을 메모리에 올리는 것 자체가 불가능하다.

### 첫 번째 시도: 디스크 오프로딩

개발자는 먼저 Hugging Face의 <strong>accelerate</strong> 라이브러리에서 `device="auto"`를 사용한 디스크 오프로딩을 시도했다. 결과는 참담했다.

- 속도: <strong>1 token / 255초</strong>
- 사실상 사용 불가능한 수준

이는 디스크 I/O 병목이 추론 속도를 극단적으로 저하시킨 전형적인 사례다.

## 해결책: 전문가 레이지 로딩 + 캐시 최적화

### MoE 아키텍처의 특성 활용

핵심 인사이트는 모델 구조 분석에서 나왔다. 80B 모델의 대부분의 대형 텐서는 <strong>MLP 전문가(expert)</strong>에 집중되어 있으며, 나머지 구성 요소는 약 <strong>4.6GB</strong>로 VRAM에 충분히 들어간다.
...
개발자는 MLP 전문가를 위한 <strong>커스텀 레이지 로딩</strong> 시스템을 구축했다.
...
캐시가 워밍업된 후에는 안정적으로 <strong>~1.2 t/s</strong>를 유지한다. 첫 번째 요청은 캐시 미스로 인해 느리지만, 이후 요청부터는 캐시 적중률이 높아지면서 속도가 향상된다.
...
RTX 5090의 32GB VRAM과 높은 메모리 대역폭으로 `max_gpu_cache=120` 설정 시 20 t/s 이상이 기대된다.

## 로컬 LLM 코딩의 최전선

이 프로젝트는 로컬 LLM 커뮤니티의 "불가능을 가능으로" 만드는 정신을 잘 보여준다. <strong>nalexand</strong> 개발자는 이전에도 LTX-2, Wan2.2, HeartMula, ACE-Step 1.5 등 다양한 대형 모델을 저사양 GPU에서 최적화한 경력이 있다.
