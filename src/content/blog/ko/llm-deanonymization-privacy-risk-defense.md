...
<strong>소셜 엔지니어링</strong>: 식별된 직원의 온라인 활동 패턴을 기반으로 정교한 피싱 공격이 가능해집니다. AI 코딩 도구를 통한 [프롬프트 인젝션 보안 위협](/ko/blog/ko/roguepilot-copilot-prompt-injection-security)도 이와 맥락을 같이하는 공격 벡터다.
...
<strong>계정 분리 원칙</strong>: 업무 관련 활동과 개인 활동의 계정을 완전히 분리합니다. 동일한 이메일, 유사한 사용자명 사용을 금지하는 정책을 수립합니다. 보다 체계적인 AI 보안 거버넌스 프레임워크는 [NIST AI 에이전트 보안 표준](/ko/blog/ko/nist-ai-agent-security-standards)을 참고할 수 있다.
...
기술 자체는 중립적이지만, <strong>현재 방어 수단이 공격 수단에 비해 크게 뒤처져 있다</strong>는 것이 문제입니다. [AI 증류 공격과 기업 방어 전략](/ko/blog/ko/ai-distillation-attacks-enterprise-defense)은 이와 유사한 비대칭 위협의 또 다른 실전 사례다. 공격자가 저비용으로 대규모 익명 해제를 실행할 수 있는 반면, 방어자는 개별적으로 대응해야 하는 비대칭 구조입니다.
