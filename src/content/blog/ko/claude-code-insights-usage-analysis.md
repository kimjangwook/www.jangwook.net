...
- **사용 통계**: 메시지 수, 세션 수, 파일 수정 내역
- **잘 되고 있는 것**: 효과적으로 활용 중인 기능과 패턴
- **방해 요인**: 생산성을 떨어뜨리는 병목 지점
- **개선 제안**: 바로 적용할 수 있는 실전 팁
- **프로젝트 영역별 분석**: 어떤 작업에 어떻게 쓰고 있는지

일종의 "AI 코딩 건강검진"이라고 생각하면 됩니다. 실행 방법도 간단합니다. Claude Code에서 `/insights`를 입력하면 끝입니다.

## 나의 실행 결과: 핵심 수치

> **분석 기간**: 2025-12-31 ~ 2026-02-03 (약 35일)
> **환경**: Claude Code v2.1.31
...
실제 활용 예시:
- **소설 프로젝트**: 종합 리뷰 → 문제 식별 → 병렬로 여러 파일 동시 수정
- **블로그 생성**: 한국어 작성 → 일본어/영어/중국어 병렬 번역
- **코드 리팩토링**: 이슈 분석 → 태스크 분배 → 동시 수정
...
- **소설 리뷰**: 여러 권에 걸친 종합 리뷰 중 중단
- **번역 작업**: 대규모 번역 중 실행 중간에 끊김
- **블로그 콘텐츠 생성**: 초기 읽기/설정 단계에서 종료

핵심 원인은 **"단일 세션에 너무 큰 작업을 할당하는 패턴"**이었습니다. 모든 것을 한 번에 해결하려는 욕심이 오히려 비효율을 만들고 있었던 셈입니다.
...
1. **큰 작업은 반드시 분할하세요**: 단일 세션에 모든 것을 넣지 말고, 챕터/섹션/모듈 단위로 나누세요.

2. **체크포인트를 파일로 남기세요**: `task-status.md` 같은 파일을 만들어 진행 상황을 기록하면, 세션이 끊겨도 이어서 작업할 수 있습니다.

3. **커스텀 슬래시 커맨드를 만드세요**: 반복 작업이 있다면 `/write-post`처럼 커맨드로 인코딩하세요. 한 번 만들면 계속 재사용할 수 있습니다.

4. **병렬 에이전트를 적극 활용하세요**: `TodoWrite` → `TaskCreate` 패턴으로 작업을 분배하면 효율이 크게 올라갑니다.

### /insights를 최대한 활용하는 방법

1. **정기적으로 실행하세요**: 한 달에 한 번 정도 `/insights`를 실행하면 작업 패턴의 변화를 추적할 수 있습니다.

2. **CLAUDE.md에 반영하세요**: `/insights`가 제안한 개선사항을 CLAUDE.md에 기록하면, Claude Code가 자동으로 그 규칙을 따릅니다.

3. **에러 패턴을 주시하세요**: "File Too Large"나 "Command Failed" 같은 에러가 많다면 워크플로우를 조정할 필요가 있습니다.

4. **HTML 리포트를 활용하세요**: `/insights`는 상세한 HTML 리포트도 생성합니다. 시각화된 데이터로 더 깊은 분석이 가능합니다.

## 다른 개발자에게 추천하는 이유

`/insights`를 실행한 뒤 가장 크게 느낀 점은, **"내가 AI를 어떻게 쓰는지 모르면서 잘 쓰고 있다고 착각하고 있었다"**는 것입니다.

데이터 기반의 객관적 진단은 다음과 같은 가치를 줍니다:

- **숨겨진 병목 발견**: 컨텍스트 한도 문제를 인식하지 못했다면, 같은 실수를 반복했을 것입니다.
- **효과적인 패턴 강화**: 병렬 에이전트 활용이 실제로 효과적이라는 확인은 자신감으로 이어집니다.
- **구체적 개선 방향**: "더 잘 쓰세요"가 아니라 "체크포인팅을 도입하세요"라는 구체적인 액션 아이템을 제시합니다.

AI 코딩 도구를 사용하는 모든 개발자에게 `/insights` 실행을 추천합니다. 5분이면 실행되지만, 거기서 얻는 인사이트는 앞으로의 워크플로우를 근본적으로 바꿀 수 있습니다.

## 마치며

Claude Code `/insights`는 단순한 통계 기능이 아닙니다. AI 코딩 도구와의 협업 방식을 데이터로 보여주고, 구체적인 개선 방향을 제시하는 **AI 워크플로우 코치**입니다.
