...
AI 코딩 도구를 매일 쓰면서도 이 질문에는 늘 답이 막혔습니다. 그러다 Claude Code에 `/insights`라는 기능이 숨어 있다는 걸 알게 됐죠. 실제 사용 데이터를 바탕으로 내 워크플로우를 객관적으로 진단해주는 기능입니다.

아래는 제 실제 프로젝트에서 `/insights`를 돌린 결과 전체와, 거기서 건진 인사이트, 그리고 바로 써먹은 팁입니다.
...
저는 대규모 작업을 할 때 반드시 태스크 리스트를 먼저 만들고, 이를 병렬 에이전트에 분배하는 패턴을 사용합니다. `/insights`는 이를 "정교한 워크플로우(sophisticated workflow)"라고 평가했습니다. AI 에이전트 협업 패턴: 5개의 전문 에이전트로 풀스택 앱 구축하기에서 이런 멀티 에이전트 접근법의 구체적인 구현 예시를 확인할 수 있습니다.
...
오후와 저녁에 집중되는 패턴이 뚜렷합니다. 심야에도 17%가 찍히는데, 이건 밤늦게까지 지치지 않는 코딩 파트너를 곁에 둘 수 있다는 AI의 장점을 그대로 활용한 결과이기도 합니다.
...
2. <strong>CLAUDE.md에 반영하세요</strong>: `/insights`가 제안한 개선사항을 CLAUDE.md에 기록하면, Claude Code가 자동으로 그 규칙을 따릅니다. Claude Code Best Practices는 공식 권장 설정을 실제 프로젝트에 적용한 사례를 상세히 정리합니다.
...
좋은 도구라도 쓰는 타이밍이 맞아야 값을 합니다. 한 달 넘게 돌려보며 정리한, `/insights`가 빛나는 상황과 오히려 헷갈릴 수 있는 상황입니다.

### 쓰면 좋은 경우

- <strong>작업량이 충분히 쌓였을 때</strong>: 분석은 로컬에 저장된 30일치 사용 데이터를 기반으로 합니다. 며칠밖에 안 쓴 상태에서 돌리면 표본이 적어 패턴이 잘 안 보입니다. 저처럼 수천 건의 메시지가 쌓였을 때 진가가 드러납니다.
- <strong>워크플로우를 바꾼 직후 점검할 때</strong>: 새 커스텀 커맨드를 도입했거나 병렬 에이전트 패턴을 바꿨다면, 한 달 뒤 다시 돌려 에러율과 세션 효율이 실제로 나아졌는지 비교하기 좋습니다.
- <strong>병목이 막연하게 느껴질 때</strong>: "왜 자꾸 중간에 끊기지?"라는 감을 데이터로 확인하고 싶을 때. 제 경우 컨텍스트 한도가 범인이라는 걸 이 기능으로 처음 명확히 봤습니다.

### 조심하거나 피할 경우

- <strong>팀 단위 지표가 필요할 때</strong>: `/insights`는 어디까지나 내 로컬 사용 데이터를 보는 개인용 도구입니다. 팀 전체의 채택률, PR 기여도, 라이선스 활용도 같은 조직 지표는 [공식 Analytics 대시보드](https://code.claude.com/docs/en/analytics)나 [Console 분석](https://platform.claude.com/claude-code)을 봐야 합니다. 둘은 목적이 다릅니다.
- <strong>수치를 절대 기준으로 받아들일 때</strong>: 세션 분류나 만족도 같은 정성 평가는 보조 모델이 추정한 값입니다. 방향을 잡는 참고치로 쓰되, 한 번의 결과를 성과 평가처럼 다루지는 마세요.
- <strong>개인정보가 민감한 환경</strong>: 리포트는 로컬 사용 기록을 읽어 들입니다. 공용 화면에 띄우거나 캡처를 공유할 때는 프로젝트 경로나 파일명이 그대로 노출될 수 있다는 점을 염두에 두세요.

병렬 세션을 자주 쓴다면 [Git Worktree로 Claude Code 병렬 세션 운영하기](/ko/blog/ko/claude-code-parallel-sessions-git-worktree)와 함께 보면, 컨텍스트 한도로 끊기는 문제를 세션 분리로 푸는 구체적인 방법까지 이어집니다.
...
Claude Code `/insights`는 단순한 통계 기능이 아닙니다. AI 코딩 도구와의 협업 방식을 데이터로 보여주고, 구체적인 개선 방향까지 제시하는 <strong>AI 워크플로우 코치</strong>에 가깝습니다.

4,516개의 메시지, 1,042개의 세션, 6,267개의 파일 수정. 이 숫자들이 말해주는 건 단순히 "많이 썼다"가 아닙니다. "어떻게 쓰고 있고, 어디를 손보면 더 나아질 수 있는가"입니다.
