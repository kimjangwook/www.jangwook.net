...
데이터 포맷은 코드에서 거의 자동으로 정해지는 값이라 평소 의식하지 않는다. 그런데 LLM 컨텍스트에 들어가는 순간, 그 무심한 `indent=2` 하나가 토큰 청구서의 절반을 만들 수 있다. 직접 재보기 전엔 나도 그 크기를 과소평가하고 있었다. 청구서의 나머지 절반인 단가 쪽은 지금도 흔들리고 있다. [Anthropic과 OpenAI가 같은 달에 IPO를 신청하면서 토큰 단가가 어떻게 움직였는지](/ko/blog/ko/anthropic-openai-ipo-token-price-war-developer-guide-june-2026/)는 따로 정리해뒀다. 단가는 내가 못 정하지만, 토큰 수는 내가 정한다.
