...
GPU 없이 언어 모델을 훈련할 수 있다면? 최근 Reddit의 r/LocalLLaMA 커뮤니티에서 <strong>CPU만으로 1.2시간 만에 1,360만 파라미터 언어 모델을 훈련</strong>한 프로젝트가 공개되어 큰 관심을 모았다. FlashLM v3라 불리는 이 모델은 행렬 곱셈(MatMul)을 완전히 제거한 아키텍처를 사용하며, 추론 시에는 덧셈과 뺄셈만으로 동작한다.

이 글에서는 MatMul-Free 아키텍처의 핵심 원리, FlashLM v3의 구조, 그리고 엣지 AI와 저비용 학습에 대한 시사점을 살펴본다.

## MatMul-Free 아키텍처란?

### 행렬 곱셈의 문제

전통적인 Transformer 모델에서 가장 많은 연산을 소비하는 것은 Attention과 FFN(Feed-Forward Network) 레이어의 <strong>행렬 곱셈</strong>이다. 이 연산은 O(n²d) 또는 O(nd²)의 복잡도를 가지며, GPU의 병렬 처리 능력에 크게 의존한다.

2024년 UC Santa Cruz의 연구팀이 발표한 논문 <strong>"Scalable MatMul-free Language Modeling"</strong>(arXiv:2406.02528)은 이 행렬 곱셈을 완전히 제거하면서도 수십억 파라미터 규모에서 경쟁력 있는 성능을 달성할 수 있음을 보여주었다.

### 삼진 가중치(Ternary Weights)

MatMul-Free 모델의 핵심은 가중치를 <strong>{-1, 0, +1}</strong> 세 가지 값으로 제한하는 것이다. 이렇게 하면:
...
FlashLM v3는 MatMul-Free 개념을 실제로 구현한 오픈소스 모델이다.
...
개발자가 공유한 가장 놀라운 발견은 <strong>훈련 시간의 86%가 출력 레이어에 소비</strong>되었다는 점이다.
...
256차원을 50,257개 어휘로 투영하는 소프트맥스 출력 레이어가 전체 연산의 대부분을 차지했다. 즉, "효율적인" 삼진 코어가 비효율적인 소프트맥스 헤드에 의해 사실상 학습 신호를 제대로 받지 못한 것이다.

v4에서는 소프트맥스를 <strong>계층적 트리 구조</strong>로 대체하여 이 병목을 해결할 계획이며, 같은 시간 내 5~10배 효과적인 훈련이 가능할 것으로 예상된다.

## Scalable MatMul-free LM 논문과의 관계

FlashLM v3는 UC Santa Cruz의 MatMul-Free 논문에서 영감을 받았지만, 몇 가지 차이점이 있다:
...
MatMul-Free 아키텍처는 GPU 접근이 제한된 환경에서의 AI 개발 가능성을 열어준다:
...
삼진 가중치의 가장 큰 장점은 <strong>엣지 디바이스에서의 추론 효율성</strong>이다:
...
MatMul-Free 아키텍처는 아직 초기 단계이지만, 몇 가지 발전 방향이 기대된다:
...
FlashLM v3는 "GPU가 없어도 언어 모델을 훈련할 수 있다"는 가능성을 실증한 흥미로운 프로젝트다. 비록 현재는 연구 프로토타입 수준이지만, MatMul-Free 아키텍처가 발전하면 <strong>AI 민주화</strong>의 중요한 축이 될 수 있다.

특히 출력 레이어 병목 현상의 발견은 향후 효율적 아키텍처 설계에 귀중한 인사이트를 제공한다. GPU 없는 AI의 시대가 오기까지는 아직 갈 길이 멀지만, 그 첫걸음은 이미 시작되었다.
