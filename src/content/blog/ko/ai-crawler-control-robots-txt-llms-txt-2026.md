...
내 입장은 이렇다. llms.txt는 <strong>지금은 로또가 아니라 보험</strong> 정도다. 생성 비용이 거의 없고 표준이 자리 잡을 가능성에 대비하는 의미는 있으니 넣어도 된다. 하지만 "llms.txt 넣었으니 AI 검색에 잘 잡히겠지"는 근거 없는 기대다. 그 시간에 차라리 위에서 정리한 robots.txt의 봇별 제어와 구조화 데이터를 손보는 편이 실측 효과가 훨씬 크다. 그 뒤 Search Central이 공식 가이드에 llms.txt를 무시한다고 적었다. [라이브 robots.txt와 Search Console 스위치를 잰 기록](/ko/blog/ko/official-geo-subtraction-gsc-control-2026/)이 그 문장을 공개 URL로 확인한다.
...
<strong>2026-07-04 후속</strong>: 이 전략을 이 블로그의 robots.txt에 그대로 적용했다. 학습 봇 4종(GPTBot·ClaudeBot·CCBot·Google-Extended)은 Disallow, 검색 봇 3종(OAI-SearchBot·Claude-SearchBot·PerplexityBot)은 Allow. 적용하면서 본문에서 경고한 파서 차이를 실제로 밟았다 — first-match 파서에서 `Allow: /`가 뒤의 Disallow를 가리는 문제가 있어, Disallow를 Allow보다 앞에 배치하는 것으로 두 파서 계열 모두에서 동일하게 동작함을 12개 시나리오로 검증했다.
