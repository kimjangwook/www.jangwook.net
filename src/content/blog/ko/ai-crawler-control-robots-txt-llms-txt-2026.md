...
내 입장은 이렇다. llms.txt는 <strong>지금은 로또가 아니라 보험</strong> 정도다. 생성 비용이 거의 없고 표준이 자리 잡을 가능성에 대비하는 의미는 있으니 넣어도 된다. 하지만 "llms.txt 넣었으니 AI 검색에 잘 잡히겠지"는 근거 없는 기대다. 그 시간에 차라리 위에서 정리한 robots.txt의 봇별 제어와 구조화 데이터를 손보는 편이 실측 효과가 훨씬 크다. 그 뒤 Search Central이 공식 가이드에 llms.txt를 무시한다고 적었다. 라이브 robots.txt와 Search Console 스위치를 잰 기록이 그 문장을 공개 URL로 확인한다.
