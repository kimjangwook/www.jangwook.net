...
RAG 앱을 처음 만들려고 문서를 펼치면 세 개의 이름이 거의 동시에 등장한다. LlamaIndex, LangChain, Haystack. 셋 다 "RAG 프레임워크"라고 불리지만, 직접 써보면 철학이 상당히 다르다.

이번에 임시 샌드박스에서 세 프레임워크를 모두 설치하고 동일한 조건으로 테스트했다. pip 설치부터 InMemory 검색 실행까지, 그리고 예상치 못한 deprecation 경고까지 발견한 것을 정리한다.
...
RAG 파이프라인을 넘어 에이전트 오케스트레이션으로 확장할 계획이라면, [LangGraph, CrewAI, Dapr 프레임워크 비교](/ko/blog/ko/ai-agent-framework-comparison-2026-langgraph-crewai-dapr-production)를 함께 읽어두면 전체 그림을 잡는 데 도움이 된다.
