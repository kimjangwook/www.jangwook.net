...
이를 통해 CTO는 "현재 조직에서 운영 중인 에이전트가 몇 개이고, 각각 어떤 시스템에 접근 권한을 가지고 있는지"를 대시보드로 확인할 수 있습니다. [여러 역할의 에이전트를 팀으로 조직하는 방법](/ko/blog/ko/claude-agent-teams-guide)과 ADL 기반 카탈로그를 결합하면 조직 전체의 AI 거버넌스 체계가 구체화됩니다.
...
아직 초기 단계이지만, Apache 2.0 라이선스의 오픈소스 표준이므로 지금부터 관심을 갖고 시범 도입을 검토하기에 좋은 시점입니다. [엔터프라이즈 AI 도입 의사결정 프레임워크](/ko/blog/ko/enterprise-ai-adoption-topdown)를 참고해 조직의 준비도를 점검한 뒤 ADL 도입 범위를 결정하는 것이 현실적인 접근입니다.
