...
2026년 AI 에이전트 생태계는 폭발적으로 성장하고 있지만, 한 가지 심각한 문제가 있다. <strong>에이전트의 정의가 코드, 프롬프트, 설정 파일에 흩어져 있어 "이 에이전트가 정확히 무엇을 할 수 있는지" 파악하기가 어렵다</strong>는 것이다.

Next Moca가 Apache 2.0 라이선스로 공개한 <strong>Agent Definition Language(ADL)</strong>는 이 문제를 정면으로 해결한다. API 세계에서 OpenAPI(Swagger)가 "API가 무엇을 하는지"를 선언적으로 정의했듯이, ADL은 "AI 에이전트가 무엇을 하는지"를 벤더 중립적으로 정의하는 표준 사양이다.

ADL의 핵심 구조, 기존 표준(MCP, OpenAPI, A2A)과의 관계, 그리고 Engineering Manager와 CTO가 주목해야 할 거버넌스 전략을 실무 관점에서 짚는다.

## ADL이 해결하는 문제

현재 대부분의 조직에서 AI 에이전트는 다음과 같은 상태로 관리된다.
...
ADL 사양은 JSON Schema 기반으로, 에이전트를 6가지 모듈로 선언적으로 정의한다.
...
시맨틱 버저닝과 소유자 정보를 통해 <strong>"누가 이 에이전트를 관리하고 어떤 버전이 운영 중인지"</strong>를 즉시 파악할 수 있다.
...
LLM 프로바이더와 모델을 명시적으로 선언하면, 모델 교체 시 변경 이력을 추적하고 성능 회귀를 감지할 수 있다.
...
각 도구의 호출 방식(Python 함수, HTTP, MCP 등)과 파라미터를 명시하면, 보안팀이 <strong>에이전트가 접근 가능한 외부 시스템의 전체 목록</strong>을 사전에 검토할 수 있다.
...
이것이 ADL의 가장 강력한 부분이다. 네트워크 접근, 파일 시스템, 환경 변수 노출 범위를 <strong>코드가 아닌 선언적 사양으로 정의</strong>하면 CI/CD 파이프라인에서 자동 검증이 가능해진다.

## ADL과 기존 표준의 관계

ADL은 기존 표준을 대체하는 것이 아니라, <strong>"정의 계층(Definition Layer)"</strong>으로서 보완한다.
...
핵심 차이점은 <strong>ADL은 런타임 프로토콜이 아니라 정적 사양</strong>이라는 것이다. 에이전트의 "설계도"를 제공하며, 실제 실행은 MCP나 A2A 같은 런타임 프로토콜이 담당한다.
...
ADL의 JSON Schema를 활용하면 에이전트 배포 전에 다음을 자동으로 검증할 수 있다.
...
조직 내 모든 에이전트의 ADL 정의를 중앙 저장소에서 관리하면, <strong>에이전트 카탈로그</strong>를 자동으로 구축할 수 있다.
...
이를 통해 CTO는 "현재 조직에서 운영 중인 에이전트가 몇 개이고, 각각 어떤 시스템에 접근 권한을 가지고 있는지"를 대시보드로 확인할 수 있다. [여러 역할의 에이전트를 팀으로 조직하는 방법](/ko/blog/ko/claude-agent-teams-guide/)과 ADL 기반 카탈로그를 결합하면 조직 전체의 AI 거버넌스 체계가 구체화된다.
...
실제 조직에서 ADL로 코드 리뷰 에이전트를 정의하는 전체 예시다.
...
ADL은 아직 초기 단계(Early-Stage Standard)이므로, 단계적 도입이 현실적이다.
...
ADL은 AI 에이전트 개발에서 <strong>"코드와 분리된 선언적 정의"</strong>라는 중요한 계층을 추가한다. OpenAPI가 REST API 생태계를 표준화했듯이, ADL은 에이전트 생태계의 가시성, 거버넌스, 재현성 문제를 해결할 잠재력이 있다.
...
아직 초기 단계이지만, Apache 2.0 라이선스의 오픈소스 표준이므로 지금부터 관심을 갖고 시범 도입을 검토하기에 좋은 시점이다. [엔터프라이즈 AI 도입 의사결정 프레임워크](/ko/blog/ko/enterprise-ai-adoption-topdown/)를 참고해 조직의 준비도를 점검한 뒤 ADL 도입 범위를 정하는 것이 현실적인 접근이다.
