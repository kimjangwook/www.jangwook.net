...
이 블로그를 실험대에 올려 [가설을 숫자로 검증해온 방식](/ko/blog/ko/multilingual-llm-token-tax-experiment) 그대로, 이번엔 렌더링을 직접 재봤다. 같은 LocalBusiness 마크업을 두 방식으로 만들었다. 하나는 서버가 내보낸 정적 HTML, 하나는 JS가 런타임에 주입하는 방식이다.
...
두 방식이 크롤러의 눈에 어떻게 다르게 보이는지 흐름으로 그리면 이렇다.

```mermaid
graph TD
    A["크롤러가 페이지 요청"] --> B["1차: 원시 HTML 수집"]
    B --> C{"원시 HTML에<br/>ld+json 블록이 있는가"}
    C -->|"(A) 서버사이드 출력"| D["즉시 구조화 데이터 인식"]
    C -->|"(B) JS 주입"| E["블록 0개 — 읽을 것이 없음"]
    E --> F["렌더링 큐 대기<br/>(자원 종속)"]
    F --> G["2차: 헤드리스 렌더링"]
    G -->|"성공"| H["DOM에서 ld+json 인식"]
    G -->|"지연·실패"| I["그날 데이터 없음"]
```

실측 결과와 특성을 정리하면:

| 항목 | (A) 서버사이드 출력 | (B) JS 주입 |
|---|---|---|
| 원시 HTML의 ld+json 블록 | 1개 (실측) | 0개 (실측) |
| 1차 크롤 시점 인식 | 즉시 | 불가 |
| 렌더링 큐 의존 | 없음 | 있음 (자원 종속) |
| 실패 모드 | 사실상 없음 | 렌더링 지연·JS 오류 시 데이터 누락 |
| 적합한 데이터 | NAP·좌표 등 정확성이 곧 신뢰인 값 | 서버 출력이 불가능한 위젯·서드파티 연동 |
...
모든 걸 서버사이드로 옮기라는 얘기가 아니다. 다음 상황이면 JS 주입도 실용적인 선택이다.

- <strong>서드파티 위젯이 마크업을 만들 때</strong>: 예약·리뷰 위젯이 자체적으로 JSON-LD를 주입하는 구조라면, 그걸 서버로 옮기는 비용이 이득보다 클 수 있다.
- <strong>서버 템플릿을 만질 수 없을 때</strong>: 레거시 CMS나 태그 매니저(GTM)만 손댈 수 있는 환경이라면 JS 주입이 유일한 선택지다. Google도 GTM 경유 주입을 지원 방식으로 문서화한다.
- <strong>정확성이 화면과 함께 움직일 때</strong>: 어차피 클라이언트에서 계산해 화면에 그리는 값이라면, 마크업도 같은 코드 경로에서 만드는 쪽이 화면-마크업 불일치를 줄인다.

반대로 매장명·주소·전화처럼 값이 거의 바뀌지 않고 정확성이 곧 신뢰인 데이터라면 서버사이드가 기본값이다. 판단 기준은 하나다 — "이 데이터가 2차 렌더링까지 기다려도 되는가, 그리고 렌더링이 실패한 날에도 존재해야 하는가."
...
- 배포 후 Rich Results Test와 URL 검사 도구로 원시와 렌더링 양쪽을 확인한다. [Lighthouse 접근성 점수를 55에서 100까지 실측으로 끌어올린 기록](/ko/blog/ko/a11y-lighthouse-audit-fix-2026)처럼, "JS로 넣었는데 되겠지"라는 추정 대신 측정으로 끝을 본다.
- 순위 효과를 약속하지 않는다. 마크업은 크롤러의 이해를 돕는 보조 장치라는 선을 지킨다.

같은 정보라도 [기계에 어떤 포맷으로 전달하느냐가 비용과 인식을 바꾼다는 걸 토큰 단위로 실측한 적이 있는데](/ko/blog/ko/llm-token-cost-data-format-experiment), 구조화 데이터의 서버사이드·JS 선택도 결국 같은 결의 문제다.
