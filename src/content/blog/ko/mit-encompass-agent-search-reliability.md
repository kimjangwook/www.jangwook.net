...
description: >-
  MIT CSAIL이 개발한 EnCompass 프레임워크로 AI 에이전트의 실행 경로에 검색 전략을 적용해 신뢰성과 정확도를 획기적으로 높이는
  방법을 실무 관점에서 분석합니다.
pubDate: '2026-03-04'
...
2026년, AI 에이전트가 프로덕션 환경에서 점점 더 많은 역할을 맡고 있지만, <strong>신뢰성</strong>이라는 근본적인 과제는 여전히 해결되지 않고 있다. LLM 기반 에이전트는 본질적으로 확률적이기 때문에 같은 작업을 시켜도 매번 다른 결과를 내놓을 수 있고, 한 단계에서의 실수가 이후 모든 작업에 연쇄적으로 영향을 미친다.

MIT CSAIL과 Asari AI가 개발한 <strong>EnCompass</strong> 프레임워크는 이 문제에 대해 근본적으로 다른 접근법을 제시한다. 에이전트 프로그램의 실행 경로에 <strong>검색 알고리즘</strong>(Beam Search, Monte Carlo Tree Search 등)을 접목하여, 에이전트가 실수했을 때 자동으로 백트래킹하고 더 나은 경로를 탐색하게 만드는 것이다. 결과적으로 <strong>15〜40%의 정확도 향상</strong>을 달성했으며, 구현에 필요한 코드는 오히려 82% 줄었다.

이 글에서는 EnCompass의 핵심 개념, 동작 원리, 그리고 Engineering Manager 관점에서의 실무 적용 전략을 분석한다.

## AI 에이전트 신뢰성 문제의 본질

### 왜 에이전트는 실패하는가

LLM 기반 AI 에이전트가 프로덕션에서 실패하는 핵심 원인은 <strong>오류의 연쇄 전파</strong>다.
...
전통적인 에이전트 시스템은 <strong>단일 실행 경로</strong>만 따른다. Step 1에서 LLM이 잘못된 판단을 하면, 이후 모든 단계가 그 잘못된 판단 위에 쌓이게 된다. 재시도(retry) 로직을 넣어도, 같은 컨텍스트에서 같은 실수를 반복하는 경우가 많다.
...
이 모든 접근법의 공통적인 한계는, <strong>이미 선택한 경로에서 벗어나지 못한다</strong>는 점이다.

## EnCompass: 검색 기반 에이전트 실행

### 핵심 아이디어 — "Choose Your Own Adventure"

EnCompass의 핵심 아이디어는 놀라울 정도로 직관적이다. 에이전트 프로그램의 실행을 <strong>"이야기"</strong>에 비유하면:

- <strong>기존 방식</strong>: 하나의 줄거리를 따라가는 소설
- <strong>EnCompass 방식</strong>: 분기점마다 선택지가 있는 "게임북(Choose Your Own Adventure)"

개발자는 에이전트 코드의 특정 지점에 <strong>"분기점(branchpoint)"</strong>이라는 어노테이션을 추가한다. EnCompass는 이 분기점에서 여러 가능한 LLM 출력을 탐색하고, 가장 좋은 결과를 내는 경로를 자동으로 선택한다.
...
EnCompass의 동작은 3단계로 이루어진다.

<strong>1단계: 분기점 정의</strong>

개발자가 에이전트 코드에서 LLM 호출이 일어나는 지점을 분기점으로 표시한다. 이것은 "여기서 LLM의 출력이 달라질 수 있고, 그 차이가 최종 결과에 영향을 준다"는 것을 선언하는 것이다.

<strong>2단계: 평가 함수 정의</strong>

각 단계의 결과가 얼마나 좋은지 평가하는 함수를 정의한다. 예를 들어 코딩 에이전트라면 "테스트 통과율"이 평가 함수가 될 수 있다.

<strong>3단계: 검색 전략 선택</strong>

EnCompass는 다양한 검색 전략을 지원한다:
...
EnCompass를 사용한 에이전트 코드의 개념적 구조다:
...
핵심은 <strong>기존 에이전트 로직을 거의 수정하지 않고</strong> 어노테이션만 추가하면 된다는 점이다. MIT 연구팀에 따르면, 수동으로 검색을 구현할 때 대비 <strong>348줄의 코드(약 82%)를 절약</strong>할 수 있었다.
...
<strong>주목할 점</strong>은 "2-level Beam Search"가 최적 전략으로 밝혀졌다는 것이다. 이는 단순히 무작위로 많이 시도하는 것보다, <strong>구조화된 검색 전략이 효과적</strong>이라는 것을 의미한다.

### 비용 대비 효과 분석

검색 예산이 16배라는 것은 LLM API 호출 비용도 16배가 된다는 뜻이다. 이것이 실무에서 합리적인지 판단해봅시다:
...
단순 비용만 보면 EnCompass가 비싸지만, <strong>[실패한 작업의 후처리 비용](/ko/blog/ko/ai-agent-cost-reality)</strong>(인간의 수동 수정, 재작업, 품질 이슈)을 포함하면 이야기가 달라진다. 특히 고가치 작업(코드 리뷰, 보안 분석 등)에서는 정확도 향상의 가치가 추가 비용을 충분히 상쇄한다.

## 실무 적용 전략

### Engineering Manager 관점의 도입 가이드

EnCompass의 개념을 실무에 적용할 때 고려해야 할 사항들을 정리한다.

<strong>1. 적용 대상 선별</strong>

모든 에이전트 작업에 검색을 적용할 필요는 없다. 다음 기준으로 선별한다:
...
EnCompass의 효과는 <strong>평가 함수의 품질</strong>에 크게 좌우된다. 좋은 평가 함수의 조건:
...
EnCompass 외에도 에이전트 신뢰성을 높이기 위한 여러 움직임이 있다:
...
이러한 움직임들의 공통된 방향은 <strong>"에이전트를 더 예측 가능하고 제어 가능하게 만드는 것"</strong>이다.

## 결론

MIT EnCompass는 AI 에이전트의 신뢰성 문제에 대해 <strong>근본적이면서도 실용적인 해결책</strong>을 제시한다. 핵심 인사이트를 정리하면:
...
Engineering Manager로서 가장 중요한 시사점은, <strong>"AI 에이전트의 성능은 모델 자체만의 문제가 아니라 하네스(harness)의 문제"</strong>라는 것이다. 같은 LLM이라도 어떤 실행 전략을 채택하느냐에 따라 결과가 크게 달라진다.

프로덕션 AI 에이전트를 운영하고 있다면, 단순히 더 좋은 모델을 기다리기보다 <strong>실행 전략 자체를 개선</strong>하는 것이 더 빠르고 확실한 성과를 낼 수 있다.
