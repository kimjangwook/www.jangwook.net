...
실험 환경은 repo 밖 임시 디렉터리에 만들었다. 추측 대신 직접 재서 결론을 내리는 건 [데이터 포맷별 토큰 비용을 9종으로 실측했을 때](/ko/blog/ko/llm-token-cost-data-format-experiment)와 같은 방식이다. Ollama 0.30.7, Apple Silicon, 모델은 두 개를 썼다. 하나는 2GB짜리 작은 Gemma 4 빌드, 하나는 9.6GB짜리 `gemma4:e4b`. 크기가 다른 모델에서 같은 패턴이 보이는지 확인하려는 의도였다.
