...
2026년 1월 26일, Model Context Protocol 팀이 조용히 발표한 기능 하나가 AI 에이전트 UX의 패러다임을 바꾸고 있다. 바로 **MCP Apps**인다. AI가 텍스트 응답 대신 AI 채팅창 안에서 직접 동작하는 인터랙티브 대시보드, 폼, 데이터 시각화를 돌려줄 수 있게 되었다.

Engineering Manager 입장에서 이것이 왜 중요한지 한마디로 정리하면: 지금까지 AI 에이전트가 "데이터를 보여주면" 사용자는 그 텍스트를 읽고 다시 수동으로 조작해야 했다. MCP Apps는 그 간극을 제거한다.

## MCP Apps란 무엇인가

MCP Apps는 MCP(Model Context Protocol)의 첫 번째 공식 확장(extension)으로, <strong>도구 호출(tool call)의 응답으로 인터랙티브 HTML UI를 반환할 수 있게 해주는 프로토콜</strong>이다.

기존 MCP 도구는 텍스트, 이미지, 구조화된 데이터를 반환했다. MCP Apps를 사용하면 같은 도구 호출이 다음과 같은 것을 반환할 수 있다:
...
그리고 이 UI가 <strong>채팅창 안에서, 대화 컨텍스트 안에서</strong> 동작한다.

## 왜 그냥 웹앱 링크를 주는 것과 다른가

"그냥 링크 주면 되지 않나?"라고 생각할 수 있다. MCP Apps가 별도 웹앱과 근본적으로 다른 이유는 네 가지다.

<strong>1. 컨텍스트 보존</strong>

UI가 대화 안에 존재한다. 사용자가 탭을 전환하거나, 어떤 채팅에서 그 대시보드를 봤는지 기억을 더듬을 필요가 없다. 대화 흐름에 자연스럽게 UI가 녹아들어 있다.

<strong>2. 양방향 데이터 흐름</strong>

MCP App은 MCP 서버의 모든 도구를 호출할 수 있고, 호스트는 새로운 결과를 앱에 푸시할 수 있다. 별도 웹앱이라면 자체 API, 인증, 상태 관리가 필요하지만 MCP Apps는 이미 존재하는 MCP 패턴을 그대로 활용한다.

<strong>3. 호스트 기능 통합</strong>

앱이 호스트에게 작업을 위임할 수 있다. "이 미팅을 일정에 추가해줘"라는 요청을 앱이 호스트에 보내면, 호스트가 사용자가 이미 연결해둔 캘린더 통합을 통해 처리한다. 모든 외부 통합을 앱이 직접 구현할 필요가 없다.
...
MCP Apps는 두 가지 MCP 프리미티브를 결합한다: UI 리소스를 선언하는 도구(tool)와, 그 데이터를 인터랙티브 HTML로 렌더링하는 UI 리소스다.
...
도구 설명(tool description)에 `_meta.ui.resourceUri` 필드가 포함되어 있다. 이 필드는 `ui://` 리소스를 가리킨다. 호스트는 도구가 호출되기 전에 이 리소스를 미리 로드할 수 있어, 스트리밍 입력 등의 기능이 가능해진다.

<strong>Step 2: 리소스 패치</strong>

호스트가 서버에서 UI 리소스를 가져온다. 이 리소스는 HTML 페이지로, 보통 JavaScript와 CSS가 번들링되어 있다.

<strong>Step 3: Sandboxed 렌더링</strong>

호스트가 대화 안에서 sandboxed iframe으로 HTML을 렌더링한다. sandbox가 앱의 부모 페이지 접근을 제한한다.

<strong>Step 4: 양방향 통신</strong>

앱과 호스트는 `ui/` 메서드 이름 접두사를 가진 JSON-RPC 프로토콜로 통신한다. 앱은 도구 호출 요청, 메시지 전송, 모델 컨텍스트 업데이트, 호스트로부터 데이터 수신이 가능하다.

## 실전 구현: MCP App 서버 만들기

이제 직접 MCP App 서버를 구현해봅시다. 간단한 매출 분석 대시보드 예제다.
...
2026년 3월 기준으로 MCP Apps를 지원하는 클라이언트는 다음과 같다:
...
MCP Apps를 도입할 때 EM이 판단해야 할 포인트들을 정리했다.

### 어떤 경우에 MCP Apps가 적합한가

<strong>복잡한 데이터 탐색</strong>이 반복되는 경우다. 팀원이 AI에게 "이번 달 장애 현황 요약해줘"라고 물을 때마다 텍스트 응답을 읽고 다시 대시보드를 열어 확인한다면, MCP Apps로 대시보드를 채팅 안에 내장하면 된다.

<strong>다단계 설정/승인 워크플로우</strong>도 적합하다. 인프라 배포 설정, 비용 승인, 코드 리뷰 트리아지 같은 작업은 "하나씩 물어보는 대화" 방식보다 전체 옵션을 한눈에 보여주는 폼이 훨씬 효율적이다.

<strong>실시간 모니터링</strong>도 강점이다. 채팅에 질문을 던지고, 그 자리에서 라이브 메트릭 대시보드가 뜨는 경험은 기존 방식과 근본적으로 다르다.

### 도입 시 주의사항

<strong>번들 크기 관리</strong>: UI 리소스는 채팅에서 로드되므로 초기 로드 성능이 중요하다. React 전체 번들보다는 Preact나 vanilla JS를 사용하는 것이 좋다.

<strong>CSP(Content Security Policy) 설정</strong>: 외부 스크립트, API 엔드포인트를 명시적으로 선언해야 한다. 보안팀과 협의해 허용 도메인 목록을 관리해야 한다.

<strong>Fallback 설계</strong>: MCP Apps를 지원하지 않는 클라이언트에서도 도구가 유용한 텍스트 응답을 반환하도록 fallback을 항상 설계해야 한다.

<strong>사용자 동의 플로우</strong>: UI에서 도구를 호출할 때 호스트가 사용자 동의를 요청한다. 이 UX를 자연스럽게 설계해야 한다.

### 클라이언트 구현 (자체 호스트 구축 시)

자체 AI 클라이언트를 구축 중인 팀이라면 두 가지 옵션이 있다:
...
공식 저장소에 있는 실제 예제들을 보면 가능성의 범위를 실감할 수 있다.
...
모든 예제는 React, Vue, Svelte, Preact, Solid, vanilla JS 버전이 제공된다.

## 결론

MCP Apps는 AI 에이전트 인터페이스의 근본적인 한계를 해결한다. 텍스트로만 소통하던 AI가 이제 <strong>살아있는 UI를 대화 안에서 직접 구동</strong>할 수 있게 되었다.
