...
description: >-
  17개 Agent와 메타데이터 우선 아키텍처로 구현한 블로그 완전 자동화 시스템 심층 분석. 60〜70% 토큰 절감, 연간 71% 비용
  절감($5.72→$1.65), 연 364시간 절약을 달성한 3-Tier 아키텍처 설계 원리와 핵심 성능 메트릭을 상세히 공개합니다.
pubDate: '2025-11-14'
...
relatedPosts:
  ...
  - slug: ai-agent-cost-reality
    ...
    reason:
      ...
      zh: 在同一 自动化 脉络中可一并阅读。
faq:
  - question: "메타데이터 우선 아키텍처는 어떻게 71% 비용을 절감하나요?"
    answer: "추천 알고리즘이 실제로 쓰는 건 제목, 설명, 태그, 카테고리 점수뿐인데 기존에는 매번 본문 전체를 읽어 추천 1회당 9만 토큰을 썼습니다. 메타데이터를 한 번만 추출해 post-metadata.json으로 재사용하면 연간 비용이 $5.72에서 $1.65로 줄어듭니다."
  - question: "왜 TF-IDF 대신 Claude LLM을 추천에 사용했나요?"
    answer: "TF-IDF는 빠르고 저렴하지만 의미를 이해하지 못하고 동의어를 처리하지 못합니다. EffiFlow는 의미와 맥락을 파악하는 Claude LLM을 선택했고, 비용 문제는 메타데이터 우선 아키텍처로 해결했습니다."
  - question: "포스트 작성 시간이 실제로 얼마나 줄어드나요?"
    answer: "수동으로는 리서치, 글쓰기, 이미지, 번역까지 포스트당 4시간 40분이 걸렸습니다. 8-Phase 자동화 후에는 명령 입력과 대기, 검토를 합쳐 약 30분으로 줄어 90%를 절감합니다."
  - question: "증분 처리는 어떻게 추가 비용을 줄이나요?"
    answer: "Content Hash로 변경된 포스트만 골라 재분석합니다. 13개 전체 분석은 2분에 $0.09가 들지만, 신규 2〜3개만 처리하면 20초에 약 $0.02로 79% 추가 절감됩니다."
---
...
블로그 자동화를 1년 가까이 돌리다 보니 한 가지가 계속 걸렸습니다. "이거 더 줄일 수 있지 않나?" 결국 7.5시간을 들여 `.claude/` 디렉토리의 28개 파일을 전부 뜯어봤습니다. 17개 Agents, 4개 Skills, 7개 Commands. 숫자만 보면 별것 아닌데, 막상 토큰 사용량을 추적해 보니 생각보다 새는 곳이 많았습니다.
