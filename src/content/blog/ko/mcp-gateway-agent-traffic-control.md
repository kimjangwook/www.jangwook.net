...
[MCP 서버를 만들면](/ko/blog/ko/mcp-server-build-practical-guide-2026) 도구(tool)를 정의하고, 클라이언트가 그 도구를 호출한다. 인증? OAuth 2.1이 스펙에 들어갔다. 하지만 "이 에이전트가 이 도구를 하루에 몇 번까지 호출할 수 있는가", "민감한 데이터를 반환하는 도구는 승인 없이 호출하면 안 된다" 같은 정책 레이어는 MCP 프로토콜 자체에 없다. 그건 구현하는 쪽의 몫이다.
...
내가 직접 겪은 케이스를 하나 들자면, [Claude Code](/ko/blog/ko/claude-code-parallel-sessions-git-worktree)에서 Notion MCP로 페이지를 수정하다가 의도치 않게 다른 팀의 페이지를 건드린 적이 있다. 에이전트가 검색 결과에서 비슷한 제목의 페이지를 골랐고, 나는 승인 버튼을 별 생각 없이 눌렀다. 데이터가 날아간 건 아니지만 민망했다.
