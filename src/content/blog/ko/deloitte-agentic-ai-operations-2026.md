...
description: >-
  ...
  중입니다. 89%가 실패하는 원인은 기술이 아닌 운영 모델. EM·VPoE가 즉시 쓸 수 있는 Delegate·Review·Own
  프레임워크를 소개합니다.
pubDate: '2026-03-08'
...
2024년부터 "AI 에이전트 시대"가 온다고 했다. 그리고 일부 기업들은 정말로 멋진 데모를 만들었다. Slack 메시지를 받고 자동으로 Jira 티켓을 생성하고, 데이터를 분석하고, 보고서를 작성하는 에이전트들 말이다.

그러나 Deloitte의 2026 Tech Trends 보고서가 폭로한 현실은 차갑다:

**<strong>전 세계 기업의 11%만이 Agentic AI를 프로덕션 환경에서 실제로 운영하고 있다.</strong>**
...
<strong>89%의 기업이 실제 운영(production)에 도달하지 못하고 있다.</strong> 이는 단순한 지연이 아니다. 이것은 조직적 운영 모델의 실패다.

이 포스트에서 다루는 것은 "왜 AI 에이전트를 도입해야 하는가"가 아니다. (그것은 이전 포스트 "생성형 AI 도입, 왜 탑다운 방식이 필요한가"에서 다뤘다.) 대신 <strong>실제로 어떻게 운영해야 하는가</strong>를 EM/VPoE 관점에서 구체적으로 제시한다.
...
이 단계의 조직들은 다음 특징을 보인다:

- <strong>분산된 PoC (Proof of Concept)</strong>: 각 팀이 독립적으로 작은 에이전트를 만들고 있다
- <strong>명확한 소유권 부재</strong>: "누가 이걸 책임지나요?" 답변이 없다
- <strong>토큰 비용에 대한 충격</strong>: 몇 개의 작은 에이전트만 돌려도 월간 청구서가 수만 달러다
- <strong>연속성 없음</strong>: 한 사람이 떠나면 에이전트도 따라 떠난다
...
- <strong>중앙 AI 팀이 구성된다</strong> (또는 시도된다)
- <strong>몇 개의 주요 워크플로우가 선택된다</strong>
- <strong>초기 성공을 거둔다</strong>: 개발자가 코드 리뷰 요청을 자동화했더니 정말 도움이 된다
- <strong>그리고 나서 정체된다</strong>: 5개 이상의 에이전트를 동시에 운영하기 시작하면, 관리 복잡성이 폭증한다
...
가장 위험한 상태다. 조직이 AI 에이전트의 필요성은 느끼지만 어떻게 진행해야 할지 모른다.

이 단계의 조직들이 보이는 패턴:

- <strong>높은 기대, 낮은 기대치</strong>: CEO는 "AI로 효율을 2배 올려달라"고 하지만, 팀은 "기본 인프라도 없는데 뭘 어떻게"라고 생각한다
- <strong>거버넌스 공백</strong>: AI 에이전트가 회사 데이터에 접근하지만, 누가 감시할지 정해지지 않았다
- <strong>토큰 비용 쇼크</strong>: Gartner에 따르면, 토큰 비용은 2년 동안 280배 떨어졌는데, 엔터프라이즈 월간 청구서는 수천만 달러다. 왜? 사용량이 폭증했기 때문이다.
...
**<strong>아젠틱 AI 프로젝트의 40%가 2027년까지 실패할 것이다. 원인은 기술 부족이 아니라, 조직이 깨진 프로세스에 에이전트를 얹혀 있기 때문이다.</strong>**
...
에이전트를 단순히 "인간의 수동 작업 부분"에 치환하는 것이다. 이것은 조직 운영의 근본적인 병목을 해결하지 못한다.
...
<strong>전체 작업의 80%가 "지루한" 일들이다.</strong>
...
에이전트를 도입하면, <strong>이 지루한 작업이 갑자기 가시화된다.</strong>
...
예상하지 못한 이 "숨은 작업"들이 에이전트 도입의 진정한 비용이다.
...
선도 기업들은 다음을 이해한다:
- 에이전트는 24/7 실행된다 (업무 시간만 아니라)
- 각 워크플로우마다 여러 에이전트가 필요하다
- 재시도, 예외 처리, 감시로 인해 토큰 사용량이 5〜10배 증가한다

**올바른 운영 모델에서는:**
- 에이전트가 "언제 실행되어야 하는지" 정의한다
- 토큰 사용을 모니터링하고 이상 탐지한다
- 각 에이전트의 비용을 측정하고 ROI를 추적한다

---

## 파트 3: Delegate, Review, Own 프레임워크

이것이 **HBR와 Google Cloud가 제시하는 "Enterprise-Wide Agentic AI Transformation"의 핵심 운영 모델**이다.
...
이 단계는 <strong>기술이 아니라 조직 설계다.</strong>
...
이것이 <strong>89%의 조직이 실패하는 지점</strong>이다.

많은 조직이 다음 중 하나의 극단으로 간다:
...
**이것이 가장 중요하다.** 89%의 실패 조직들은 이 단계를 건너뛴다.
...
당신이 EM이나 VPoE라면, 이번 주 월요일 아침에 이 체크리스트를 실행하라.
...
**<strong>Senior leadership이 AI 거버넌스에 직접 관여하는 기업은 그렇지 않은 기업보다 3배 더 많은 비즈니스 가치를 창출한다.</strong>**

이것은 왜일까?

### 거버넌스의 진정한 의미

거버넌스는 "에이전트가 뭘 하도록 할 것인가"가 아니다. (그것은 기술 결정이다.)

거버넌스는 **"조직이 AI 에이전트를 통해 어떤 가치를 창출할 것인가"를 정의하는 것**이다.
...
Deloitte의 현실은 차갑지만 명확하다:

**<strong>기술은 준비되어 있다. 남은 것은 운영 모델이다.</strong>**

당신이 EM이나 VPoE라면:

1. **이번 주 월요일**: 조직의 현황을 파악하라. "우리는 Deloitte 보고서의 11%에 속하는가, 89%에 속하는가?"

2. **이번 달**: Delegate, Review, Own 프레임워크를 도입하라. 한 번에 조직 전체를 바꾸려 하지 말고, 한 에이전트부터 시작하라.

3. **이번 분기**: 거버넌스 정책을 수립하라. 토큰 비용을 추적하고, 성과를 측정하라.

4. **3개월 후**: 당신의 조직이 11%에 진입했는지, 여전히 89%에 있는지 평가하라.

HBR의 말이 옳다: <strong>"Senior leadership이 AI 거버넌스에 직접 관여하는 기업은 그렇지 않은 기업보다 3배 더 많은 비즈니스 가치를 창출합니다."</strong>

당신의 리더십이 이 변화의 중심에 있을 때, 조직은 비로소 Agentic AI 시대에 도달할 수 있다.
