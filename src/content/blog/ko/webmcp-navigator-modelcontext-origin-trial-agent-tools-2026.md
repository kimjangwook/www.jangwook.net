...
방향이 반대인 접근도 같이 놓고 보면 구도가 선명해진다. WebMCP가 에이전트를 페이지 쪽으로 들여보낸다면, [MCP Apps는 UI를 채팅 쪽으로 끌어온다](/ko/blog/ko/mcp-apps-interactive-ui-agent-ux/). 같은 문제를 서로 반대편에서 푸는 셈이고, 어느 쪽을 고르느냐는 결국 사용자의 세션이 어디에 있느냐로 갈린다.
