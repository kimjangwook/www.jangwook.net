...
덧붙이면 `Cache-Control`은 캐시 판단의 한 축일 뿐이다. 나머지 축인 검증자(ETag·Last-Modified)는 저장 여부가 아니라 재검증 비용을 정한다. 그 검증자가 배포 한 번에 통째로 리셋되는 문제는 [배포가 지우는 캐시 검증자](/ko/blog/ko/etag-deploy-invalidation-conditional-requests-2026/)에서 따로 쟀다.
