...
ChatGPT, Claude, Gemini… 요즘 AI 챗봇 안 써본 사람이 없을 것이다. 그런데 한 번이라도 이런 생각 해본 적 없는가?
...
바로 이런 고민을 해결하기 위해 등장한 프로젝트가 있다. <strong>OpenClaw</strong> 인다.

오늘은 OpenClaw이 뭔지, 뭐가 특별한지, 그리고 어떤 사람에게 딱 맞는 도구인지 낱낱이 소개해 드리겠다!

---

## OpenClaw이란?

<strong>OpenClaw</strong>은 오픈소스 <strong>개인 AI 비서 플랫폼</strong>이다.

쉽게 말하면, 여러분이 이미 매일 쓰고 있는 메신저(텔레그램, WhatsApp, Discord 등)에 <strong>나만의 AI 비서를 붙일 수 있는 시스템</strong>이다. AI 모델을 직접 골라 연결하고, 다양한 도구와 스킬을 추가해서 나만의 워크플로우를 만들 수 있다.
...
MIT 라이선스이기 때문에 개인이든 회사든 마음껏 사용하고 수정할 수 있다. 커뮤니티도 활발하게 운영 중이니 궁금한 게 있으면 Discord에 들어가 보면 된다.

### GitHub 저장소 둘러보기

아래는 OpenClaw의 GitHub 저장소 메인 페이지다. README에서 프로젝트의 전체적인 구조와 빠른 시작 가이드를 확인할 수 있다.

![OpenClaw GitHub 저장소 — README와 프로젝트 구조를 한눈에 볼 수 있다](../../../assets/blog/github-readme.png)
...
OpenClaw의 가장 큰 매력 중 하나는 <strong>채널의 다양성</strong>이다.
...
별도의 앱을 설치할 필요 없이, <strong>이미 쓰고 있는 메신저에서 바로</strong> AI 비서를 사용할 수 있다. 텔레그램에서 메시지 보내듯이 AI에게 말 걸면 끝이다.
...
OpenClaw의 심장은 <strong>Gateway</strong>다. 단일 제어 평면(Control Plane)으로 동작하며, `ws://127.0.0.1:18789`를 통해 모든 채널과 도구를 하나로 연결한다.
...
하나의 모델에 종속되지 않고, 용도에 따라 <strong>자유롭게 모델을 교체</strong>할 수 있다. 크론 작업에는 저렴한 모델을, 중요한 분석에는 고성능 모델을 배정하는 식의 <strong>모델 라우팅</strong>도 가능하다.

### 강력한 도구(Tools) 세트

OpenClaw에 내장된 도구들은 단순 플러그인이 아니라, <strong>에이전트가 실제로 세상과 상호작용하는 수단</strong>이다.
...
<strong>AgentSkills 포맷</strong>과 호환되는 스킬 시스템을 갖추고 있어, <strong>ClawHub 마켓플레이스</strong>에서 다른 사용자가 만든 스킬을 설치하거나 자신의 스킬을 공유할 수 있다.

![ClawHub — OpenClaw 스킬 마켓플레이스. Trello, Slack, Calendar 등 다양한 스킬이 제공된다](../../../assets/blog/clawhub-main.png)

스킬은 세 곳에서 로드된다 (우선순위 순서):
...
iOS, Android, macOS 디바이스를 <strong>노드</strong>로 연결하면 AI가 물리적 세계와 상호작용할 수 있다:
...
노드는 Gateway WebSocket에 연결되며, <strong>페어링 승인</strong>을 거쳐야 활성화된다. 내 폰이 AI의 눈과 귀가 되는 셈이죠!

### 멀티 에이전트 시스템

OpenClaw은 하나의 Gateway에서 <strong>여러 에이전트를 동시에 운영</strong>할 수 있다.
...
키보드 없이 <strong>음성으로 AI와 대화</strong>할 수 있다. macOS 앱의 Wake word 기능으로 호출하고, Talk Mode로 자연스러운 대화를 이어갈 수 있다.
...
- <strong>프라이버시를 중요하게 생각하는 분</strong> — 모든 데이터가 내 컴퓨터에만 남다
- <strong>자동화를 좋아하는 개발자</strong> — 크론, 웹훅, 스킬, MCP로 무한 확장
...
반대로, "그냥 ChatGPT 웹에서 가끔 질문하는 정도"로 만족하는 사람이라면 굳이 OpenClaw까지 갈 필요는 없을 수 있다. OpenClaw은 <strong>"AI를 내 삶에 깊이 통합하고 싶은 사람"</strong>을 위한 도구다.

---

## 프로젝트 생태계

OpenClaw은 단독 프로젝트가 아니라, 여러 컴포넌트로 이루어진 <strong>생태계</strong>다:
...
이번 글에서는 OpenClaw이 무엇이고, 왜 특별한지 알아봤다.

<strong>[2편: 튜토리얼편](/ko/blog/ko/openclaw-installation-tutorial/)</strong>에서는 실제로 OpenClaw을 <strong>설치하고 설정하는 과정</strong>을 단계별로 함께 해볼 예정이다.
...
*이 글이 도움이 되셨다면 공유 부탁드려요! 질문은 [Discord 커뮤니티](https://discord.gg/clawd)에서 환영한다.*
