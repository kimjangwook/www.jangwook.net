...
**두 번째 문제가 더 심각하다. MCP 설정 파일이다.** Claude Code, Cursor, Windsurf 같은 도구들이 MCP 서버에 연결할 때 사용하는 설정 파일에는 데이터베이스 접속 정보, API 키, OAuth 토큰이 평문으로 들어간다. GitGuardian에 따르면 공개 리포지토리의 MCP 설정 파일에서만 **24,008개**의 고유 시크릿이 발견됐다. [MCP 생태계가 60일 만에 30개 CVE가 보고된 공격 표면임을](/ko/blog/ko/mcp-security-crisis-30-cves-enterprise-hardening) 감안하면 놀랍지 않다.
...
개인적으로 이 기능이 기대되는 이유는, AI 에이전트가 만드는 코드의 시크릿 유출을 **에이전트 워크플로우 안에서** 잡아준다는 점이다. 사람이 리뷰하기 전에 시스템이 먼저 잡아주니까, [에이전트가 커밋한 코드를 내가 리뷰하는](/ko/blog/ko/cursor-agent-trace-ai-code-attribution) 현실적인 워크플로우에 잘 맞는다.
...
2,900만 개라는 숫자는 무서운데, 사실 대부분의 시크릿 유출은 복잡한 공격이 아니라 "깜빡하고 커밋한" 수준의 실수에서 발생한다. AI 코딩 에이전트가 이 실수의 빈도를 높이고 있다는 게 이번 리포트의 핵심이다. [AI 코딩 도구의 프롬프트 인젝션 취약점](/ko/blog/ko/roguepilot-copilot-prompt-injection-security)이 크리덴셜 탈취로 이어지는 경로도 같은 맥락이다.
