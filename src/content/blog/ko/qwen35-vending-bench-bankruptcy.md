...
Alibaba가 개발한 대규모 언어 모델 <strong>Qwen 3.5 Plus</strong>는 MMLU, HumanEval, MATH 등 표준 벤치마크에서 항상 최상위권 성적을 기록하고 있습니다. 하지만 Andon Labs가 개발한 비표준 벤치마크 <strong>Vending-Bench 2</strong>에서는 놀랍게도 <strong>「파산」</strong>이라는 결과를 받았습니다. 이 사실은 Reddit r/LocalLLaMA에서 595 이상의 Upvote를 기록하며 AI 평가 방식에 대한 논의를 불러일으켰습니다.

## Vending-Bench 2란?

Vending-Bench 2는 Andon Labs가 개발한 <strong>자판기 경영 시뮬레이션 벤치마크</strong>입니다. AI 모델에게 가상의 자판기 비즈니스를 약 365일간 운영하게 하여 <strong>재무 관리, 의사결정, 전략 수립</strong> 능력을 종합적으로 측정합니다.

기존 벤치마크와 달리 다음과 같은 실전적 능력을 측정합니다:

- <strong>장기적 전략 사고</strong>: 1년간의 연속적인 비즈니스 판단
- <strong>재무 리스크 관리</strong>: 수지 균형 유지와 성장의 양립
- <strong>적응력</strong>: 변화하는 시뮬레이션 환경에 대한 대응
- <strong>실용적 추론 능력</strong>: 단순 지식이 아닌 지식의 응용력
...
표준 벤치마크에서 상위권에 있는 Qwen 3.5 Plus가 <strong>전체 모델 중 최하위</strong>로 잔고 제로라는 충격적인 결과입니다.
...
표준 벤치마크는 <strong>정적인 지식이나 단발성 태스크</strong> 측정에 뛰어납니다. 하지만 다음과 같은 능력은 측정할 수 없습니다:
...
AI 모델 개발에서 표준 벤치마크 점수 향상은 중요한 개발 지표입니다. 이는 <strong>「벤치마크 해킹」</strong>이라는 현상을 초래합니다:
...
Reddit r/LocalLLaMA에서의 논의에서는 다음과 같은 의견이 있었습니다:

- <strong>「활성 파라미터 수 ≠ 지능」</strong>: 모델 크기만으로 능력이 결정되지 않음
- <strong>아키텍처의 중요성</strong>: MoE(Mixture of Experts) 라우팅 효율이 결과를 좌우
- <strong>학습 데이터의 질</strong>: 양뿐만 아니라 데이터의 질과 다양성이 중요

GLM-5가 $8,000 이상의 수익으로 1위를 차지한 점도 주목됩니다. 표준 벤치마크에서 Qwen 3.5보다 하위에 있는 모델이 실전 태스크에서는 압도적으로 우수할 수 있음을 보여줍니다.
...
이번 결과는 <strong>단일 벤치마크 결과만으로 모델의 우열을 판단해서는 안 된다</strong>는 것을 명확히 보여줍니다:
...
Qwen 3.5 Plus의 Vending-Bench 2 파산은 <strong>벤치마크 편중 AI 평가가 얼마나 위험한지</strong>를 상징적으로 보여주는 사건입니다. 표준 벤치마크에서 최상위 점수를 받는 모델이 실전 시나리오에서는 최하위가 될 수 있다는 사실은, AI 모델 선택 시 <strong>「숫자 뒤의 실력」</strong>을 파악해야 할 필요성을 일깨워줍니다.

AI의 진정한 능력을 측정하려면 표준화된 테스트뿐 아니라 <strong>현실 세계의 복잡성을 반영한 다양한 벤치마크</strong>가 필수적입니다.
