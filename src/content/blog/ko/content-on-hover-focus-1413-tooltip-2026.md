...
V4는 같은 8px을 마진이 아니라 패딩으로 만들었다. 팝업 상자 자체는 트리거에 붙어 있고, 상자 안쪽 위에 18px의 투명한 패딩을 두어 내용물만 아래로 밀어냈다. 스크립트가 잰 간격은 0px이다. 눈에는 떨어져 보이지만 포인터에게는 붙어 있다. 여기에 `:has()`로 팝업 자신에 대한 호버까지 열림 조건에 넣으면 CSS만으로 Hoverable이 성립한다.
