...
또 멀티 에이전트 시나리오는 아직 제한적이다. 복잡한 오케스트레이션이 필요하다면 LangGraph 위에서 Pydantic AI를 구조화 출력 레이어로만 쓰는 조합이 더 현실적이다. 이 조합에 대해서는 LangGraph vs CrewAI vs Dapr 비교 가이드에서 상위 레이어를 먼저 정리했으니 참고하면 좋다.
...
구조화 출력의 기본기가 궁금하다면 DeNA 사내 LLM 스터디 시리즈가 좋은 출발점이다. 이 라이브러리들이 해결하는 문제의 배경을 이해하는 데 도움이 된다.
...
LLM API 가격 비교도 참고하면 좋다. 어떤 라이브러리를 쓰든 모델 선택에 따라 비용이 크게 달라지는데, 특히 Instructor의 재시도 비용이나 Smolagents의 코드 생성 루프 비용을 사전에 추정할 때 도움이 된다.
