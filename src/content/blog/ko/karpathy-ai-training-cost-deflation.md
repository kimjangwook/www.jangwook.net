...
Andrej Karpathy가 자신의 <strong>nanochat</strong> 프로젝트를 통해 놀라운 사실을 공개했다. 2019년 OpenAI가 GPT-2(1.5B 파라미터)를 학습시키는 데 약 <strong>$43,000</strong>이 들었지만, 2026년 현재 같은 성능을 달성하는 데 불과 <strong>$73</strong>이면 충분하다는 것이다. 이는 <strong>약 600배의 비용 절감</strong>이며, 연간 약 40%씩 비용이 하락하는 디플레이션 추세를 보여준다.

이 글에서는 Karpathy의 분석을 바탕으로 AI 학습 비용 하락의 구조적 요인과 업계에 미치는 영향을 살펴본다.
...
Karpathy는 비용 하락이 단일 요인이 아닌 <strong>4가지 축의 동시 개선</strong>에 의한 것이라고 분석한다.
...
소프트웨어 스택의 개선이 동일한 하드웨어에서도 극적인 성능 향상을 가져온다.
...
옵티마이저와 아키텍처의 혁신이 학습 효율을 근본적으로 개선한다.
...
고품질 데이터 큐레이션과 효율적인 데이터 로딩이 학습 효율을 높인다.
...
Karpathy는 효과가 <strong>없었던</strong> 기법들도 투명하게 공개하여 커뮤니티에 귀중한 인사이트를 제공한다.
...
연 40%의 비용 하락은 AI 학습의 <strong>민주화</strong>를 가속한다. 과거에는 대형 테크 기업만 가능했던 규모의 학습이 이제 스타트업이나 개인 연구자도 접근 가능해지고 있다.

### 경쟁 축의 전환

비용이 더 이상 차별화 요소가 되지 않으면서, 경쟁의 축이 전환된다:
...
연 40% 하락은 무어의 법칙(약 2년에 2배, 연 ~29% 하락)보다 <strong>빠른 속도</strong>이다. 이는 하드웨어뿐 아니라 소프트웨어·알고리즘·데이터의 동시 개선이 만들어내는 복합 효과다.

## 결론

Karpathy의 nanochat 프로젝트는 단순한 벤치마크 기록 갱신을 넘어, AI 학습 비용의 <strong>구조적 디플레이션</strong>을 실증적으로 보여주는 사례다. 하드웨어, 소프트웨어, 알고리즘, 데이터 — 이 4가지 축의 동시 개선이 연 40%라는 놀라운 하락률을 만들어내고 있으며, 이 추세는 AI 업계의 경쟁 구도를 근본적으로 변화시키고 있다.

중요한 것은 Karpathy 자신이 "이것은 과소추정이며, 추가 개선이 충분히 가능하다"고 언급한 점이다. 디플레이션은 아직 끝나지 않았다.
