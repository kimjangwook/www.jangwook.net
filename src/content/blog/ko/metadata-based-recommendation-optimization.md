...
description: >-
  블로그 콘텐츠 추천 시스템에서 78,000 토큰을 완전히 제로화하고 실행 시간을 99% 단축한 메타데이터 기반 최적화 실전 사례를
  소개합니다. post-metadata.json 캐싱 전략과 해시 비교 기반 증분 처리로 추천 비용을 획기적으로 절감한 전 과정을 설명합니다.
pubDate: '2025-10-18'
...
이 글은 <strong>토큰 사용량을 63% 줄이는 것을 목표로 시작했지만, 결국 100% 제거</strong>하고, <strong>실행 시간을 99% 단축</strong>하며, <strong>비용을 완전히 제로화</strong>한 최적화 여정의 기록이다.
...
포스트를 분석하는 것과 추천을 생성하는 것을 분리하면 어떨까? 각 포스트의 핵심 정보를 사전에 추출해두고, 추천 생성 시에는 이 메타데이터만 사용하는 것이다.
...
전체 콘텐츠를 분석하여 구조화된 메타데이터를 생성하는 전문 에이전트를 만들었다:
...
좋은 투자였다. 하지만 여기서 멈추지 않았다.

## 3단계: 두 번째 최적화 - 한국어만 분석

### 돌파구: 다국어 포스트의 비밀

블로그는 한국어(ko), 영어(en), 일본어(ja) 3개 언어를 지원한다. 각 포스트는 3개 언어로 존재하죠:
...
여기서 깨달음: <strong>내용은 동일하다. 언어만 다를 뿐이죠!</strong>

메타데이터 생성 시 39개 파일(13개 포스트 × 3개 언어)을 모두 분석할 필요가 없다. <strong>한국어 버전 13개만 분석하면 된다.</strong>
...
이제 초기 투자 비용도 67% 줄였다. 하지만 아직도 LLM 호출이 필요했다.

## 4단계: 세 번째 최적화 - 완전한 알고리즘화

### 근본적인 질문: "LLM이 정말 필요한가?"

여기서 멈춰서 생각했다. 메타데이터 기반 추천 생성은 다음 작업이다:
...
<strong>이 모든 것이 결정론적 계산이다.</strong> LLM이 필요하지 않다!
...
오히려 알고리즘이 더 <strong>결정론적</strong>(항상 동일한 결과)이고 <strong>투명</strong>(가중치 조정 가능)한다!
...
알고리즘의 품질은 메타데이터 품질에 의존한다. 우리는 수동으로 메타데이터를 생성했다:
...
LLM은 강력하지만 만능이 아니다. 적절한 문제에 적절한 도구를 사용한다:
...
"미리 계산하고, 빠르게 조회"는 시스템 설계의 황금률이다:
...
이 프로젝트는 <strong>측정</strong>에서 시작해서 <strong>측정</strong>으로 끝났다:
...
예상을 초과하는 성과였다. 핵심은 <strong>적절한 기술을 적절한 문제에 적용</strong>하는 것이었다.

LLM은 놀랍다. 하지만 모든 문제가 LLM을 필요로 하지는 않다. 때로는 오래된, 입증된 알고리즘이 더 나은 해답이다.
