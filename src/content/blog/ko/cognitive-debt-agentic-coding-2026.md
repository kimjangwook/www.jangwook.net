...
Anthropic 리포트도 이 위험을 인식한다. 개발자들이 AI에게 <strong>검증 가능하거나 저위험 작업</strong>은 위임하지만, 개념적으로 복잡하거나 설계 의존적인 작업은 직접 하거나 AI와 협력한다고 밝혔다. 즉, 위임의 경계를 의식적으로 관리하는 개발자는 인지 부채를 통제하고 있다. [Claude Code 에이전틱 워크플로우 5가지 패턴](/ko/blog/ko/claude-code-agentic-workflow-patterns-5-types)은 이 위임 경계를 실전에서 어떻게 설정하는지 보여준다. 문제는 그렇지 않은 팀이다.

## Engineering Manager가 지금 해야 할 5가지

<strong>1. "AI가 만들어서" 면죄부 금지 규칙 도입</strong>
코드 리뷰에서 "이 부분은 AI가 생성했습니다"는 설명으로 불충분하다. <strong>왜 이 구조인지, 어떤 트레이드오프가 있는지</strong> 설명할 수 있어야 머지 가능하다. [Claude Code 모범 사례](/ko/blog/ko/claude-code-best-practices)에서 AI 생성 코드를 팀이 함께 검증하는 프로세스를 확인할 수 있다.
