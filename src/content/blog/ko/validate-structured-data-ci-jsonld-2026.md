...
<strong>값이 아니라 DOM을 가리키는 필드는 이 검사로 안 잡힌다.</strong> `speakable`의 `cssSelector`처럼 페이지의 어느 부분을 가리키는 값은 문자열로서는 언제나 유효하다. 선택자가 아무것도 못 잡거나 열세 곳을 잡아도 스키마 검사는 통과시킨다. 이 종류의 부패를 실제로 재본 기록은 음성으로 읽을 문단을 13개나 지목하고 있었다에 있다.
