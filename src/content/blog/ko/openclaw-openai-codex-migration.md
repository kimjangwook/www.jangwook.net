...
![OpenClaw Codex 전환 체크리스트 - backup, OAuth, model switch, verification](../../../assets/blog/openclaw-openai-codex-migration-checklist.jpg)
## 발단: Claude와 Gemini의 ToS 변경

2026년 초, Anthropic(Claude)과 Google(Gemini)이 이용약관을 업데이트했다. 문제의 핵심은 <strong>구독 플랜(Pro/Max 등)의 OAuth 인증을 서드파티 도구에서 쓸 수 있는가</strong>였다.

그동안 많은 OpenClaw 사용자가 Claude Pro나 Gemini Advanced 구독을 OAuth로 연결해 에이전트를 운용해 왔다. ToS 변경 이후 "이대로 써도 괜찮은 건가?"라는 불안이 퍼졌고, 커뮤니티에서 대안 찾기가 시작되었다.

결론부터 말하면, <strong>OpenAI의 ChatGPT Plus/Pro 구독에 포함된 Codex OAuth는 서드파티 연동을 명시적으로 허용</strong>한다. 이 글에서는 실제 전환 절차와 주의할 점을 정리한다.

## 전환 전에 확인할 것

바로 옮기기 전에 세 가지를 점검한다.

- <strong>프롬프트 호환성</strong>: Claude와 GPT-5.x-Codex는 같은 프롬프트에 다르게 반응한다. `SOUL.md`, `AGENTS.md`를 며칠간 손봐야 안정된다
- <strong>모델별 강점 차이</strong>: Codex는 코드와 도구 호출(Tool Use)에 강한다. 반면 에세이나 SNS 포스트처럼 자연스러운 문장이 필요한 작업에서는 Claude 쪽 평가가 높다
- <strong>API 키 방식이라는 선택지</strong>: Claude를 계속 쓰고 싶다면 구독 OAuth 대신 API 키 방식(`console.anthropic.com`)을 쓸 수 있다. 단, Opus 에이전트 루프를 돌릴 경우 월 $100 이상 나오는 사례가 많다. [AI 에이전트 비용 vs 인건비의 현실](/ko/blog/ko/ai-agent-cost-reality)에서 실제 운영 비용을 비교 분석한 자료를 참고할 수 있다

## 전환 절차 (4단계)

커뮤니티에서 검증된 순서다. 소요 시간은 약 15분이다.

### 1단계: 백업

가장 먼저 현재 설정을 통째로 백업한다.
...
채널 설정, 메모리, 크론잡, 인증 정보가 모두 이 안에 들어간다.
...
위저드가 시작되면 몇 가지 질문이 나온다. 아래 표대로 선택하면 된다.
...
브라우저가 열리면 ChatGPT 계정으로 로그인하고 권한을 승인한다.

<strong> 주의</strong>: 승인이 끝나면 <strong>터미널의 위저드를 바로 닫는다.</strong> 이미 기존 설정이 있는 상태에서 위저드를 끝까지 돌리면 설정이 덮어씌워질 수 있다.
...
`primary`에 `openai-codex/gpt-5.3-codex`가 표시되면 성공이다.

## Anthropic 인증 완전 제거 (선택)

기존 인증을 깔끔하게 정리하고 싶다면 아래 명령을 실행한다.
...
전환 후 가장 흔한 실수가 여기서 발생한다. <strong>`openclaw models set`은 글로벌 기본값만 변경한다.</strong> 에이전트에 모델이 개별 지정되어 있으면 그 에이전트는 이전 모델을 그대로 쓴다.

`openclaw.json`의 `agents.list`를 확인해 본다.
...
전체를 일괄 전환하려면 에이전트별 `model` 블록을 삭제하는 편이 간단하다.

## 레이어 전략 — 커뮤니티 추천 아키텍처

하나의 프로바이더에 올인하는 대신, 용도별로 모델을 나누는 <strong>레이어 전략</strong>이 커뮤니티에서 자리 잡고 있다.
...
이 구조에서는 프로바이더가 정책을 바꾸더라도 <strong>설정 파일 한 줄만 수정</strong>하면 된다. [LangGraph·CrewAI·Dapr 프레임워크 비교](/ko/blog/ko/ai-agent-framework-comparison-2026-langgraph-crewai-dapr-production)에서 멀티 에이전트 아키텍처 설계에 대한 더 넓은 관점을 확인할 수 있다.

## 전환 후 체감

실제로 전환한 사용자들의 공통된 의견이다.

- <strong>프롬프트 튜닝은 필수</strong>: `SOUL.md`, `AGENTS.md`를 며칠 동안 조정해야 이전과 같은 수준으로 안정된다
- <strong>코드·도구 호출은 확실히 강한다</strong>: 코드 생성과 Tool Use에서 체감할 수 있는 차이가 있다
- <strong>자연스러운 글쓰기는 아쉽다</strong>: 에세이나 SNS 포스트에서는 Claude가 더 자연스럽다는 평가가 많다
- <strong>안정성은 높다</strong>: OAuth 인증이 안정적이고, 토큰 소진 걱정 없이 에이전트를 돌릴 수 있다

## Claude를 유지하고 싶다면

API 키 방식은 ToS와 무관하게 사용할 수 있다.

```bash
openclaw onboard --auth-choice anthropic
```

`console.anthropic.com`에서 API 키를 발급받고, 토큰당 과금으로 운용한다. Opus 에이전트 루프의 경우 월 비용이 $100을 넘기 쉬우므로, 비용 모니터링이 중요하다.
