...
오늘 아침 `curl`로 `https://jangwook.net/llms.txt`를 쳤다. HTTP/2 404.

같은 시각 `robots.txt`는 달랐다. 저장소 `public/robots.txt`는 45줄, 1,101바이트. 라이브는 106줄, 2,937바이트.

GEO 체크리스트 맨 위에 `llms.txt`를 올려두고 티켓을 넣으려던 사람은, 이 두 숫자만 봐도 할 일이 바뀐다. Google Search Central이 2026년 5월 15일에 올린 [생성형 AI 기능 최적화 가이드](https://developers.google.com/search/docs/fundamentals/ai-optimization-guide)는 7월 10일에 한 번 더 고쳤다. 그 문서는 파일을 더 만들라고 하지 않는다. Search Console 속성에 스위치가 생겼다고 한다.

![공식 GEO는 빼는 목록과 스위치 하나다](../../../assets/blog/official-geo-subtraction-gsc-control-2026/hero.png)

## 저장소 45줄, 서버는 106줄

크롤러가 읽는 바이트를 받기로 했다. 2026년 8월 14일, 공개 URL만 `curl`로 집었다.
...
`LLMs.txt`와 `www` 호스트도 404였다. Google Search가 무시한다고 적어 둔 파일을 이 사이트는 깔아 두지 않은 상태다.

robots.txt는 저장소 밖에서 길어져 있었다. git 쪽은 학습 봇(`GPTBot`, `ClaudeBot`, `CCBot`, `Google-Extended`)을 막고, 검색 봇과 `*`에는 교차 언어 URL만 가린다. 라이브 응답은 그 앞에 CDN이 붙인 접두가 있다. `User-agent: *`에 `Content-Signal: search=yes,ai-train=no,use=reference`가 있고, 학습·확장 봇 `Disallow: /`가 한 번 더 있다. 저장소 본문은 그 뒤에 이어진다.
...
홈 JSON-LD는 `Organization`·`Person`·`WebSite`, 글은 `BlogPosting`·`WebPage`·`BreadcrumbList`였다. 오늘 문서 기준으로 그 마크업은 생성형 검색 입장권이 아니다. 리치 결과와 엔티티 정리 쪽에 남는다. [FAQPage 리치 결과가 끝난 뒤에도 Q&A 마크업을 남긴 이유](/ko/blog/ko/faqpage-deprecation-ai-citation-2026)와 같은 선이다.
...
시중 체크리스트와 어긋나는 행은 네 줄이다.

| 시중에 도는 항목 | 공식 입장 | 코드에서 할 일 |
| --- | --- | --- |
| `llms.txt`·전용 AI 파일 | Google Search는 쓰지 않는다. 만들어도 노출·순위에 득도 해도 없다 | Google Search용으로 만들지 않는다. 다른 시스템이 읽으면 그 목적만 남긴다 |
...
다른 서비스가 읽으면 둬도 된다. Google Search 할 일 목록에서는 뺀다. [robots.txt와 llms.txt로 크롤러를 나누는 글](/ko/blog/ko/ai-crawler-control-robots-txt-llms-txt-2026)에서 학습용과 검색용을 이미 갈라 두었다면, 오늘은 "Google이 이 파일을 읽는다"는 착각만 지우면 된다.
...
검색 나머지에 대한 순위 신호가 아니다. 학습 제한도 이 스위치 일이 아니다. 학습을 줄이려면 [Google-Extended](https://developers.google.com/search/docs/crawling-indexing/google-common-crawlers#google-extended), 검색 전체에서 빼려면 `noindex`. 반영은 제어가 살아난 뒤 대체로 1〜2일. 캐시 때문에 더 걸릴 수 있다.
...
이 글을 쓰면서 Search Console에는 로그인하지 않았다. 내 속성에 그 메뉴가 있는지는 여기서 단정하지 않는다. 문서상 기본값이 포함이라는 점, 상속이 적혀 있다는 점만 단정한다.
...
라이브 robots.txt diff가 안 맞거나, 부모 속성에 스위치가 있는 자리조차 안 보이면 그 페이지만 들고 와도 된다. 나는 공식 문서와 배포된 바이트가 어긋난 지점을 맞추는 일을 한다.
---
