...
팀장으로서 1년에 두 번, 전 사원 앞에서 반기별 성과 보고를 해야 한다. 릴리스 내역, 연구개발 성과, 로드맵 대비 실제 달성도, 팀 커뮤니케이션 현황까지 모든 것을 종합하여 PPT로 정리하고 발표해야 한다.

<strong>기존 프로세스는 총 60시간 이상이 소요</strong>됐다.
- 릴리스 분석: 4-5시간
- R&D 자료화: 20-30시간
- 로드맵 리뷰: 4-5시간
- 회의록 분석: 8-10시간
...
하지만 AI와 자동화 도구를 도입한 결과, <strong>이 시간을 약 10시간으로 단축</strong>할 수 있었다. 어떻게 이런 혁신적인 변화를 만들어냈는지 실전 프로세스를 공유한다.

## 얻을 수 있는 효과

AI 자동화 시스템 도입을 통해 다음과 같은 효과를 얻을 수 있었다.

### 1. 시간 단축: 60시간 → 10시간
단순히 작업 속도만 빨라진 것이 아니다. 반복적이고 지루한 데이터 수집 작업을 AI가 대신하면서, 핵심 메시지 구성과 스토리텔링에 집중할 수 있게 됐다.

### 2. 데이터 누락 방지
사람이 수작업으로 정리하다 보면 일부 릴리스나 회의 내용을 빠뜨리기 쉽다. AI 에이전트는 정해진 기간의 모든 데이터를 빠짐없이 수집하여 체계적으로 정리한다.

### 3. 불필요한 내용 제거
과거에는 "혹시 필요할까봐" 모든 내용을 담다가 PPT가 50-60장까지 늘어나곤 했다. AI가 중요도를 판단하고 핵심만 추려주면서, 임원진에게 전달해야 할 메시지에만 집중할 수 있게 됐다.

### 4. 효과적인 스토리텔링
AI는 수집한 데이터를 "Overview → 성과 중심 Review → 과제 도출"이라는 논리적 구조로 자동 정리한다. 이를 기반으로 설득력 있는 스토리를 만들 수 있다.

## 기존 프로세스 분석

먼저 기존의 비효율적인 프로세스를 자세히 살펴본다.

### 1. 릴리스 노트 및 코드 분석 (4-5시간)
반기 동안 배포한 기능들을 Git 히스토리와 릴리스 노트를 뒤져가며 수작업으로 추출한다. 커밋 메시지가 불명확하거나 릴리스 노트가 부실한 경우, 코드를 직접 열어서 확인해야 했다.

### 2. R&D 자료화 (20-30시간)
팀 내에서 진행한 연구개발 활동(새로운 기술 검토, POC, 성능 최적화 등)을 정리하는 작업이다. 각 엔지니어가 작성한 Notion 문서, 슬랙 대화, 코드 주석 등을 모두 찾아서 통합해야 했다. <strong>가장 많은 시간이 소요되는 단계</strong>다.

### 3. 개발 로드맵 리뷰 (4-5시간)
연초에 세운 개발 계획과 실제 달성 내역을 비교한다. "왜 A 기능은 완료했는데 B 기능은 지연되었는가?" 같은 갭 분석이 핵심이다.

### 4. 회의록 분석 (8-10시간)
매주 진행되는 팀 미팅, 격주 비즈니스 사이드 미팅 등의 회의록을 읽으며 주요 이슈와 의사결정 사항을 추출한다. 반기 동안 최소 20-30건의 회의록을 검토해야 한다.

### 5. 전사/부서 목표 파악 (2-3시간)
우리 팀의 성과가 전사 목표와 어떻게 연결되는지 정리한다. CEO 발표 자료나 부서 OKR 문서를 참조한다.

### 6. PPT 자료 생성 (20-30시간)
수집한 모든 자료를 바탕으로 스토리를 구성하고, 슬라이드를 디자인하고, 그래프와 표를 만든다. <strong>두 번째로 많은 시간이 소요되는 단계</strong>다.

### 7. 리뷰 및 개선 (6-11시간)
동료나 상사에게 리뷰를 받고, 피드백을 반영하여 수정한다. 보통 2-3번의 반복이 필요하다.

### 8. 발표 연습 (5-10시간)
발표 시간(보통 10-15분)에 맞춰 스크립트를 작성하고, 흐름을 점검하며, 실제로 발표 연습을 한다.

## AI 및 생산성 도구 도입

본격적인 자동화 전에, 일상 업무에 AI와 생산성 도구를 먼저 도입했다.

### 1. Notion AI로 회의록 자동 작성
[Notion AI의 회의 노트 기능](https://www.notion.so/help/guides/using-notions-meeting-notes-feature)을 활용하여, 회의 중 음성을 실시간으로 텍스트로 변환하고 요약한다.
...
Claude Code, ChatGPT, Gemini 등 다양한 AI 도구를 코드 리뷰와 인프라 문제 해결에 적극 활용한다.
...
<strong>구현 팁</strong>:
Claude Code의 MCP를 활용하여 Git API에 직접 접근하거나, GitHub Actions로 주기적으로 릴리스 데이터를 수집하여 Notion에 저장한다.
...
<strong>구현 팁</strong>:
Notion API를 통해 특정 기간의 페이지를 필터링하고, AI가 각 페이지의 핵심 내용을 요약하도록 한다. 태그나 속성으로 카테고리를 자동 분류한다.
...
<strong>구현 팁</strong>:
Product Manager Agent는 Release Manager와 R&D Agent의 결과물을 input으로 받아, 로드맵 Excel과 비교한다. Claude의 structured output을 활용하여 JSON 형태로 갭 분석 결과를 생성한다.
...
<strong>구현 팁</strong>:
Notion AI 회의록은 이미 구조화되어 있으므로, 특정 키워드(액션 아이템, 의사결정, 이슈 등)를 추출하는 간단한 스크립트를 작성한다. AI가 전사 목표 문서와 매칭하여 기여도를 자동 계산한다.

## PPT 자동 생성 프로세스

4개 에이전트의 작업이 완료되면, 모든 자료가 마크다운 파일로 저장된다. 이제 Product Manager Agent가 최종 PPT를 생성한다.

### Step 1: 구조화된 마크다운 생성

Product Manager Agent에게 다음과 같이 지시한다.
...
생성된 초안을 Claude, Gemini, ChatGPT에 각각 첨부하여 리뷰를 요청한다.
...
AI 피드백을 참고하여 틀린 부분이나 맥락이 잘못된 내용을 직접 수정한다. 특히 민감한 숫자나 비즈니스 데이터는 반드시 검증한다.

### Step 4: PowerPoint 생성

Claude Code의 "Create File" 기능을 활용하여 마크다운을 PowerPoint로 변환한다.
...
또는 Python 라이브러리(`python-pptx`)를 사용하여 프로그래밍 방식으로 생성할 수도 있다.
...
마지막으로 AI에게 10분 분량의 발표 스크립트를 작성하도록 지시한다.
...
반복적인 데이터 수집 작업에서 해방되어, 창의적인 스토리텔링에 집중할 수 있게 됐다.

<strong>2. 신뢰도 향상</strong>
AI가 체계적으로 정리한 데이터 덕분에 "이 수치가 맞나요?" 같은 질문을 받는 일이 줄었다.

<strong>3. 재사용 가능한 시스템 구축</strong>
한 번 구축한 에이전트 시스템은 다음 반기, 다음 해에도 계속 쓸 수 있다. 개선도 쉽다.

<strong>4. 팀 전체의 생산성 향상</strong>
Notion AI 회의록, 자동 문서화 등 구축한 인프라를 팀 전체가 활용하면서, 다른 업무의 효율도 함께 높아졌다.

## 구축 시 주의사항

### 1. 데이터 품질이 핵심
"Garbage in, garbage out." AI는 입력 데이터의 품질에 전적으로 의존한다. Notion 문서가 부실하거나, Git 커밋 메시지가 불명확하면 AI도 제대로 정리할 수 없다.

<strong>해결책</strong>: 평소에 문서화와 커밋 메시지 작성 규칙을 팀에 정착시킨다.

### 2. AI는 검증이 필요
AI가 생성한 내용은 반드시 사람이 검토해야 한다. 특히 숫자, 날짜, 인과관계는 틀릴 수 있다.

<strong>해결책</strong>: Step 3 (수동 검토)를 절대 생략하지 않는다.

### 3. 초기 구축 비용
에이전트 시스템을 처음 구축하는 데는 시행착오가 필요하다. 첫 번째 반기 보고서는 오히려 시간이 더 걸릴 수 있다.

<strong>해결책</strong>: 장기적 관점에서 투자한다. 두 번째부터는 정말 10시간이면 충분하다.

### 4. 보안 및 민감 정보
회사 내부 데이터를 AI에 입력할 때는 보안 정책을 준수해야 한다.

<strong>해결책</strong>: 온프레미스 LLM(예: Llama, GPT-4 Azure Enterprise)을 사용하거나, 민감한 정보는 마스킹 처리한다.

## 확장 가능성

이 시스템은 반기별 보고서 외에도 다양하게 활용할 수 있다.
...
AI 자동화의 핵심은 기술 자체가 아니라 <strong>프로세스 설계</strong>다.
- 어떤 데이터가 필요한가?
- 어떤 순서로 정리할 것인가?
- 최종 산출물의 형태는 무엇인가?

이런 질문에 먼저 답한 후, AI는 그 프로세스를 실행하는 "도구"로 활용해야 한다.

60시간을 10시간으로 줄인 것은 단순히 "빠른 도구"를 쓴 게 아니라, <strong>반복 작업을 시스템화</strong>했기 때문이다. AI는 그 시스템을 구현하는 최고의 수단일 뿐이다.

업무에 이런 반복적이고 시간이 많이 걸리는 작업이 있다면, 한 번 AI 자동화를 시도해보길 권한다. 첫 시도는 어렵지만, 일단 시스템이 구축되면 그 효과는 상상 이상이다.
