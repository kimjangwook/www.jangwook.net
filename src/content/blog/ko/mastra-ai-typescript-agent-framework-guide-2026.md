...
description: 'Mastra.ai TypeScript AI 에이전트 프레임워크를 직접 설치하고 Google Gemini와 연동해 날씨 에이전트를 만들어봤다. 설치부터 실제 툴 호출까지 실습 기록.'
pubDate: '2026-06-14'
...
relatedPosts:
  - slug: 'pydantic-ai-type-safe-agent-tutorial-2026'
    score: 0.87
    reason:
      ko: 'PydanticAI가 Python에서 타입 안전 에이전트를 구현하는 방식과 Mastra의 TypeScript 접근법을 비교하면 두 프레임워크의 설계 철학 차이가 명확해진다.'
  ...
  - slug: 'google-adk-vs-langgraph-agent-framework-comparison-2026'
    score: 0.82
    reason:
      ko: 'Google ADK와 LangGraph를 직접 비교했던 경험이 있다면, Mastra가 그 둘과 어떻게 다른 포지션을 취하는지 이해하는 데 도움이 된다.'
  ...
  - slug: 'python-ai-agent-library-comparison-2026'
    score: 0.79
    reason:
      ko: 'Python 에이전트 라이브러리를 비교한 관점에서 Mastra를 보면, TypeScript 진영에서 유사한 포지션을 점령하려는 시도가 얼마나 성숙했는지 가늠할 수 있다.'
  ...
  - slug: 'ai-agent-framework-comparison-2026-langgraph-crewai-dapr-production'
    score: 0.75
    reason:
      ko: 'LangGraph, CrewAI, Dapr를 프로덕션 관점에서 비교했다면, Mastra가 같은 기준으로 봤을 때 어느 단계에 있는지 판단하는 데 유용하다.'
  ...
  - slug: 'vercel-ai-sdk-claude-streaming-agent-2026'
    score: 0.72
    reason:
      ko: 'Vercel AI SDK는 Mastra의 기반 레이어 중 하나다. AI SDK를 직접 써본 경험이 있으면 Mastra가 그 위에 무엇을 더하는지 이해하기 쉽다.'
...
JavaScript 생태계에서 AI 에이전트를 만들 때 선택지가 없다는 말을 자주 들었다. "TypeScript 개발자면 결국 LangChain.js 아니면 Vercel AI SDK 정도지." 나도 그렇게 생각하고 있었는데, 올해 초 YC W25 배치를 통해 $13M 투자를 받고 v1.0을 출시한 Mastra.ai를 알게 됐다.

이름은 들었지만 직접 써본 적이 없었다. 오늘은 실제로 설치하고 Google Gemini와 연동해 툴 호출이 되는 에이전트를 돌려봤다. 5분 만에 실행된다는 홍보 문구가 실제인지 확인하는 게 목적이었다.
...
지원하는 LLM 제공자는 OpenAI, Anthropic, Google Gemini, Meta Llama 등 Vercel AI SDK가 지원하는 거의 모든 모델이다. 내부적으로 Vercel AI SDK를 기반으로 쓰기 때문에 [Vercel AI SDK로 Claude 스트리밍 에이전트를 만들어본 경험](/ko/blog/ko/vercel-ai-sdk-claude-streaming-agent-2026)이 있다면 기반 레이어는 이미 아는 셈이다.
...
## 결론: 이 시점에서 Mastra를 써볼 만한가

나는 Yes라고 본다. 단, 조건이 있다.

TypeScript로 새 에이전트 프로젝트를 시작한다면 Mastra는 확실히 살펴볼 가치가 있다. 설치에서 첫 에이전트 실행까지 5〜10분이면 충분하고, 프레임워크 구조도 명확하다.
