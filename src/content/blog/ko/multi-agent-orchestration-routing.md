---
title: 마루치에이전트 오케스트레이션 — 루팅 설계의 본질
description: 'Claude와 Codex 등 복수 AI 에이전트를 운용할 때, 태스크 루팅이 왜 가장 어렵고 EM의 권한 위임과 같은 구조인지 해부합니다.'
pubDate: '2026-02-09'
...
AI 에이전트를 한 대가 아니라 여러 대 동시에 운용하는 시대가 왔습니다. Claude는 판단과 문맥 이해에, Codex는 치밀한 코드 생성에 강점을 가집니다. 하지만 가장 어려운 문제는 "이 태스크를 어느 에이전트에게 보낼 것인가?"라는 **루팅(Routing)** 설계입니다.

이 글에서는 Engineering Manager(EM)의 시점에서, 멀티에이전트 루팅이 왜 **부하 관리에서의 권한 위임**과 동일한 구조인지 논의합니다.
...
3. **실패 시 재루팅**: 한 에이전트가 실패했을 때의 폴백 전략
4. **의존성 관리**: A의 출력이 B의 입력이 되는 파이프라인 설계

```mermaid
graph TD
    Task[새로운 태스크] --> Router{루팅 엔진}
...
NabbilKhan은 8개의 에이전트를 동시 운용하는 구성을 공개했습니다. 이때 부딪힌 가장 큰 문제가 바로 **루팅**이었습니다:
...
매니저가 위임 결과를 보고 다음 판단을 조정하듯, 에이전트 루팅도 결과를 기반으로 개선해야 합니다:
...
4. **피드백으로 지속 개선** — 루팅 결과를 추적하고 규칙을 세분화
