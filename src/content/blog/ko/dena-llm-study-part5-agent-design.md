...
relatedPosts:
  ...
  - slug: effiflow-automation-analysis-part1
    ...
    reason:
      ...
      zh: 在同一 LLM 脉络中可一并阅读。
faq:
  - question: "왜 2025년에는 완전 자율 에이전트보다 오케스트레이션이 권장되나요?"
    answer: "완전 자율 에이전트는 무제한 API 호출로 비용이 폭발하고, 행동을 통제하기 어려워 프로덕션에서 불안정하기 때문입니다. 명확한 워크플로우 안에서 자율성을 발휘하게 하면 비용을 예측할 수 있고 신뢰성과 디버깅이 쉬워집니다. n8n이나 LangGraph 같은 명시적 오케스트레이션 도구가 각광받는 이유입니다."
  - question: "멀티 에이전트 패턴은 어떤 기준으로 선택하나요?"
    answer: "명확한 단계가 있으면 Sequential 또는 Parallel을, 동적 작업 분배가 필요하면 Supervisor를, 창의적 협업이 필요하면 Network 패턴을 선택합니다. 다만 Network(예: AutoGen)는 대화 길이에 따라 비용이 예측 불가능하므로 반드시 비용 리밋을 설정해야 합니다."
  - question: "LLM의 함수 호출은 왜 검증 레이어와 Self-Healing이 필요한가요?"
...
시리즈의 마지막 편이다. 프롬프트 한 줄로 LLM을 부리던 단계는 지났다. 이번에는 스스로 판단하고 도구를 호출하는 에이전트를 어떻게 설계하는지, 그리고 여러 에이전트를 어떻게 묶어 굴리는지를 다룬다. 다만 자율성만 좇다 보면 비용이 터지고 동작을 통제할 수 없게 된다. 그래서 프로덕션에서 부딪히는 비용, 성능, 신뢰성 문제를 함께 짚는다.
...
정리의 뼈대는 DeNA 공식 스터디 자료다. 여기에 최신 연구와 실무 사례를 덧붙였다.

## 언제 이 내용이 유용한가, 언제 건너뛰어도 되나

에이전트 설계나 멀티 에이전트는 멋져 보이지만, 모든 문제에 맞는 도구는 아니다. 본문을 읽기 전에 지금 만들려는 게 정말 이 구조가 필요한지부터 따져보라.
...
- 단순 분류, 요약, 번역처럼 한 번의 호출로 끝나는 작업. 이때는 에이전트가 아니라 그냥 함수 하나면 충분하다.
- 입력과 출력이 고정된 결정론적 처리. 정규식이나 규칙 엔진이 더 싸고 빠르고 안정적이다.
...
[n8n](https://n8n.io/)은 <strong>노코드/로우코드 워크플로우 자동화 플랫폼</strong>이다. 2025년 기준으로 422개 이상의 통합을 지원하며, LLM 에이전트 구축에 특화된 기능을 제공한다.
...
n8n에서 ReAct (Reasoning and Acting) 패턴을 구현하는 예제다:
...
n8n은 로컬 LLM 실행 도구인 [Ollama](https://ollama.com/)와의 통합을 지원한다:
...
DeNA 스터디 자료와 최신 연구에 따르면, 2025년 에이전트 시스템의 핵심 트렌드는 <strong>"완전 자율"에서 "오케스트레이션"으로의 전환</strong>이다.
...
n8n 같은 워크플로우 도구가 각광받는 이유는 바로 <strong>명시적 오케스트레이션</strong>을 제공하기 때문이다.

## 2. 에이전트 설계 원칙

### 핵심 컴포넌트

LLM 에이전트는 4개의 핵심 컴포넌트로 구성된다:
...
DeNA 스터디 자료에서 지적한 중요한 문제다: <strong>LLM의 함수 호출은 100% 신뢰할 수 없다</strong>.
...
복잡한 작업을 여러 에이전트로 분산 처리하는 패턴이다.

#### 1. Sequential (순차)

한 에이전트의 출력이 다음 에이전트의 입력이 되는 선형 구조다.
...
여러 단계의 감독자-워커 관계가 트리 구조를 이루는 방식이다.
...
실제 프로젝트에서 패턴 선택이 비용에 미치는 영향을 분석한다.
...
[MemGPT](https://arxiv.org/abs/2310.08560)는 운영체제의 가상 메모리 개념을 LLM에 적용한 혁신적 접근이다.
...
[A-MEM](https://arxiv.org/abs/2502.12110)은 2025년 Rutgers 대학에서 제안한 혁신적 메모리 시스템이다. Zettelkasten(독일어로 "메모 상자") 방법론을 LLM 에이전트에 적용했다.
...
A-MEM의 핵심은 <strong>에이전트가 스스로 메모리를 조직</strong>한다는 점이다.
...
Context Window가 부족할 때 메모리 압축이 필요하다.
...
DeNA에서 실제 프로덕션에 적용한 <strong>NOC (Network Operations Center) Alert Agent</strong> 사례다.
...
DeNA 스터디 자료에서 강조하는 핵심 개념이다: <strong>"워크플로우를 자연어로 설명하면 에이전트가 이해하고 실행할 수 있다"</strong>.
...
LLM 에이전트 시스템의 최대 과제는 <strong>비용</strong>과 <strong>레이턴시</strong>다. 4가지 핵심 최적화 기법을 소개한다.
...
Part 5를 마치며 머릿속에 남은 것들을 적어둔다.

### 1. "완전 자율"보다 "오케스트레이션"

2025년 트렌드의 핵심이다. 에이전트가 모든 것을 스스로 결정하게 하는 것보다, <strong>명확한 워크플로우 안에서 자율성을 발휘하게 하는 것</strong>이 실무에서 더 효과적이다.
...
MemGPT, A-MEM 같은 고급 메모리 시스템은 에이전트를 단순한 "프롬프트 실행기"에서 <strong>"학습하고 진화하는 시스템"</strong>으로 변화시킨다.
...
6가지 패턴 중 "만능"은 없다. 각 패턴은 명확한 장단점이 있다.
...
실무에서 LLM 에이전트 시스템의 최대 장애물은 <strong>비용</strong>이다.
...
LLM은 확률적 시스템이다. 100% 정확한 응답은 불가능하다.
...
이렇게 다섯 편이 모두 끝났다. 여정 전체를 한 번 돌아볼 만하다.
...
<strong>DeNA LLM 스터디 시리즈를 완료했다!</strong> 이 시리즈가 LLM 에이전트 개발 여정에 도움이 되기를 바란다. 실무에서 적용하며 새로운 인사이트를 얻길 기대한다.
