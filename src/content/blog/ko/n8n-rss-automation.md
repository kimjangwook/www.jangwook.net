...
description: >-
  ...
  Gemini 2.5 Pro가 각 플랫폼에 맞게 최적화된 포스트를 자동 생성하는 완전 무인 콘텐츠 배포 파이프라인 구축 방법을 소개합니다.
pubDate: '2025-11-11'
...
블로그 포스트를 작성하고 나면 또 다른 작업이 기다린다. X(Twitter), LinkedIn 등 다양한 소셜 미디어에 맞춤형 콘텐츠를 작성하고 배포하는 것이다. 각 플랫폼마다 최적의 글자 수, 톤, 해시태그가 다르기 때문에 상당한 시간과 노력이 필요하다.

이 글에서는 <strong>GitHub Actions로 블로그를 자동 빌드하고, RSS 피드를 생성한 뒤, n8n으로 소셜 미디어 배포까지 완전 자동화하는 방법</strong>을 소개한다. 한 번 설정해두면 블로그 포스트만 작성하면 모든 것이 자동으로 처리된다.
...
Astro에서 RSS 피드를 생성하는 것은 매우 간단하다. `@astrojs/rss` 패키지를 사용하여 각 언어별로 RSS 피드를 생성할 수 있다.

### RSS 피드 생성 코드

`src/pages/rss-ko.xml.js` 파일을 생성한다:
...
빌드 후 다음 URL에서 RSS 피드를 확인할 수 있다:
...
n8n은 노코드/로우코드 워크플로우 자동화 도구다. 시각적인 인터페이스로 복잡한 자동화를 쉽게 구축할 수 있다.

### n8n 워크플로우 노드 구성

![n8n 워크플로우 다이어그램](../../../assets/blog/n8n-workflow-diagram.png)

전체 워크플로우는 6개의 노드로 구성된다:
...
이 노드가 전체 워크플로우의 핵심이다. Google Gemini 2.5 Pro 모델을 사용하여 블로그 콘텐츠를 분석하고 플랫폼별 맞춤형 소셜 미디어 포스트를 생성한다.
...
위에서 설명한 JSON 구성을 n8n에 임포트할 수 있다:
1. n8n 워크플로우 편집기 열기
...
블로그 발행부터 소셜 미디어 배포까지의 전체 프로세스를 자동화하면 콘텐츠 제작자는 정말 중요한 일에 집중할 수 있다. <strong>바로 좋은 콘텐츠를 만드는 것이다.</strong>
...
이제 여러분의 블로그도 자동화 시스템을 구축해보자. 시간은 절약하고, 품질은 높이고, 도달 범위는 넓힐 수 있다.

<strong>다음 글에서는</strong> n8n으로 댓글 모니터링과 AI 기반 커뮤니티 관리를 자동화하는 방법을 다룰 예정이다.
