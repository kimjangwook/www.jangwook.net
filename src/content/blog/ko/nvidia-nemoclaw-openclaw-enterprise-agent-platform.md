...
이게 나는 꽤 실용적이라고 본다. 에이전트한테 "이건 해도 되고 이건 안 돼"를 선언적으로 정의할 수 있다는 건, 보안팀과의 대화를 훨씬 구체적으로 만들어준다. "에이전트 도입하고 싶은데 괜찮을까요?"보다 "이 정책 파일 기준으로 에이전트가 동작합니다"가 훨씬 설득력 있다. 에이전트 행동 범위를 선언적으로 표준화하려는 시도는 NemoClaw만이 아닌데, [ADL(Agent Definition Language)](/ko/blog/ko/adl-agent-definition-language-governance)처럼 산업 전반에서 에이전트 거버넌스 표준화 움직임이 병행되고 있다는 점도 같이 주목할 만하다.
...
다만, 지금 당장 NemoClaw를 도입하겠다고 나서기보다는 **정책 파일 설계를 미리 시작하는 것**이 현실적이다. [AI 에이전트 도입의 실제 비용](/ko/blog/ko/ai-agent-cost-reality)을 따져보면, 거버넌스 부재로 인한 사고 하나가 몇 달치 구독료보다 비쌀 수 있다. 우리 팀의 에이전트가 어떤 도구에 접근해야 하고, 어떤 데이터는 건드리면 안 되는지를 정리해두면, NemoClaw든 다른 솔루션이든 도입 시점에 바로 적용할 수 있다. [Anthropic의 에이전트 스킬 구조화 접근법](/ko/blog/ko/anthropic-agent-skills-practical-guide)을 참고하면 도구 권한 설계에 실질적인 도움이 된다.
