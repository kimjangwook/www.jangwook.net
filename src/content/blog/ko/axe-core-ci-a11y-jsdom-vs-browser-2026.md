...
그 바닥이 실제로 어디쯤인지는 따로 재봤다. [W3C ACT 테스트 케이스에 axe-core를 전수로 돌려보니](/ko/blog/ko/act-rules-axe-coverage-wcag-sc-2026/) 36개 성공기준 중 22개는 검출 건수가 0이었다. 침묵의 일부는 규칙이 아예 꺼져 있어서 생긴 것이었다. CI에 붙이기 전에 `rules` 설정부터 확인할 이유가 여기 있다.
