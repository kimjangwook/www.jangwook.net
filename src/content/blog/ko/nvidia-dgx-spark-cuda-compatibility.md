...
description: >-
  ...
  분석합니다. Triton 미지원, FP4/FP6 양자화 불가 등 실제 호환성 실태와 소비자 구매 판단에 필요한 체크리스트를 정리합니다.
pubDate: '2026-02-15'
...
NVIDIA DGX Spark는 "$3,000 개인용 AI 슈퍼컴퓨터"라는 타이틀로 2025년 CES에서 발표되었다. 128GB 통합 메모리와 Blackwell 아키텍처를 탑재한 데스크톱급 AI 워크스테이션이라는 포지셔닝이었다. 하지만 실제 사용자들의 리포트는 마케팅과 상당히 다른 현실을 보여주고 있다.

Reddit의 r/LocalLLaMA 커뮤니티에서 한 사용자가 DGX Spark를 1주일간 사용한 후 반품을 결정했다는 글이 큰 반향을 일으켰다. 핵심 문제는 <strong>CUDA 소프트웨어 호환성</strong>이었다.

## sm121: 데이터센터도 아니고 게이밍도 아닌 특수 아키텍처

DGX Spark의 가장 큰 문제점은 GPU 아키텍처에 있다.
...
DGX Spark는 데이터센터용 Blackwell(sm_100)도, 게이밍용 Blackwell(sm_120)도 아닌 <strong>sm_121이라는 독자적인 아키텍처</strong>를 사용한다. 이로 인해 많은 CUDA 소프트웨어가 동작하지 않거나, sm_80(Ampere) 폴백 코드패스로 실행된다.

## 핸드헬드 게이밍 칩 유용 의혹

NVIDIA 공식 포럼에서 담당자가 남긴 답변이 의혹을 더욱 키웠다:
...
이 사실들은 DGX Spark의 GPU가 원래 <strong>게이밍 핸드헬드 디바이스용으로 설계된 칩</strong>을 AI 워크스테이션으로 전용했을 가능성을 시사한다. 통합 메모리를 지원하는 게이밍 GPU라는 점이 이 가설을 뒷받침한다.
...
- <strong>Blackwell 최적화 기능</strong>: [FP4/FP6 양자화](/ko/blog/ko/nvidia-llm-inference-cost-reduction/) 등 Blackwell 전용 기능 사용 불가

더욱 문제가 되는 것은 NVIDIA 포럼 담당자가 <strong>존재하지 않는 소프트웨어 버전과 릴리스</strong>를 언급하며 문제가 해결되었다고 답변한 점이다. 이는 LLM 할루시네이션으로 의심되며, NVIDIA의 고객 지원 품질에 대한 우려도 제기되고 있다.

## 하드웨어 품질 문제

소프트웨어 호환성 외에도 기본적인 하드웨어 문제가 보고되고 있다:
...
DGX Spark 사례에서 얻을 수 있는 교훈은 명확하다:
...
로컬 LLM 목적의 하드웨어 선택 시 [DDR5 vs GPU VRAM 비용 분석](/ko/blog/ko/ddr5-rdimm-vs-rtx3090-local-llm/)도 참고할 만하다.

## Apple Silicon과의 비교

같은 통합 메모리 아키텍처를 사용하는 Apple Silicon(M4 Ultra 등)과의 비교도 의미있다:
...
Apple Silicon은 CUDA를 지원하지 않지만, MLX 프레임워크의 에코시스템이 빠르게 성숙하고 있어 로컬 LLM 추론에서는 오히려 안정적인 경험을 제공한다. 소비자 GPU 활용이 목표라면 [8GB VRAM으로 80B 모델 실행하기](/ko/blog/ko/qwen3-coder-8gb-vram/)도 고려해볼 수 있다.

## 결론

DGX Spark는 "CUDA 에코시스템의 힘을 개인 데스크톱에서"라는 매력적인 약속을 했지만, 현실은 <strong>6년 전 Ampere 수준의 CUDA 호환성</strong>과 <strong>게이밍 칩 기반의 제한된 AI 성능</strong>이라는 gap이 존재한다.

소비자용 AI 하드웨어 시장은 아직 초기 단계다. NVIDIA가 sm_121에 대한 완전한 소프트웨어 지원을 제공하기 전까지, DGX Spark 구매는 신중하게 판단해야 한다. "AI 워크스테이션"이라는 라벨에 프리미엄을 지불하기 전에, 실제로 어떤 워크로드를 실행할 수 있는지 반드시 확인해야 한다.
