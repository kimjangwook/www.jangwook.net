...
The Hacker News가 3월 보도한 사례를 보면, 에이전트가 "가장 잘 작동하는" 방식을 찾다가 수개월 전 퇴직자의 고아 계정을 활용하는 패턴이 발견되었습니다. 이 계정 하나가 **여러 에이전트의 재사용 지름길**이 되면서, 한 번의 침해가 전체 에이전트 플릿에 영향을 줄 수 있는 구조가 만들어졌습니다. 외부 라이브러리를 통한 유사한 공급망 침해 사례는 [LiteLLM 공급망 공격 — AI 의존성 보안 리스크](/ko/blog/ko/litellm-supply-chain-attack-ai-dependency-security)에서 확인할 수 있습니다.
...
44%의 기업이 사용하는 정적 API 키는 가장 위험한 인증 방식입니다. 만료되지 않는 키는 침해되면 영구적 피해를 낳습니다. MCP 설정 파일에 시크릿이 확산되는 실제 패턴은 [AI 코딩 도구의 시크릿 스프롤: MCP 설정 보안 가이드](/ko/blog/ko/ai-coding-secrets-sprawl-mcp-config-security)에서 구체적으로 살펴볼 수 있습니다.
...
에이전트가 무엇을 했는지 추적할 수 없다면, 문제가 발생해도 원인을 파악할 수 없습니다. 프로덕션 환경에서 에이전트 행동을 체계적으로 관찰하는 방법은 [프로덕션 AI 에이전트 옵저버빌리티 가이드](/ko/blog/ko/ai-agent-observability-production-guide)에서 자세히 다루고 있습니다. 모든 에이전트 액션을 로깅하고, 그것을 인간 스폰서에게 연결하세요.
