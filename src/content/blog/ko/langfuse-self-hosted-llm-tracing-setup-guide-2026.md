...
이 글의 모든 설정은 Langfuse 공식 자료를 기준으로 검증했다. 직접 따라 해보고 싶다면 [Langfuse 공식 사이트](https://langfuse.com), [GitHub 저장소](https://github.com/langfuse/langfuse), 그리고 [Docker Compose 셀프호스팅 문서](https://langfuse.com/self-hosting/deployment/docker-compose)를 옆에 띄워두는 것을 권한다. 버전이 자주 바뀌므로 환경 변수 이름이나 포트는 항상 공식 문서를 1차 출처로 확인하는 게 안전하다.
...
앞에서 셀프호스팅과 Cloud를 짧게 비교했지만, 실제 결정은 한 가지 질문으로 좁혀진다. "트레이스 데이터가 우리 인프라 밖으로 나가도 괜찮은가, 그리고 그 인프라를 유지보수할 사람이 있는가." 이 두 축으로 판단하면 대부분의 경우 답이 분명해진다.

**셀프호스팅을 선택해야 하는 경우:**

- 트레이스에 의료 기록, 금융 거래, 개인 식별 정보가 섞여 들어가서 외부 SaaS로 보낼 수 없다
- 월 트레이스가 10만 건을 꾸준히 넘어 Cloud Pro 요금이 직접 운영 비용보다 비싸졌다
- 이미 Kubernetes나 Docker 기반 인프라를 운영 중이고, ClickHouse·Redis 같은 스테이트풀 서비스를 다뤄본 경험이 있다
- 사내 규정상 모든 관측 데이터를 자체 리전에 보관해야 한다

**Cloud를 그냥 쓰는 게 나은 경우:**

- 팀이 3명 이하이고 인프라 유지보수에 쓸 시간이 없다
- 트레이스가 월 5만 건 이하라 Cloud 무료 티어 안에서 끝난다
