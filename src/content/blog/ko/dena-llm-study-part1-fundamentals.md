...
- LLM을 막 쓰기 시작했고, "토큰이 뭔지", "Temperature를 왜 만지는지"부터 정리하고 싶은 사람
- 프롬프트를 감으로 쓰다가 정답률이 들쭉날쭉해서, Zero-shot/Few-shot/CoT의 차이를 한 번 정돈하고 싶은 사람
- 팀에 모델을 추천해야 하는데 GPT, Claude, Gemini, DeepSeek 중 무엇을 기준으로 골라야 할지 막막한 사람
- 사내 스터디나 신입 온보딩 자료로 "LLM 한 장 요약"이 필요한 사람
...
> <strong>다음 글 바로가기</strong>: Part 2: 구조화 출력과 멀티 LLM 파이프라인
