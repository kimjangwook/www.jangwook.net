...
2026년 2월 5일, Anthropic이 Claude Code의 새로운 실험적 기능인 <strong>Agent Teams</strong>를 발표했습니다. 기존의 서브에이전트(subagent)는 하나의 세션 안에서 결과만 돌려주는 단방향 구조였습니다. Agent Teams는 다릅니다. 완전히 독립된 여러 Claude Code 인스턴스가 <strong>서로 메시지를 주고받으며 협업</strong>합니다.
...
발표 당일, 이 기능을 OpenClaw 환경에서 곧바로 테스트했습니다. 이 글은 그 과정에서 겪은 삽질과 발견을 정리한 실전 가이드입니다.

## 사전 준비: OpenClaw dev 빌드

Agent Teams를 쓰려면 최신 Claude Code가 필요합니다. 당시 OpenClaw stable 채널에는 크론잡 버그가 있어서, 어차피 dev 채널로 전환해야 하는 상황이기도 했습니다. (관련 포스트)
...
- <strong>in-process</strong>: 모든 팀원이 메인 터미널 안에서 실행됩니다. `Shift+↑/↓`으로 팀원을 선택할 수 있습니다.
- <strong>tmux</strong>: 각 팀원이 tmux 분할 패인에서 실행됩니다. 모든 출력을 한눈에 확인할 수 있습니다.
- <strong>iTerm2</strong>: iTerm2 사용 시 자동으로 분할됩니다.

기본값은 `auto`로, tmux 세션 안이면 분할 모드, 아니면 in-process 모드가 됩니다.

필자는 <strong>tmux 모드</strong>를 명시적으로 설정했습니다:
...
tmux를 선택한 이유는 간단합니다. 5개 팀을 동시에 운영할 때, 각 팀원의 작업 현황을 <strong>한 화면에서 실시간으로</strong> 볼 수 있어야 병목을 빠르게 파악할 수 있기 때문입니다.
...
Agent Teams의 핵심 메커니즘 중 하나가 <strong>공유 태스크 리스트</strong>입니다. 팀 리더가 태스크를 생성하면 팀원들이 자율적으로 가져가서(claim) 처리합니다.
...
기본적으로 팀 리더도 직접 작업을 수행할 수 있지만, <strong>Delegate 모드</strong>를 활성화하면 리더는 조율 전담이 됩니다:
...
여기서 흥미로운 점은 OpenClaw 자체의 멀티에이전트 기능과 Agent Teams가 <strong>서로 다른 레이어</strong>에서 동작한다는 것입니다.
...
Agent Teams의 가장 큰 함정은 <strong>여러 팀원이 같은 파일을 수정하는 것</strong>입니다.
...
팀원은 CLAUDE.md, MCP 서버, 스킬은 자동 로드하지만, <strong>리더의 대화 히스토리는 상속받지 않습니다</strong>. 따라서:
...
- Agent Teams는 <strong>토론, 리뷰, 병렬 탐색</strong>에 집중하세요
...
1. <strong>실험적 기능</strong>: `EXPERIMENTAL` 환경변수명에서 알 수 있듯이, 아직 정식 기능이 아닙니다. API가 변경될 수 있습니다.

2. <strong>토큰 비용</strong>: 5명 팀이면 최소 5배의 토큰 소비가 발생합니다. ROI를 따져봐야 합니다.

3. <strong>디버깅의 어려움</strong>: 여러 팀원이 동시에 작업하면 문제 발생 시 원인 추적이 복잡해집니다.

4. <strong>순차 작업에는 비효율</strong>: 의존성이 많은 작업은 어차피 직렬로 처리되므로 팀을 쓸 이유가 없습니다.

5. <strong>같은 파일 수정 위험</strong>: 현재 파일 레벨 잠금은 지원되지 않습니다. 태스크 설계로 우회해야 합니다.

6. <strong>tmux 환경 필수</strong>: 5개 팀을 제대로 모니터링하려면 tmux가 사실상 필수입니다. in-process 모드로는 한계가 있습니다.

## 언제 쓰고, 언제 피할까

공식 문서도 "병렬 탐색이 실제 가치를 만드는 작업"에서만 Agent Teams를 권장합니다. 하루 굴려보며 정리한 판단 기준은 다음과 같습니다.
...
- 의존성이 많아 결국 직렬로 흘러갈 수밖에 없는 작업. 팀을 써도 빨라지지 않습니다.
- 여러 팀원이 같은 파일을 만져야 하는 작업. 파일 잠금이 없어 덮어쓰기 위험이 큽니다.
- 단순 반복 작업. 이런 건 서브에이전트 하나로 충분하고 토큰도 훨씬 적게 듭니다.
- 비용에 민감한 상황. 5명 팀이면 토큰이 최소 5배입니다.

처음이라면 코드를 직접 쓰지 않는 리뷰·리서치 작업부터 시작하길 권합니다. 경계가 분명해서 병렬 협업의 가치를 가장 안전하게 체감할 수 있습니다.

## 함께 읽으면 좋은 글

같은 멀티에이전트 흐름을 다른 각도에서 다룬 글들입니다.

- [멀티 에이전트 오케스트레이션 개선기](/ko/blog/ko/multi-agent-orchestration-improvement) — 에이전트들을 조율하는 패턴을 한 단계 더 깊이 파고듭니다.
- [Git Worktree로 Claude Code 병렬 세션 돌리기](/ko/blog/ko/claude-code-parallel-sessions-git-worktree) — Agent Teams 없이 수동으로 병렬 세션을 운영하는 대안입니다.
- [AI 에이전트 비용의 현실](/ko/blog/ko/ai-agent-cost-reality) — 토큰 비용이 급증하는 멀티에이전트 운용의 경제성을 따져봅니다.
...
Agent Teams는 아직 실험 단계지만, 가능성은 확실히 보입니다. 특히 OpenClaw의 멀티에이전트 아키텍처와 결합하면, 채널 레벨의 자동화 위에 세션 레벨의 병렬 협업을 얹는 이중 구조를 만들 수 있습니다.

다만 현시점에서 모든 작업에 Agent Teams를 적용하는 것은 비효율적입니다. 독립적인 작업이 많고 팀원 간 토론이 실제로 가치를 만드는 시나리오. 병렬 탐색, 코드 리뷰, 경쟁 가설 검증 같은 경우. 그런 곳에 집중하시길 권합니다.

설정 자체는 30분이면 끝납니다. 진짜 어려운 건 따로 있습니다. <strong>어떤 작업을 팀으로 묶고, 어떻게 태스크를 분해할 것인가</strong>라는 설계 문제죠. 그 감각은 직접 써보면서 키우는 수밖에 없더군요.
