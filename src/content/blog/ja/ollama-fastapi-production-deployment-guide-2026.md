...
ここまでモデル別の推奨とコストの話をしてきたが、「そもそもこのOllama + FastAPI構成を導入すべきか」を判断する基準は別に整理しておきたい。

使うべきとき：

- 開発・テスト段階で、API呼び出しごとのコストを気にせずプロンプトを無制限に反復実験したいとき
- 社内文書や個人情報のように、外部APIへ送れないデータを扱うとき
- 複数のクライアント（Webアプリ、CLI、モバイル）が同じモデルエンドポイントを共有し、モデルの差し替えを一箇所で管理したいとき
- ネットワークが不安定、または閉域環境でオフライン動作が必要なとき
- 特定ドメインにファインチューニングした、あるいは検閲されていないモデルを使うとき

避けるべきとき：

- 一人か二人がたまに使う程度なら、アダプター層なしで`ollama run`や`curl`で直接呼ぶほうがシンプルだ。わざわざFastAPIを挟む必要はない。
- GPUインフラを維持管理する人手がなく、応答品質がビジネスに直結するユーザー向け機能なら、クラウドAPIのほうが妥当だ。
- 数十〜数百の同時ユーザーをGPU1枚で捌く必要があるなら、ローカル単一ノードはすぐ限界に達する。推論サーバーを水平スケールするかクラウドへ移るのが正しい。
- ミリ秒単位のレイテンシがSLAなら、CPUのみのローカル推論（14.9秒）はそもそも候補外だ。

境界が曖昧なときに私が使う基準はシンプルだ。「トークンコストを節約する価値 > GPU運用の負担」ならローカル、逆ならクラウド。そして両者を頻繁に行き来しそうなら、最初からこのFastAPIアダプターを入れておくほうが後の切り替えコストを大きく下げてくれる。
...
この記事のコードと設定は、以下の公式ドキュメントを基準に作成・検証した。

- [Ollama API公式ドキュメント](https://docs.ollama.com/api) — `/api/generate`、`/api/tags`、ストリーミングNDJSONレスポンス形式の一次出典。GitHubの[docs/api.md](https://github.com/ollama/ollama/blob/main/docs/api.md)にも同じリファレンスがある。
- [FastAPI公式ドキュメント — StreamingResponse](https://fastapi.tiangolo.com/advanced/custom-response/#streamingresponse) — SSEストリーミングレスポンスと`media_type`設定の根拠。
- [FastAPI公式ドキュメント — Lifespan Events](https://fastapi.tiangolo.com/advanced/events/) — モデルウォームアップに使った`lifespan`パターン（旧`@app.on_event`の代替）の公式ガイド。
- [Docker Compose公式リファレンス — healthcheck](https://docs.docker.com/reference/compose-file/services/#healthcheck) — `condition: service_healthy`で起動順序を保証する方法。
- [Ollama公式サイト](https://ollama.com) — インストールスクリプトとモデルライブラリ。
