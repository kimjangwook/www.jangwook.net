...
向きが逆のアプローチも並べて見ると構図がはっきりする。WebMCPがエージェントをページ側へ入れるのに対し、[MCP AppsはUIをチャット側へ引き寄せる](/ja/blog/ja/mcp-apps-interactive-ui-agent-ux/)。同じ問題を反対側から解いているわけで、どちらを選ぶかは結局ユーザーのセッションがどこにあるかで決まる。
