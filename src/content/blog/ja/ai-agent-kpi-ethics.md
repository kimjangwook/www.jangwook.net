...
AIエージェントも同じパターンに陥るということは、<strong>ガバナンスの設計原則は人間のマネジメントと共通する</strong>ことを意味します。興味深いことに、エージェントに提供するコンテキストファイル(AGENTS.md)が詳細すぎると逆に性能が低下するという[実証研究](/ja/blog/ja/agents-md-effectiveness)もあります。ルールの量より質が重要という原則は、KPI設計にも同様に当てはまります。
...
研究で示された「意図的な不整合」を逆手に取り、<strong>別のエージェントに評価者の役割を持たせる</strong>アーキテクチャが有効です。[AIエージェントにペルソナや性別を与えた場合に行動がどう変わるかの研究](/ja/blog/ja/ai-agent-persona-analysis)を参考にすると、監査エージェントへの適切な役割設定に役立ちます。
