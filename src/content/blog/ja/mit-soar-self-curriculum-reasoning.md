...
カリキュラムをモデル自身に作らせず、環境の側から供給する路線もあります。専門業務のワークフローを模したRL環境でエージェントを訓練する[Deeptuneの「トレーニングジム」アプローチ](/ja/blog/ja/deeptune-ai-agent-training-gyms-rl-professional-workflows/)がそれです。SOARが問題を内側で作るのに対し、トレーニングジムは問題を外から調達します。どちらにせよボトルネックは同じで、採点可能な難度の階段をどう確保するかに帰着します。
