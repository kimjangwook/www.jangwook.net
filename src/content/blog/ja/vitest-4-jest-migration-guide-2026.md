...
本格的に始める前に、この記事が扱う移行経路を先に見るとこうなる。
...
| 状況 | 判断 |
|---|---|
| Vite・SvelteKit・Nuxt・Astroベースのプロジェクト | 今移行する |
| ts-jest・ESM/CJS衝突を繰り返しデバッグ中 | 今移行する |
| 実ブラウザでのコンポーネントテストが必要 | 今移行する（Browser Mode） |
| Next.js・Express の大型サーバースイート | 見送る |
| jest.config資産・スナップショット依存が深い | 見送る |
| 純粋なNodeライブラリでBrowser Mode不要 | 利得が小さい |
| 締め切り直前 | 安定したスプリントへ延期 |
