...
[MCPサーバーを作る](/ja/blog/ja/mcp-server-build-practical-guide-2026)とツール（tool）を定義し、クライアントがそのツールを呼び出す。認証？OAuth 2.1がスペックに入った。しかし「このエージェントがこのツールを1日に何回まで呼び出せるか」「機密データを返すツールは承認なしで呼び出してはならない」といったポリシーレイヤーはMCPプロトコル自体にはない。それは実装側の責任だ。
...
私が実際に経験したケースを一つ挙げると、[Claude Code](/ja/blog/ja/claude-code-parallel-sessions-git-worktree)でNotion MCPを使ってページを編集中に、意図せず別チームのページに触れてしまったことがある。エージェントが検索結果から似たタイトルのページを選び、私は承認ボタンを何も考えずに押した。データが消えたわけではないが、気まずかった。
