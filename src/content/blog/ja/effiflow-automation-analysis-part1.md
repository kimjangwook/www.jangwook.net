...
ブログ自動化を1年近く回してきて、ずっと引っかかっていたことがあります。これ、もっと削れるんじゃないか。そう思って7.5時間かけ、`.claude/`ディレクトリのファイルを片っ端から開いてみました。全部で28個。内訳は17個のAgents、4個のSkills、7個のCommandsです。数字だけ見れば大したことはないのに、いざトークン使用量を追ってみると、漏れている箇所が思った以上に多かった。ここで言うAgents・Skills・Commandsがそれぞれ何なのかは、[Claude Code公式ドキュメント](https://code.claude.com/docs/en/overview)で確認できます。

この分析は、以前まとめた[マルチエージェント・オーケストレーション改善記](/ja/blog/ja/multi-agent-orchestration-improvement)の延長でもあります。そこで扱った協業構造を、今度はコストの観点から見直したわけです。
...
Skillの構造と自動発見の仕組みは、[Claude Code Skills公式ドキュメント](https://code.claude.com/docs/en/skills)に詳しくまとまっています。外部データやツールを取り込むMCP連携は、[Model Context Protocol](https://modelcontextprotocol.io)の標準に準拠しています。
...
[Claude Code公式ベストプラクティス](https://code.claude.com/docs/en/best-practices)との比較結果:
...
この規模のマルチエージェント自動化が、すべてのブログにとって正解とは限りません。実際に1年回してみた立場から、正直なところを書いておきます。

<strong>こういう場合はよく合います</strong>:
- 発行頻度が安定しているとき。週1〜2回以上の更新なら、自動化に費やした初期コストはすぐに回収できます。
- 多言語運用が前提のとき。4言語を毎回人手で揃えるのは現実的にしんどい作業です。自動化の効果が最も体感できるのがここです。
- メタデータや推薦のように、繰り返し作業が明確に定義されているとき。入力と出力が決まっているほど、エージェントは安定して処理します。
- トークンコストを実際に追跡して削る意志があるとき。メタデータ優先設計の利点は、コストを覗き込む人にだけ見えます。

<strong>こういう場合はむしろ避けたほうがいい</strong>:
- 月に1〜2本書く程度なら、17個のエージェントをセットアップして維持するコストが削減額を上回ります。自分で書いたほうが速いです。
- 記事ごとに形式やトーンが大きく異なり、定型化が難しい場合。自動化はパターンが繰り返されるときに輝き、毎回新しい判断が要る作業には弱いです。
- 1言語だけ運用し、SEOメタデータも手作業で十分管理できている場合。複雑さだけを抱え込むことになります。
- このシステムを保守できる人がチームにいないとき。エージェント定義が変われば、誰かがデバッグする必要があります。コストそのものより先に、運用の負担を見積もるべきです。

要するに、自動化の損益分岐点は「頻度 × 反復性 × 言語数」です。3つのうち2つ以上が高ければ導入を、2つ以上が低ければ保留をおすすめします。似たような検討は[AIエージェントのコストの現実](/ja/blog/ja/ai-agent-cost-reality)でも扱っているので、判断の参考に併せて読んでみてください。
