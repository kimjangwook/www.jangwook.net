...
Part 4のテーマはRAG(Retrieval-Augmented Generation)でした。最初は「プロンプトに文書を数本くっつけるだけ」くらいに軽く捉えていたのですが、資料を追ううちに見方がすっかり変わりました。検索の品質がそのまま回答の品質になり、その検索をどう設計するかが勝負のすべてだったのです。

この記事にまとめたのは、その日扱った内容です。RAGの全体アーキテクチャ、ハイブリッド検索、Reranking、そしてGraphRAGやAgentic RAGといった比較的新しい方向まで。DeNAスタディ資料を軸にしつつ、自分で手を動かして気づいたことも書き添えています。
...
今回学んだことを踏まえ、いま運用中のシステムで手を入れたい箇所を整理してみました。
