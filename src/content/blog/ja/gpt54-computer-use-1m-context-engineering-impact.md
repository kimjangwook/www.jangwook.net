...
ツール検索はエージェントが必要なツールをオンデマンドで動的に探索できるようにする。大規模なエンタープライズMCP環境でのコスト削減効果が特に大きい。プロダクション環境へのMCPサーバー導入方法については[MCP サーバー構築実践ガイド2026](/ja/blog/ja/mcp-server-build-practical-guide-2026)で確認できる。
...
128Kを基準に設計していたエージェントアーキテクチャが1Mへと拡張される。複雑なRAGパイプラインの代わりに「必要なものを全てコンテキストに入れる」アプローチも現実的な選択肢となる。ただし、256K〜1M区間の精度（21.4%）はまだ限定的であることを認識しておく必要がある。プロダクションAIエージェントでコンテキストを効率的に設計する方法は[プロダクションAIエージェントのコンテキストエンジニアリング](/ja/blog/ja/context-engineering-production-ai-agents)で詳しく解説している。
...
最後に、MCPサーバー数が10個を超えるならツール検索の導入を検討しよう。47%のトークン削減は無視しにくい数字だ。これらの機能と組み合わせる上でのエージェントワークフローパターン全般については[Claude Codeエージェントワークフロー5パターン](/ja/blog/ja/claude-code-agentic-workflow-patterns-5-types)を参照するとよい。
