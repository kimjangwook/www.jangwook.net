...
私の立場はこうだ。llms.txtは<strong>今は宝くじではなく保険</strong>程度だ。生成コストはほぼゼロで、標準が定着する可能性に備える意味はあるので、入れてもいい。だが「llms.txtを入れたからAI検索によく拾われるだろう」は根拠のない期待だ。その時間はむしろ、上で整理したrobots.txtのボット別制御と構造化データに使うほうが、実測の効果ははるかに大きい。その後 Search Central は公式ガイドに、llms.txt を無視すると書いた。本番 robots.txt と Search Console スイッチを測った記録が、その一文を公開 URL で確かめる。
