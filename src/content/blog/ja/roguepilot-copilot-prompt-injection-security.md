...
Copilotをチームに入れるときに点検すべき項目は、脆弱性だけではありません。同じツールがコードをどこまで持ち出すのかも別途の判断であり、[4月から始まった学習データ方針とオプトアウト手順](/ja/blog/ja/github-copilot-data-policy-opt-out-guide/)は既定値が「同意」なので、放置するとそのまま通ってしまいます。
