...
「モデルを自社データに合わせて学習させたいんですが、何から始めれば?」実務でいちばんよく受ける質問だ。事前学習? ファインチューニング? 強化学習? 用語ばかり増えて、肝心の「自分の状況にどれが合うか」が見えてこない。
...
ファインチューニングやRLHF、LoRAはかなり深いテーマだ。全員に必要なわけではない。始める前に、自分の状況がどこに当てはまるかを確認しておくと時間を節約できる。

<strong>この内容が役立つ場合</strong>

- オープンソースモデル(Llama、Qwen、Gemmaなど)を自社インフラで運用している、または運用する予定がある。
- プロンプトやfew-shot例では、望む出力形式やドメイン語彙が安定して出てこない。
- 社内文書、ログ、顧客対応記録のように外部APIに送りにくいデータでモデルの挙動を変えたい。
- 推論コストやレイテンシのために、小型モデルを特定タスク向けに引き上げる必要がある。
- 応答のトーン、安全性、拒否の仕方といった「好み」をデータで整列(alignment)させたい。

<strong>読み飛ばしていい場合</strong>

- GPT、Claude、Geminiのような商用APIをそのまま使っていて、当面そのつもりだ。この場合、ファインチューニングよりプロンプト設計とRAGがほぼ常に先に来る。
- タスクが分類、抽出、要約レベルで、数行のプロンプトで十分に解ける。
- 学習データが数百件未満だ。この規模ならファインチューニングよりfew-shotのほうが安定することが多い。
- GPU予算も運用人員もない。LoRAが敷居を下げたとはいえ、ゼロではない。

まとめると、<strong>「外部APIで十分か?」</strong>をまず問う。その答えが「いいえ」のときに初めて、この記事の本論が必要になる。逆にモデルを自前で回さなければならない状況なら、以下の内容はコストを一桁減らしてくれる知識になる。

## 事前学習 vs ファインチューニング vs 強化学習

### 料理人のたとえが意外としっくりくる

DeNA資料は3つの学習方式をレストラン運営になぞらえる。最初はこじつけかと思った。ところが読み進めると、各段階の役割がくっきり見えてくる。
...
### ファインチューニングは、もう大企業だけのものじゃない

DeNA資料でいちばん刺さったのは、<strong>LLMファインチューニングがもはや大企業の専有物ではない</strong>という点だ。QLoRAとDPOが出てきて、前提が変わった:
...
- [Training language models to follow instructions with human feedback (InstructGPT、RLHF原論文)](https://arxiv.org/abs/2203.02155) (OpenAI, 2022)
