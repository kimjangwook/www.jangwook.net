...
<strong>Frontier Red Team</strong>は、AIシステムの極限能力をストレステストするチームです。最近ではClaudeを活用してFirefoxコードベースから22件のCVE（セキュリティ脆弱性）を自律的に発見したプロジェクトが代表的です。単に脆弱性を見つけるだけでなく、AIがその脆弱性を自律的にエクスプロイトできるかまでテストしました。
...
レガシーコードベースのマイグレーションと技術的負債の解消に向けた標準化されたスタートポイントをパートナー企業に提供します。
