...
description: >-
  OpenAI
  AgentKit実践ガイド2部：並列オーケストレーション、ハンドオフパターン、エラー復旧、本番デプロイまで。エンタープライズ環境でAgentKitを安定運用するための高度なアーキテクチャパターンとコード例を網羅。
pubDate: '2025-10-21'
...
faq:
  - question: "階層型、イベント駆動、グラフベースのパターンはどう使い分ければよいですか？"
    answer: "明確な責任分離が必要でエージェント1体では回らなくなったら階層型マネージャー・ワーカーが向いています。イベントを待ち受けて適切なエージェントを起こすリアルタイム系にはイベント駆動が、ループや条件分岐など実行時に変わる複雑な判断フローにはグラフベースが適しています。"
  - question: "なぜカスタムMCPサーバーが必要で、エージェントとどう接続しますか？"
    answer: "AgentKitの本領は自作ツールを通じてSlackやJIRA、社内DBと連携できる点にあり、それをMCPプロトコルで実現します。本文のSlack例のようにServerを継承し、send_messageなどをToolデコレータで登録してstdioで起動します。あとはエージェントのmcp_serversパラメータに接続するだけです。"
  - question: "複数テナントのデータ分離はどう担保しますか？"
    answer: "テナントごとに独立したSessionを作り、tenant_idをハッシュ化してセッションIDの名前空間を分けます。本文の例ではテナントを限定するセキュリティ用システムメッセージを注入し、クエリにデータスコープのフィルタを付けます。GDPR対応として削除・エクスポート要求の処理と監査ログも別途扱います。"
  - question: "これらのパターンを適用すると実際どの程度の効果がありますか？"
    answer: "本文のケーススタディではSaaSオンボーディング時間が7日から2日へ70%短縮し、30日チャーン率は40%から12%へ下がりました。データパイプラインのMTTRは2時間から22分へ82%短縮、DevOpsのデプロイ待ち時間は3時間から8分になりました。いずれも特定企業の数値なので環境により変わります。"
---
