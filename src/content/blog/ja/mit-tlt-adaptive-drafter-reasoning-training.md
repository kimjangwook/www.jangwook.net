...
推論LLM（例：DeepSeek-R1、o1シリーズ）は、複雑な問題を段階的に考える能力を備えるために[RL訓練](/ja/blog/ja/dena-llm-study-part3-model-training)が不可欠ですが、このプロセスで<strong>全体実行時間の最大85%</strong>がロールアウト（ロールアウト）段階に費やされます。TLTはこのボトルネックを解決し、同じハードウェアで訓練速度を実質2倍に加速します。
...
第2の革新は、<strong>[推論的デコーディング](/ja/blog/ja/llama-cpp-iq-quantization-merge)（スペキュラティブ・デコーディング）をRL訓練のロールアウトに適用する</strong>ことです。
...
- [CUDAGraphメモリ最適化](/ja/blog/ja/heretic-12-vram-reduction)：30.39GB → 10.69GB（<strong>2.8倍削減</strong>）
