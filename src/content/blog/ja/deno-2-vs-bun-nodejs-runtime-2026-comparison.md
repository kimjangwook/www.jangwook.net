...
抽象的な比較より、意思決定の基準のほうが役に立つ。実測結果をもとに状況別に整理する。

**Bunを使うべきとき**

- 既存のNode.jsサービスの速度を改善しつつ、移行コストは最小限に抑えたいとき。`package.json`と`node_modules`がそのまま動く。
- Jestベースのテスト資産が多いとき。`bun:test`がJest APIと互換なので、移行の負担が小さい。
- npm依存が深いサービス。社内ライブラリやレガシーパッケージがnpmにしかないなら、Bunが安全だ。

**Bunを避けるべきとき**

- 信頼できないサードパーティコードを実行するとき。デフォルトのサンドボックスがなく、ファイルシステムとネットワークが開いている。
- CIで厳格な型検証をゲートにしたいとき。Bunは型チェックをしないので、`tsc`を別途呼び出す必要がある。
...
この記事の数値は自分で測定したが、動作の仕組みと互換性は各ランタイムの公式ドキュメントで確認した。

- [Deno公式サイト](https://deno.com) — ランタイム概要、JSR、権限モデルの紹介
- [DenoのNodeおよびnpm互換性ドキュメント](https://docs.deno.com/runtime/fundamentals/node/) — `node:`接頭辞と`npm:`指定子、package.jsonの認識方法
- [Denoのセキュリティと権限ドキュメント](https://docs.deno.com/runtime/fundamentals/security/) — `--allow-*`フラグとデフォルトのサンドボックス設計
- [Bun公式サイト](https://bun.sh)と[Bunインストールドキュメント](https://bun.sh/docs/installation) — 単一実行ファイルのインストールとプラットフォーム要件
- [Node.js公式サイト](https://nodejs.org) — 比較の基準となった標準ランタイム
