...
<strong>値ではなくDOMを指すフィールドは、この検査では捕まらない。</strong>`speakable`の`cssSelector`のように、ページのどこかを指す値は文字列としては常に妥当だ。セレクタが何も掴まなくても十三箇所を掴んでも、スキーマ検査は通してしまう。この種の腐り方を実際に測った記録は[「ここを読み上げて」が13段落を指していた](/ja/blog/ja/speakable-cssselector-pointer-rot-2026/)にある。
