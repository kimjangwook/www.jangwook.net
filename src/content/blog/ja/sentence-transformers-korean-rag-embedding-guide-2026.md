...
埋め込みを生成する前に、モデルはまずテキストをトークンに分割する。all-MiniLM-L6-v2はWordPieceトークナイザーを使う。同じテキストでも言語によってトークン数が大きく変わるのが興味深い。
...
texts = [
    "AI agent uses tools to complete tasks",          # 英語
    "AIエージェントがツールを使ってタスクを完了する",              # 日本語
    "AI 에이전트가 도구를 사용해 작업을 완료한다",               # 韓国語
]

for text in texts:
    tokens = tokenizer.tokenize(text)
    print(f"トークン数: {len(tokens):2d} | {text[:35]}")
    print(f"  トークン: {tokens[:8]}...")
    print()
```

このコードをそのまま実行してはいないが、WordPieceトークナイザーの既知の特性から見て、英語の文は単語単位で分割される一方、韓国語や日本語は対応する文字が語彙にないため未知のサブワード（UNK処理または文字単位の分割）として処理されることが多い。結果として同じ意味の文でも韓国語・日本語版のほうが多くのトークンを消費し、モデルの最大入力トークン（256）を早く使い切る。

これが韓国語クエリの精度低下の二つ目の原因だ。一つ目は学習データの偏り、二つ目はサブワードトークナイズの過程で韓国語の形態素が持つ意味情報が失われる点である。多言語モデルは語彙辞書（vocabulary）に韓国語を含むため、韓国語テキストも意味のあるサブワード単位に分割できる。
...
- **キーワード一致が意味的類似度より重要なとき。** 正確な用語・コード・固有名詞のマッチングが核心なら、埋め込みよりBM25などのキーワード検索が良い。実務では両者を組み合わせた[ハイブリッド検索](/ja/blog/ja/llamaindex-vs-langchain-vs-haystack-rag-2026)が正解の場合が多い。
