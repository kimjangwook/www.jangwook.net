...
faq:
  - question: "/insightsはどうやって実行しますか？"
    answer: "Claude Code v2.1で導入された機能で、ターミナルで/insightsと入力するだけです。ローカルに保存された使用データをもとに、使用統計、強み、ボトルネック、改善提案を分析してくれます。"
  - question: "分析で最大のボトルネックとされたのは何ですか？"
    answer: "コンテキスト制限でした。小説レビュー、大規模翻訳、ブログコンテンツ生成などの作業が途中で終了し、根本原因は単一セッションに大きすぎる作業を割り当てるパターンでした。"
  - question: "File Too Largeエラーが293件も発生したのはなぜですか？"
    answer: "小説プロジェクトの大容量テキストファイルが原因でした。このデータを見て、ファイル分割戦略の必要性を実感したと本文で述べています。"
  - question: "今すぐ適用できる改善策は何ですか？"
    answer: "大きな作業をチャプター・セクション・モジュール単位で分割し、task-status.mdのようなファイルに進捗をチェックポイントとして残すことです。セッションが切れても次のセッションで続きから作業できます。"
---

「自分はClaude Codeを上手く使えているのだろうか？」

毎日使っているのに、この問いにはどうも歯切れよく答えられませんでした。そんなとき、Claude Codeに`/insights`という機能がひっそり用意されていることに気づきます。実際の使用データをもとに、自分のワークフローを客観的に診断してくれる機能です。

以下は、私が実プロジェクトで`/insights`を回した結果のすべてと、そこで見えてきたこと、そしてその週からさっそく取り入れたTipsです。

## /insightsとは何か？

`/insights`はClaude Code v2.1で導入された使用パターン分析機能です。ローカルに保存された使用データに基づいて、以下を分析してくれます：

- <strong>使用統計</strong>：メッセージ数、セッション数、ファイル修正履歴
- <strong>うまくいっていること</strong>：効果的に活用できている機能とパターン
- <strong>障害要因</strong>：生産性を低下させるボトルネック
- <strong>改善提案</strong>：すぐに適用できる実践的なTips
- <strong>プロジェクト領域別分析</strong>：どの作業にどう使っているか

いわば「AIコーディング健康診断」のようなものです。実行方法も簡単で、Claude Codeで`/insights`と入力するだけです。

## 実行結果：コア指標

> <strong>分析期間</strong>：2025-12-31 〜 2026-02-03（約35日間）
> <strong>環境</strong>：Claude Code v2.1.31

### 使用量一覧

| 指標 | 数値 |
|------|------|
| 総メッセージ | <strong>4,516件</strong> |
| 総セッション | <strong>1,042回</strong> |
| ファイル修正 | <strong>6,267件</strong> |
| コード変更量 | <strong>+1,046,046 / -109,155行</strong> |
| 活動日数 | <strong>15日</strong> |
| 1日平均メッセージ | <strong>301.1件</strong> |
...
私は大規模作業を行う際に必ずタスクリストを先に作成し、これを並列エージェントに分配するパターンを使っています。`/insights`はこれを「洗練されたワークフロー（sophisticated workflow）」と評価しました。AIエージェント協働パターン：5つの専門エージェントでフルスタックアプリを構築する方法では、このマルチエージェントアプローチの具体的な実装例を確認できます。

実際の活用例：
- <strong>小説プロジェクト</strong>：総合レビュー → 問題特定 → 並列で複数ファイル同時修正
- <strong>ブログ生成</strong>：韓国語作成 → 日本語/英語/中国語の並列翻訳
- <strong>コードリファクタリング</strong>：イシュー分析 → タスク分配 → 同時修正
...
### コンテキスト制限が最大のボトルネックだった

> 「コンテキスト制限により、最も野心的なセッションが途中で終了」

これが最大の問題でした。特に以下の作業で頻繁に発生しました：

- <strong>小説レビュー</strong>：複数巻にわたる総合レビュー中に中断
- <strong>翻訳作業</strong>：大規模翻訳の実行中に途切れる
- <strong>ブログコンテンツ生成</strong>：初期読み込み・設定段階で終了

根本原因は<strong>「単一セッションに大きすぎる作業を割り当てるパターン」</strong>でした。すべてを一度に解決しようとする欲張りが、かえって非効率を生んでいたのです。
...
午後と夜間に作業が集中するパターンがはっきり出ています。深夜でも17%あるのは、夜遅くまで疲れ知らずのコーディングパートナーでいてくれるというAIならではの強みを、そのまま活かしている証でもあります。
...
1. <strong>大きな作業は必ず分割しましょう</strong>：単一セッションにすべてを詰め込まず、チャプター/セクション/モジュール単位で分けましょう。

2. <strong>チェックポイントをファイルに残しましょう</strong>：`task-status.md`のようなファイルを作成して進捗を記録すれば、セッションが切れても続きから作業できます。

3. <strong>カスタムスラッシュコマンドを作りましょう</strong>：繰り返し作業があれば`/write-post`のようにコマンドにエンコードしましょう。一度作れば何度でも再利用できます。

4. <strong>並列エージェントを積極的に活用しましょう</strong>：`TodoWrite` → `TaskCreate`パターンで作業を分配すれば効率が大幅に向上します。

### /insightsを最大限活用する方法

1. <strong>定期的に実行しましょう</strong>：月に1回程度`/insights`を実行すれば、作業パターンの変化を追跡できます。

2. <strong>CLAUDE.mdに反映しましょう</strong>：`/insights`が提案した改善事項をCLAUDE.mdに記録すれば、Claude Codeが自動的にそのルールに従います。Claude Code ベストプラクティスでは公式推奨設定を実際のプロジェクトに適用した事例を詳しく解説しています。

3. <strong>エラーパターンに注目しましょう</strong>：「File Too Large」や「Command Failed」のようなエラーが多い場合、ワークフローの調整が必要です。

4. <strong>HTMLレポートを活用しましょう</strong>：`/insights`は詳細なHTMLレポートも生成します。可視化されたデータでより深い分析が可能です。

## /insightsを使うべき場面と避けるべき場面

良いツールでも、使うタイミングが合って初めて価値が出ます。1か月以上回してみて整理した、`/insights`が活きる場面と、かえって誤解を招きやすい場面です。

### 使うと良い場面

- <strong>作業量が十分に溜まったとき</strong>：分析はローカルに保存された約30日分の使用データを基にします。数日しか使っていない状態で回すと標本が少なく、パターンが見えにくいです。私のように数千件のメッセージが溜まったときに真価が出ます。
- <strong>ワークフローを変えた直後の点検</strong>：新しいカスタムコマンドを導入したり、並列エージェントのパターンを変えたりしたら、1か月後にもう一度回し、エラー率やセッション効率が実際に改善したか比較するのに向いています。
- <strong>ボトルネックが漠然と感じられるとき</strong>：「なぜ途中で切れるのか」という勘をデータで確かめたいとき。私の場合、コンテキスト制限が原因だとこの機能で初めて明確に見えました。

### 注意・回避すべき場面

- <strong>チーム単位の指標が必要なとき</strong>：`/insights`はあくまで自分のローカル使用データを見る個人向けツールです。チーム全体の採用率、PR貢献度、シート利用率といった組織指標は[公式Analyticsダッシュボード](https://code.claude.com/docs/en/analytics)や[Console分析](https://platform.claude.com/claude-code)を見るべきです。両者は目的が異なります。
...
`/insights`を実行して最も強く感じたのは、<strong>「自分がAIをどう使っているか知らないまま、上手く使えていると勘違いしていた」</strong>ということです。

データに基づく客観的な診断は以下の価値をもたらします：

- <strong>隠れたボトルネックの発見</strong>：コンテキスト制限の問題を認識していなければ、同じ失敗を繰り返していたでしょう。
- <strong>効果的なパターンの強化</strong>：並列エージェント活用が実際に効果的であるという確認は自信につながります。
- <strong>具体的な改善方向</strong>：「もっと上手く使ってください」ではなく「チェックポイントを導入してください」という具体的なアクションアイテムを提示してくれます。

AIコーディングツールを使うすべての開発者に`/insights`の実行をお勧めします。5分で実行できますが、そこから得られるインサイトは今後のワークフローを根本的に変える可能性があります。

## 統計ではなく、コーチだった

Claude Code `/insights`は単なる統計機能ではありません。AIコーディングツールとの協業のしかたをデータで映し出し、具体的な改善の方向まで示してくれる<strong>AIワークフローコーチ</strong>に近い存在です。

4,516件のメッセージ、1,042回のセッション、6,267件のファイル修正。これらの数字が語るのは「たくさん使った」ということではありません。「どう使っていて、どこに手を入れればもっと良くなるか」ということです。
...
- [Claude Code Analytics ダッシュボード (Console)](https://platform.claude.com/claude-code)
