...
description: 'anthropic 0.101.0 SDKを直接インストールしてtool_useエージェンティックループを実装した。JSONスキーマツール定義から複数ツール呼び出し、エラーハンドリング、ストリーミング、コスト最適化まで — チャットボットとエージェントを分けるコアパターンを段階的な実践コードで解説する。'
pubDate: '2026-05-13'
...
この記事では、anthropic SDK 0.101.0を実際にインストールして検証したTool Useパターンをまとめた。基本的なツール定義からエージェンティックループ、エラーハンドリング、コストまで — 実際に使えるコード中心で。

## Tool Useがチャットボットと違う理由 — 構造的な差

LLMは確率分布からトークンをサンプリングする。日付計算、正確な数値演算、外部API照会などの作業は構造的に信頼しにくい。学習データのパターンを再現するだけだからだ。
...
<strong>1. Prompt Cachingとの組み合わせ</strong>: ツール定義は毎リクエスト同一だ。[Claude API Prompt Cachingガイド](/ja/blog/ja/claude-api-prompt-caching-cost-optimization-guide)で扱ったように、キャッシングを活用できる。
...
[Vercel AI SDK方式](/ja/blog/ja/vercel-ai-sdk-claude-streaming-agent-2026)を参考にすると、フロントエンド統合でこの部分がどのように抽象化されるか比較できる。
...
anthropic 0.101.0で直接実験した結果をまとめると：
