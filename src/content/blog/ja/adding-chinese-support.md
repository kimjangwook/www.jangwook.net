...
この記事では、<strong>Astroベースの多言語ブログに新しい言語を追加する全過程</strong>を詳しく共有します。ファイル構造の設計から大規模翻訳自動化、SEO最適化まで - 実践で直面した問題と解決策を透明に記録しました。コンテンツ生産全体を LLM で自動化した背景は[LLM ベースのブログ自動化](/ja/blog/ja/llm-blog-automation)で扱っています。
...
40件の投稿に中国語版を追加し、既存の韓国語/英語/日本語版のrelatedPostsに中国語reasonを追加する作業は、<strong>手作業では不可能</strong>でした。複数のエージェントに作業を分けて並列処理するパターンは[AI エージェントと Notion MCP 自動化](/ja/blog/ja/ai-agent-notion-mcp-automation)でも同じ方式で活用しました。
