---
title: 'ローカルエージェントがシステムプロンプトを忘れた理由 — Ollama num_ctx無言truncationを実測'
description: >-
...
これが単なる豆知識でない理由はportabilityだ。私の32GBデスクトップで快調だったエージェントが8GBクラウドインスタンスに載ると、既定num_ctxがより小さく決まり、同じコードが同じ入力で無言で切られうる。ローカルでテストすると平気なのにデプロイすると品質が崩れる、再現の難しい事故につながる。私は「既定値を信じず常に明示せよ」という立場だ。既定値がマシンごとに違うなら、それは事実上信頼できない値だ。しかもこの天井はリクエスト一本だけのものではない。同時リクエストを何本受ける設定にするかで、一スロットが使える窓はさらに狭くなる。そちらは[`num_parallel`を上げながら同時リクエストの処理量を測った実験](/ja/blog/ja/local-llm-concurrent-requests-num-parallel-experiment/)で別に扱った。
...
[コールドスタート時にload_durationを追跡した](/ja/blog/ja/local-llm-cold-start-load-duration-experiment/)ときと同じ筋だ。Ollamaが応答にそっと添えて返す数字たちは、ドキュメントにあまり書かれていなくても、実挙動を追う一番正直な手がかりだ。`load_duration`がコールドスタートを密告したように、`prompt_eval_count`は切り落としを密告する。対になる`eval_count`も同じ性格で、推論モードを入れたとき[見えない思考トークンがコストをどれだけ押し上げるか](/ja/blog/ja/local-llm-reasoning-mode-token-cost-experiment/)はその数字で測った。ローカルモデルを本気で回すなら、この数字たちを一度のぞいてみることをすすめる。
