...
エージェント17個、コマンド6個、スキル4個。ブログ自動化システムがここまで膨らんだころには、ファイルを一つ直すたびに別のどこかが壊れる予感がして、手をつけるのが怖くなっていました。そこで一つずつ手で直すのをやめ、Claude Codeのマルチエージェントオーケストレーションパターンに48ファイルをまとめて読ませました。出てきた問題は61件。この記事は、その作業の記録です。Claude Code集中運用時のコストと効率についての分析は[Claude Code実使用分析インサイト](/ja/blog/ja/claude-code-insights-usage-analysis)で別途紹介している。Claude Codeにおけるエージェントワークフローの5つのパターンは[Claude Agent SDKツール使用完全ガイド](/ja/blog/ja/claude-agent-sdk-tool-use-complete-guide-2026)で体系的に解説している。
...
## マルチエージェントを使うべき時、避けるべき時

このパターンは万能ではありません。48個のファイルを手作業で分割してみて、むしろ単一エージェントの方がよかった場面も確かにありました。導入前に、以下の基準で一度ふるいにかけてみてください。

### 向いているケース

- <strong>作業範囲が自然に分かれるとき</strong>: agents、commands、scripts、guidelinesのように領域が独立していて共有コンテキストが少なければ、各エージェントは自分の領域だけ見ても十分に判断できます。
- <strong>ファイル・課題の数が単一コンテキストを超えるとき</strong>: 一つのエージェントが丸ごと読むと途中から文脈が薄れる規模（おおよそ30ファイル以上）で、分割の利点が明確になります。
- <strong>反復検証が品質を左右するとき</strong>: レビュー → 修正 → 再検証のループを何度も回す文書・コード整理に適しています。
- <strong>同じ種類の作業を繰り返すとき</strong>: 公式サブエージェント文書が推奨するように、同一の指示を繰り返すワーカーは専用エージェントとして固定する方が効率的です。

### 避けるべきケース

- <strong>作業が強く絡み合っているとき</strong>: 一つのファイルの判断が他の五つを連鎖的に変える構造なら、エージェント間の調整コストが分割の利点を食い潰します。単一コンテキストで順次処理した方が速いです。
- <strong>規模が小さいとき</strong>: 3〜4個のファイル修正にオーケストレーターを立てるのは過剰設計です。オーケストレーション自体のオーバーヘッドの方が大きくなります。
- <strong>探索的で範囲が毎回変わるとき</strong>: 何を直すかまだ分からない段階では、分割の基準そのものが揺れます。まず単一エージェントで全体を俯瞰してから分割してください。
- <strong>コスト・遅延に敏感なとき</strong>: 複数エージェントを同時に走らせるとトークンと呼び出しが掛け算で増え得ます。メタデータキャッシュなしに無闇に並列化すると、かえって高くつきます。

要するに、マルチエージェントは<strong>独立した大きな作業</strong>に強く、<strong>小さく絡み合った作業</strong>に弱いということです。迷ったら単一エージェントで始め、コンテキストが溢れたときに初めて分割しましょう。チーム単位の協働構造をもっと知りたい場合は、[Claude Code エージェントチーム実践ガイド](/ja/blog/ja/claude-agent-teams-guide)が参考になります。
...
- [カスタムサブエージェントの作成（公式ドキュメント）](https://code.claude.com/docs/en/sub-agents)
