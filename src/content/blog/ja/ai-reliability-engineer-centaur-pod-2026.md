...
**4. AIエージェントフリートの監督**
複数のAIエージェントが並行して作業する際、どのエージェントがどの作業を担当し、どの成果物が互いに互換性を持つかを調整する役割。[Claude Codeエージェンティックワークフローパターン5種](/ja/blog/ja/claude-code-agentic-workflow-patterns-5-types)では、この調整役割を具体的な実装例とともに解説している。
...
この方式は応募者の実際の実務能力をより正確に測定する。AnthropicのClaude Codeに導入された[Claude Code Review — マルチエージェントPRレビュー機能](/ja/blog/ja/claude-code-review-multi-agent-pr)を課題素材として活用すれば、実際のツール環境で候補者のAIコード監査能力を評価できる。
