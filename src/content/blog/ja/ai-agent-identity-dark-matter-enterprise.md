...
The Hacker Newsが3月に報じた事例では、エージェントが「最もうまく動作する」方法を見つける過程で、数ヶ月前に退職した人物の孤立アカウントを活用するパターンが発見されました。このアカウント1つが**複数エージェントの再利用ショートカット**となり、一度の侵害がエージェントフリート全体に影響を与える構造が生まれていました。外部ライブラリを経由した類似のサプライチェーンリスク事例は[LiteLLMサプライチェーン攻撃——AI依存性セキュリティリスク](/ja/blog/ja/litellm-supply-chain-attack-ai-dependency-security)で確認できます。
...
44%の企業が使用する静的APIキーは最も危険な認証方式です。有効期限のないキーは、侵害されると永続的な被害をもたらします。MCPの設定ファイルにシークレットが広がる実際のパターンは[AIコーディングツールのシークレットスプロール: MCP設定セキュリティガイド](/ja/blog/ja/ai-coding-secrets-sprawl-mcp-config-security)で詳しく解説しています。
...
エージェントが何をしたかを追跡できなければ、問題が発生しても原因を特定できません。すべてのエージェントアクションをロギングし、人間スポンサーに紐づけてください。プロダクション環境でエージェント行動を体系的に監視する方法は[プロダクションAIエージェントオブザーバビリティガイド](/ja/blog/ja/ai-agent-observability-production-guide)で詳しく説明しています。
