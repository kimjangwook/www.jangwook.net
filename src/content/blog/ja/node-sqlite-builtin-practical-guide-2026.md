...
`node:sqlite` と `better-sqlite3` で迷うなら、判断は意外とシンプルだ。どちらも同期専用で SQLite を埋め込む点は同じなので、分かれ目は「外部依存を背負ってでも欲しい機能があるか」だ。

**node:sqlite を選ぶとき:**

- CI や Alpine コンテナでネイティブビルドが失敗するのをもう見たくないとき。これが最大の動機だ
- 内部 CLI ツール、ビルドスクリプト、キャッシュ層など、依存をゼロに近づけたい環境
- トランザクションが単純、または `withTransaction` ヘルパー一つで足りる場合
- Docker イメージを小さくしたいとき。`node_modules` にネイティブアドオンがなければ、ビルドステージが丸ごと消える

**better-sqlite3 を維持するとき:**

- `db.transaction()` のネストトランザクション（savepoint）や `deferred`/`immediate`/`exclusive` モードを実際に使っているとき
- `serialize()`/`deserialize()` でメモリ DB を Buffer にダンプ・復元するロジックがコードに組み込まれているとき
- Node.js 22 LTS に固定された本番サーバーで、experimental 警告やマイナーバージョン間の API 変更を許容できないとき
...
自分で確かめたいなら、以下の公式ドキュメントを参照してほしい。

- [Node.js 公式ドキュメント — SQLite (node:sqlite)](https://nodejs.org/api/sqlite.html): `DatabaseSync`、`StatementSync`、`db.function()`、`db.aggregate()` などの全 API リファレンス
- [WiseLibs/better-sqlite3 (GitHub)](https://github.com/WiseLibs/better-sqlite3): 比較対象ライブラリの公式リポジトリと API ドキュメント
- [SQLite 公式ドキュメント — Result and Error Codes](https://www.sqlite.org/rescode.html): `errcode` 値（1、19 など）の意味の全リスト
- [SQLite 公式ドキュメント — Write-Ahead Logging](https://www.sqlite.org/wal.html): WAL モードの仕組みとトレードオフ
