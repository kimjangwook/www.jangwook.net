...
当前LLM生态由AR模型主导——GPT、Claude、Gemini都采用AR方式。CDLM证明扩散模型可以在<strong>速度和质量两方面都具有竞争力</strong>。在软件算法加速的同时，[通过专用ASIC芯片让Llama 8B推理达到16,000 tok/s的硬件方法](/zh/blog/zh/asic-llm-inference-16k-tps)也在同步推进，推理速度革命正在整个技术栈中全面展开。
...
CDLM是扩散语言模型实用化的重要一步。块级因果结构实现了KV缓存，一致性训练大幅减少了步骤数同时保持了质量。最高14.5倍的延迟改善对以AR为中心的当前范式构成了有意义的挑战。如需了解通过硬件量化技术降低推理成本的方案，可参考[NVIDIA NVFP4将LLM推理成本降至八分之一的方法](/zh/blog/zh/nvidia-llm-inference-cost-reduction)。
