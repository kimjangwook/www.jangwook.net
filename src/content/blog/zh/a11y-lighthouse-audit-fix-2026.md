...
这和我那篇[主张"爬虫读取的标记才是真正胜负手"的结构化数据文章](/zh/blog/zh/localbusiness-structured-data-server-side-vs-js-2026/)是同一条思路。在屏幕上好看，和被机器理解结构，是两回事。无障碍就是从人类用户这一侧去看那个"机器读取层"的问题。 至于在那个机器层上、一开始要放哪些爬虫进来的问题，我在[拒绝训练、允许引用的 robots.txt 策略](/zh/blog/zh/ai-crawler-control-robots-txt-llms-txt-2026/)里另作讨论。
...
- **把自动审计放进CI，但别把通过当成最终放行。** 用Lighthouse CI或`axe-core`接进构建，防止回归。`lang`、`alt`、对比度、标题、链接名称这些机器可判定的违规，在这一层全部拦下。同样是`axe-core`，在jsdom和真实浏览器下能抓到的违规有何差异，我在[把axe-core接进CI并实测两种环境的记录](/zh/blog/zh/axe-core-ci-a11y-jsdom-vs-browser-2026/)里另作了整理。
...
这个顺序的核心，是自动与手动的分工。自动化工具用很低的成本挡住可重复的机械违规；人则去看工具原理上看不到的那件事——它到底能不能用。在模态框里按 Tab、数一数焦点落到哪，就是最典型的一例。我[实测只挂了 `aria-modal="true"` 的模态框的焦点逃逸](/zh/blog/zh/modal-focus-escape-inert-measure-2026/)时，审计工具一声不吭，焦点径直走到了背后的页面上。只做其中一样，就只做了一半。而无论哪一样，在轻信别人的基准之前，第一步都是在自己的环境里亲手跑出before/after的数字——就像我当初[把自己的网站当作实测对象](/zh/blog/zh/multilingual-llm-token-tax-experiment/)那样。
