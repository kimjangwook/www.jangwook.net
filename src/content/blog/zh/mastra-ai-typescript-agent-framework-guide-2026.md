...
description: '我亲自安装了Mastra.ai TypeScript AI代理框架，将其连接到Google Gemini并构建了一个天气代理。从安装到实际工具调用的完整实验记录。'
pubDate: '2026-06-14'
...
在JavaScript生态中构建AI代理时，经常听到这样的说法："TypeScript开发者的选择无非就是LangChain.js或Vercel AI SDK。"我也这么认为，直到今年初了解了Mastra.ai——这个通过YC W25批次获得1300万美元融资并发布v1.0的框架。

听说过这个名字，但从未亲自试用过。今天，我实际安装了它，将其与Google Gemini连接，并运行了一个具有工具调用能力的代理。目的是验证"5分钟内运行"的宣传是否属实。
...
支持的LLM提供商包括OpenAI、Anthropic、Google Gemini、Meta Llama等，几乎覆盖了Vercel AI SDK支持的所有模型。Mastra内部以Vercel AI SDK为基础，所以如果你有[使用Vercel AI SDK构建Claude流式代理的经验](/zh/blog/zh/vercel-ai-sdk-claude-streaming-agent-2026)，那么对底层已经很熟悉了。
...
如果你正在用TypeScript启动一个新的代理项目，Mastra绝对值得认真考虑。从安装到运行第一个代理只需5〜10分钟，框架结构也很清晰。
