...
另外，所有结果只属于 axe-core 4.13.0，换个检查器就是另一份清单。最后一句虽然是常识但还是要写：<strong>把自动规则全跑通，不等于 WCAG 合规。</strong>合规是人来判定的。这个落差最明显的一条是 1.4.12 文本间距。我[按标准值把字距调宽，570 处文字被切掉](/zh/blog/zh/text-spacing-1412-clamp-audit-2026/)，而这些页面在检查器里全是 AA 通过。
