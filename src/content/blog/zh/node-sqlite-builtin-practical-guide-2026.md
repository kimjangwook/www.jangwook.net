...
graph TD
    ...
    A --> C["db.prepare(SQL)<br/>生成 StatementSync"]
    A --> D["db.function()、db.aggregate()<br/>注册自定义函数"]
    C --> E["run() — 写入"]
    C --> F["get() — 单行"]
    C --> G["all() — 全部行"]
```
...
| 项目 | node:sqlite | better-sqlite3 |
|---|---|---|
| 安装 | Node 22.5+ 内置，零依赖 | npm 安装 + 原生构建 |
| CI、Alpine 构建失败 | 无 | 可能发生 |
| 事务 API | 手动 exec('BEGIN'/'COMMIT') | db.transaction() 助手、savepoint |
| serialize/deserialize | 不支持 | 支持 |
| 稳定性 | experimental（小版本可能变动） | 生产环境验证 |
| 适合场景 | 内部 CLI、构建脚本、缓存 | 现有生产环境、高级事务 |
