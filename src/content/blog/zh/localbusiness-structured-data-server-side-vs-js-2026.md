...
沿用我一贯[把这个博客当作实验台、用数字验证假设的方式](/zh/blog/zh/multilingual-llm-token-tax-experiment/)，这次我直接测了渲染。我把同一份LocalBusiness标记做成两种方式。一种是服务器输出的静态HTML，另一种在运行时用JS注入。
...
- 页面上不可见的信息不要标记。
- 部署后用富媒体结果测试和网址检查工具，检查原始与渲染两侧。就像我[靠实测把Lighthouse无障碍分数从55提到100那次](/zh/blog/zh/a11y-lighthouse-audit-fix-2026/)一样，用测量而非"用JS注入了应该没事"的猜测来收尾。
- 不承诺排名效果。守住"标记是帮助爬虫理解的辅助"这条线。

同样的信息，[用什么格式喂给机器会改变成本与识别，这一点我曾按token逐一实测过](/zh/blog/zh/llm-token-cost-data-format-experiment/)——结构化数据服务端与JS的取舍，本质上也是同一类问题。
