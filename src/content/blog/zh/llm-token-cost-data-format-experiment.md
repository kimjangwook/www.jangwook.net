...
再看表格：TSV（1,568）比 CSV（1,650）少 82 个 token。字符数同为 3,742，只有 token 不同。我一开始怀疑"难道 tab 比逗号更省 token？"，于是单独测了一下。
...
按行来看，逗号和 tab 都是 12 个 token。分隔符本身并不更便宜。差距来自 BPE（Byte Pair Encoding）在整个文档范围内合并相邻字节时，tab 与周边的值碰巧合并得更好。所以我不会把这 5% 的差距泛化成"该用 TSV"。<strong>选择"一行表头 + 只排值"的表格类格式，这个大决定占 95%；tab 还是逗号只是剩下 5% 的噪声。</strong>说实话，那 5% 换一份数据就可能反转。

不过有一条实用建议是确定的：缩进很贵。pretty JSON 的两空格缩进（`"  "`）本身就占一个 token，50 条记录 × 9 个字段时，这个 token 会被铺几百次。如果不是给人读的，仅去掉 `indent` 就能让 JSON 减少 37.5%。
