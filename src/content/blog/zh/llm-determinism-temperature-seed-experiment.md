...
所以我自己测了。不是在云端API上，而是在我能从头到尾掌控的本地Ollama + Gemma 4环境里。把同一提示词发送数十次，按哈希把输出归类，数一数出现了几种。先说结论：在我的环境里，可复现性归结于恰好两个参数。
...
还有一个值得一提的参数是`num_predict`，即生成的最大token数。它短时，模型可分叉的区段就少，看起来更确定；它长时，越往后微小差异越有累积的余地。所以我先用一句短标语(约40个token)抓取干净的信号，长输出另行测试。结果正是在这个长输出测试中，我遇到了后面要讲的空响应问题。
...
实验环境建在repo之外的临时目录里。靠自己实测而非猜测来下结论，和我[用9种数据格式实测token成本](/zh/blog/zh/llm-token-cost-data-format-experiment)时是同一套做法。Ollama 0.30.7、Apple Silicon，用了两个模型：一个2GB的小Gemma 4构建，一个9.6GB的`gemma4:e4b`。用两种尺寸是想看看同样的模式是否与模型规模无关。
...
这个结果干净到我反而又怀疑了一次，于是换模型重跑。尺寸相差5倍的两个模型给出了相同的模式。至少在我的环境里，可复现性由temperature和seed两个参数决定就够了。
