...
知道"何时该走开"和知道"该选什么"同样重要。以下是我在实测中遇到的回避信号。

**何时避免LlamaIndex**

- 在一个进程里运行多条管道时。全局`Settings`对象会覆盖各管道的模型配置。如果你需要同时跑两种嵌入模型，最好另寻方案。
- 在意依赖体积时。28个core依赖是三者中最重的，在对镜像大小敏感的无服务器环境里会成为负担。
- 需要完全掌控内部行为时。抽象层较厚，检索结果不符预期时难以定位原因。

**何时避免LangChain**

- 全新项目却想深度依赖`langchain-community`时。在弃用方案尘埃落定之前，只用独立包或推迟决定更稳妥。
- 维护对破坏性变更敏感的长期项目时。API变化快，连小版本升级有时也会出问题。

**何时避免Haystack**

- 目标是当天就能跑起来的原型时。同样的RAG要写将近两倍的代码，对快速验证而言过重。
- 生态规模和第三方集成数量是决定因素时。Haystack的集成数量少于LangChain。
- 独自编写小脚本时。显式的图声明反而成了累赘。

## 官方文档与出处

这些是我直接核对过的一手来源。版本更迭后，它们也是该最先查阅的地方。

- LlamaIndex官方文档：[https://docs.llamaindex.ai](https://docs.llamaindex.ai)（目前跳转至`developers.llamaindex.ai/python/framework`）
- LangChain官方文档：[https://python.langchain.com](https://python.langchain.com)（目前跳转至`docs.langchain.com/oss/python`）
