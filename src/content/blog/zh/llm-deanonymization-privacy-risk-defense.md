...
<strong>社会工程学攻击</strong>：基于识别出的员工在线活动模式，攻击者可以实施精心策划的钓鱼攻击。AI编程工具的[提示词注入安全威胁](/zh/blog/zh/roguepilot-copilot-prompt-injection-security)是另一个值得关注的类似攻击向量。
...
<strong>账户分离原则</strong>：将工作相关活动与个人活动的账户完全分离。制定禁止使用相同电子邮件或类似用户名的政策。构建更系统的AI安全治理框架，可参考[NIST AI智能体安全标准](/zh/blog/zh/nist-ai-agent-security-standards)。
...
技术本身是中立的，但<strong>当前防御手段远落后于攻击手段</strong>是问题所在。攻击者能以低成本执行大规模去匿名化，而防卫者必须逐个应对，形成不对称结构。[企业AI蒸馏攻击与防御策略](/zh/blog/zh/ai-distillation-attacks-enterprise-defense)记录了具有类似不对称特征的另一类实际威胁。
