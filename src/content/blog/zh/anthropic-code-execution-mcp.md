...
如果您想从头构建自己的 MCP 服务器，[自建 MCP 服务器 — 使用 Streamable HTTP 传输实现真实 AI 工具](/zh/blog/zh/mcp-server-build-practical-guide-2026)提供了完整的 Python FastMCP 实现教程。
...
每次工具调用都需要完整的<strong>模型推理 → 工具执行 → 结果返回 → 再次推理</strong>循环。对于需要多步骤协作的复杂任务，可能需要 15〜20 次往返调用，每次都会产生网络延迟和模型推理时间，导致总体执行时间显著增加。不同的智能体架构模式对这一问题的影响各有不同，[Claude Code 5种工作流模式](/zh/blog/zh/claude-code-agentic-workflow-patterns-5-types)对各类模式进行了详细对比。
