...
无审查模型移除的是<strong>第一层（模型级别）</strong>的约束。对研究者而言，这相当于获得了"原材料"的访问权，但同时也意味着所有安全机制都被去除了。正如[多语言环境下LLM护栏失效案例的研究](/zh/blog/zh/dont-trust-the-salt-multilingual-llm-safety)所示，护栏本身可能内含语言偏见，这也削弱了其可靠性。
...
最有前景的方向是<strong>模块化安全架构</strong>。与此同时，[开放权重模型与专有模型之间的性能差距正在缩小](/zh/blog/zh/minimax-m25-open-weight-vs-proprietary)，这也在加速无审查开源LLM的商业竞争力提升：
- 基础模型不受限制地公开
- 安全层作为独立模块提供
- 根据用途选择适当的安全级别
- 明确部署环境中的责任归属
