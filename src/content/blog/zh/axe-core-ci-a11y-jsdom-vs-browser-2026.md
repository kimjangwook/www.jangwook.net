...
这块地板究竟在什么高度，我另外量过。[把axe-core跑遍W3C ACT测试用例](/zh/blog/zh/act-rules-axe-coverage-wcag-sc-2026/)后，36条成功准则里有22条检出数为0。其中一部分沉默，是因为规则根本就是关着的。这就是接入CI之前先确认 `rules` 配置的理由。
