...
V4 把同样的 8px 换成内边距做。弹出层的盒子本身贴着触发器，盒子内侧上方留 18px 透明内边距，只把内容往下推。脚本量到的间距是 0px。看着是分开的，对指针来说是连着的。看得见的尺寸和命中区域分家，这个结构和我[实测目标尺寸标准](/zh/blog/zh/wcag22-target-size-audit-2026)时看到的在同一条轴上。再用 `:has()` 把「悬停在弹出层自己身上」也算进打开条件，纯 CSS 就能拿下 Hoverable。
