...
<strong>指向 DOM 而不是承载值的字段，这套检查抓不到。</strong>像 `speakable` 的 `cssSelector` 这类值，指的是页面的某个部分，作为字符串它永远合法。选择器什么都没匹配到，或者一口气匹配了十三处，schema 检查照样放行。这种腐坏我实测过一次，记录在[让机器"读这里"的标记，指向了13个段落](/zh/blog/zh/speakable-cssselector-pointer-rot-2026/)。
