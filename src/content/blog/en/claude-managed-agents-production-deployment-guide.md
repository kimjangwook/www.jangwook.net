...
I'm not using it in production yet myself. The reason is straightforward: the agents I run use a tmux-based setup for running multiple agents simultaneously, and the current Managed Agents public beta only supports single agents. When multi-agent capabilities become generally available, I'll reevaluate.
