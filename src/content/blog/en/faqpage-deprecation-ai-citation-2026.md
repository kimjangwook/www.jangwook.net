...
The results-page space that FAQ rich results left behind is filling fast with AI Overviews and other generative answers. And the way an AI answer engine pulls information off a page is fundamentally different from a Google rich result. Rich results read a separate channel called JSON-LD. Most AI crawlers, by contrast, extract meaning from the [rendered HTML body itself](/en/blog/en/ai-crawlers-dont-render-javascript-csr-2026/). And a "clear question → short, self-contained answer" pattern is the most quotable shape a machine can find. How much of it surfaces in that citation slot, though, isn't settled by markup alone. It pays to also check [how far robots snippet controls like `max-snippet` and `nosnippet` actually reach into AI Overviews](/en/blog/en/robots-snippet-controls-ai-overviews-2026/). And there's one more step after being cited: whether the link attached to that answer really lands back on the sentence. [Clicking through 69 text-fragment links in Chromium](/en/blog/en/text-fragment-citation-deep-link-audit-2026/), every prose link arrived. Inside code blocks, 14 of 15 broke.
