...
It's the same thread as [tracking load_duration during cold starts](/en/blog/en/local-llm-cold-start-load-duration-experiment/). The numbers Ollama quietly tucks into each response, however thinly documented, are the most honest clues to its real behavior. Just as `load_duration` snitched on cold starts, `prompt_eval_count` snitches on truncation. Its counterpart `eval_count` has the same character: it's the number I used to measure [how far invisible thinking tokens push the cost up](/en/blog/en/local-llm-reasoning-mode-token-cost-experiment/) once reasoning mode is on. If you're running local models seriously, give these numbers a look.
