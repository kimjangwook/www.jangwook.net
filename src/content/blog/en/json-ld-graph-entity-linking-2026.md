...
6. (Optional) `flatten` it with `jsonld` and check via script that the connected components equal <strong>1</strong>. Two or more means a reference is missing somewhere. On a multilingual site, the same "verify it yourself, not the docs" stance is worth running against [hreflang reciprocity, audited with a 30-line script](/en/blog/en/hreflang-reciprocity-audit-multilingual-2026/). Passing the connectivity check still says nothing about whether each node's values are right. I covered that hole separately in [validating restaurant opening hours across three layers](/en/blog/en/restaurant-jsonld-opening-hours-validation-2026/).
