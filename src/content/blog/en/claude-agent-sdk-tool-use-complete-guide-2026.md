...
<strong>When Tool Use fits</strong>:
...
<strong>When to avoid Tool Use</strong>:
...
- [Tool use with Claude — overview](https://platform.claude.com/docs/en/agents-and-tools/tool-use/overview): the official explanation of `tool_use` / `tool_result` blocks, `stop_reason`, and the agentic loop.
- [Tool use — tool definition reference](https://platform.claude.com/docs/en/agents-and-tools/tool-use/tool-reference): primary material on the `name` / `description` / `input_schema` schema and token overhead.
