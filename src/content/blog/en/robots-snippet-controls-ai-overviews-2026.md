...
Time to lower expectations. Opening `max-snippet:-1` and `max-image-preview:large` does not make AI Overviews quote your page. These directives only open the **eligibility** to be quoted; whether you actually are is Google's call. They don't raise your ranking either. Google has never said snippet directives are a ranking signal. There's no promise that removing `nosnippet` brings more visitors. Above the page directive sits another switch, on the Search Console property. A merged HTML PR does not help if someone clicked exclude on the parent domain. I measured the switch official GEO left behind on public URLs.
