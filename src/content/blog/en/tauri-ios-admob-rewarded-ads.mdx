---
title: 'AdMob Rewarded Ads in Tauri 2.x iOS: Swift Plugin Guide'
...
faq:
  - question: "Do I really have to build my own AdMob plugin for Tauri?"
    answer: "For now, yes. There is no official plugin, so you need a custom Swift plugin that wraps the GoogleMobileAds SDK. You can use this article's AdmobPlugin.swift and Rust bridge code as a working starting point."
  - question: "Why use the xcode option instead of xcframework when creating the plugin?"
    answer: "When you have an external SDK dependency like GoogleMobileAds, the xcode option makes it far easier to add that dependency through Swift Package Manager. The xcframework option is simpler to distribute but more awkward for wiring up external dependencies."
  - question: "Why does #[cfg(target_os = ios)] not work in build.rs?"
    answer: "Because build.rs runs on the host machine (macOS), not the build target. To detect an iOS build you have to check whether std::env::var(TARGET) contains ios, otherwise the framework linking will not fire."
  - question: "What should I check first when a rewarded ad fails to appear?"
    answer: "Confirm in order that you are using the test ad ID, that GADMobileAds.sharedInstance().start() was called, and that you are testing on a real device rather than the simulator. Simulator support for ads is limited."
---
...
- Solo-developer apps with a high share of non-paying users, where in-app purchases alone won't sustain revenue. As [an individual developer's AI SaaS monetization journey](/en/blog/en/individual-developer-ai-saas-journey) shows, the smaller the app, the more a low-barrier revenue source matters.
...
- Minimal projects where the SDK's added build size and complexity are a burden. The maintenance cost can outweigh the ad income. If your project leans on AI tooling, the cost-versus-benefit lens from [the reality of AI agent costs](/en/blog/en/ai-agent-cost-reality) applies directly to this call.

In short, rewarded ads fit best in **games with short sessions and a strong retry motive**. If you're building something similar to Shadow Dash, start with the game-loop design in [iOS game development with Tauri + PixiJS](/en/blog/en/tauri-pixijs-ios-game-development).
