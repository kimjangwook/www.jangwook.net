...
2. **lang ×4** — codex (`gpt-5.6-luna`, reasoning effort `max`), one process per language. Before each run the other three files for today's slug are moved out of the repo, so there is no draft on disk to translate from. If codex dies, that one language falls back to claude opus at effort `xhigh`, from the FACT CORE, not from whatever codex left behind. `data/write-engines.txt` records who wrote what.
