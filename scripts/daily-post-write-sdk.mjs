...
- 도입: 첫 두 단락이 답이 정해지지 않은 긴장을 하나 만드는가, 아니면 선언만 하는가.`;

// 편집자 B: 정형구·구조. 감사에서 실물로 확인된 사이트 전역 반복 문구를 명시적으로 잡는다.
const EDITOR_PATTERN = `${lang} 원고의 정형구·구조 편집자다.
${EDITOR_COMMON}
너만 보는 기준:
- 발행 이력에서 이미 정형구가 된 리터럴이 이 원고에 또 있는가: "내 판단은 이렇다"/"判断はこうだ"류, "이 판단이 틀리는 조건은 하나다"/"条件は一つある"류, "결론부터 말한다"/"結論から書く"류 오프닝. 있으면 같은 뜻을 다른 문장으로 쓰라고 지적하라 — 규칙(입장을 고르고, 틀릴 조건을 밝힌다)은 지키되 문구가 매일 같으면 그게 곧 패턴이다.
