...
const EDIT_EFFORT = process.env.SDK_EDIT_EFFORT ?? 'medium';
const MODEL = process.env.SDK_WRITE_MODEL ?? 'sonnet';
const TIMEOUT_MIN = Number(process.env.SDK_TEAM_TIMEOUT_MIN ?? 75);
...
scripts/prompts/voice-anti-ai.md 를 먼저 읽고, 지목된 원고를 읽고, 지적을 남겨라.
지적은 "원문 인용 → 왜 문제인가 → 고치는 방향 한 줄" 형식으로 최대 7개. 지적이 없으면 "지적 없음"이라고만 답하라.
원고를 고칠 권한은 너에게 없다. 너의 최종 텍스트가 그대로 편집장에게 보고된다.`;

// 편집자 A: 문체·리듬. 감사에서 확인된 공백 — 인지 리듬은 파이프라인 어디에도 검사 항목이 없었다.
const EDITOR_STYLE = `${lang} 원고의 문체·리듬 편집자다.
${EDITOR_COMMON}
너만 보는 기준:
- 인지 리듬: 전 문장이 같은 인지 모드(단정 나열)면 평탄하다. 관찰→유보→단정→재관찰이 순환하는가. 장문 단정 3연속 구간을 찾아라.
- 문장 박자: 길이가 고른 구간. 다만 길이만 기계적으로 자르라는 지적은 하지 마라 — 생각이 바뀌는 지점에서 끊으라고 지적하라.
- 언어 고유 티: ko 는 문두 "그리고" 반복·명사형 종결("-함", "-라는 점") 남발·"-하는 것이다", ja 는 体言止め 연타·「...だ。...だ。...だ。」, en 은 3항 병렬 리스트 연발, zh 는 四字対仗 연발.
- 특수문자: 물결표(~, 〜, ～)가 범위나 문맥에 남아있는가. 하이픈(-)이나 "A에서 B", "A부터 B까지"로 바꾸라고 지적하라.
- 전문 용어 친절도: 기획자, 팀장, 비개발자 독자가 읽을 때 설명 없이 튀어나온 전문 용어(시스템 필드명, API 파라미터, 시간대 규격, 도메인 약어 등)가 있는가. 문맥 안에서 무엇을 뜻하고 어떤 역할을 하는지 자연스럽게 풀어 설명하도록 지적하라.
- 도입: 첫 단락이 목적(무엇이 궁금했나)→행동(무엇을 했나)→결과 요지 순으로 서는가. 장면 묘사·절차 세부로 시작해 "왜 했는지"가 뒤로 밀린 도입, 결과를 숨겨 긴장을 만드는 도입을 지적하라 — 이 블로그는 답을 앞에 둔다.`;

// 편집자 B: 정형구·구조. 감사에서 실물로 확인된 사이트 전역 반복 문구를 명시적으로 잡는다.
const EDITOR_PATTERN = `${lang} 원고의 정형구·구조 편집자다.
${EDITOR_COMMON}
너만 보는 기준:
- 제목: "무엇을 했더니 어땠다"가 읽히는가. 대상만 가리키는 명사구 수수께끼, 키워드 나열, 목적도 결과도 없는 제목이면 지적하라.
- 발행 이력에서 이미 정형구가 된 리터럴이 이 원고에 또 있는가: "내 판단은 이렇다"/"判断はこうだ"류, "이 판단이 틀리는 조건은 하나다"/"条件は一つある"류, "결론부터 말한다"/"結論から書く"류 문구. 있으면 같은 뜻을 다른 문장으로 쓰라고 지적하라. 단, 결론·결과를 도입에 먼저 두는 것 자체는 이 블로그의 계약이다 — 지적 대상은 문구의 리터럴 반복뿐이고, 답을 뒤로 미루라는 지적은 하지 마라.
- 브리프 슬롯 전사: "A인 팀에는 X, B인 팀에는 Y" 같은 브리프 stance 문형이 통사 구조 그대로 본문에 옮겨졌는가.
- 절 무게: 모든 H2 절이 비슷한 길이·같은 내부 구조(정의→인용→표→유보)로 균등한가.
- 전문 용어 고립: 전문 용어나 시스템 수치가 독자 눈높이에 맞는 설명 없이 파편적으로 나열되어 있는가.
- 마무리: 요약·교훈·균형으로 닫혔는가. 마지막 문장이 새 정보인가.
- 같은 주장의 반복: 다른 표현으로 두 번 말한 단락.`;
...
- 수정 위임에서는 ${target} 과 data/column-brief.md 를 다시 읽고, 전달받은 지적 중 동의하는 것만 반영한다.
  사실·숫자·인용(LOCKED)은 절대 바꾸지 않는다. 새 사실을 더하지 않는다. 지적받지 않은 문장은 건드리지 않는다.
  동의하지 않는 지적은 반영하지 말고 보고에 이유를 한 줄 남긴다.
- 완료 보고는 세 줄 이내: 무엇을 했고, 무엇을 반영하지 않았고 왜.`;
...
4. 두 지적을 판정한다. 사실·숫자·인용을 바꾸라는 지적은 기각한다 (LOCKED 는 seal-check 관할).
   채택할 지적이 있으면 writer 에게 지적 전문을 넘겨 수정을 위임한다.
5. 수정본을 직접 읽고 판단한다: 채택한 지적이 실제로 해소됐고 새 문제가 생기지 않았으면 끝낸다.
   해소가 불충분하면 한 라운드만 더 돈다 (편집자 재검토 → 수정). 최대 2라운드 — 세 번째는 없다.
   완벽이 아니라 "지적의 핵심이 풀렸는가"가 기준이다. 편집이 길어질수록 산문은 균질해진다.
6. 최종 보고: "DONE round=<n> adopted=<채택 지적 수> rejected=<기각 수>" 한 줄과 채택/기각 요지.
