...
async function commandWritePost() {
  ...
  for (const lang of languages) {
    ...
    const body = [
      ...
      '',
      '## Image plan',
      '- Hero: generate with Codex and save to the heroImage path.',
      '- Supporting image: add at least one architecture, workflow, execution-example, or UI-example asset.',
      '',
...
async function commandWriteGaPost() {
  ...
  for (const lang of languages) {
    ...
    const body = [
      ...
      '## Image plan',
      '- Hero: generate with Codex and save to the heroImage path.',
      '- Supporting image: add at least one chart, dashboard capture, or insight summary visual.',
      '',
      '## 실행 개요',
      '- 기간:',
      '- 핵심 지표:',
