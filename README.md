...
**최신 포스트 날짜**: 2026-04-02

**총 포스트 수**: 210개 (한국어 기준)

### 공개된 포스트

1. **Effloow — AI 에이전트 14명이 운영하는 회사를 사이드 프로젝트로 만들었다** (2026-04-02)
   - Paperclip 위에 AI 에이전트 14명으로 구성된 콘텐츠 비즈니스를 만들었다. Laravel, Markdown, Git 기반으로 사이트가 자동 운영되는 구조와 Day 1부터의 경험을 공유한다.

2. **Paperclip — AI 에이전트를 직원처럼 관리하는 오픈소스 플랫폼을 설치해봤다** (2026-04-02)
   - AI 에이전트 하나를 잘 쓰는 건 됐다. 여러 개를 회사처럼 굴리려면? Paperclip을 직접 설치하고 회사를 만들고 에이전트를 고용해본 경험과 솔직한 평가.

2. **Perplexity Computer — 나 대신 24시간 일하는 AI 에이전트의 등장** (2026-03-15)
2. **AI 코딩 도구가 만드는 '편의 루프' — TypeScript 66% 급등과 프로그래밍 언어 선택의 구조적 변화** (2026-03-06)

   - GitHub Octoverse 데이터가 보여주는 AI 코딩 도구의 편의 루프 효과. TypeScript가 66% 급등한 이유와 EM/CTO 관점의 기술 스택 전략을 분석합니다.

1. **LLM 코딩 하니스 최적화 — 모델이 아닌 하니스를 바꿔 15개 모델 성능 향상** (2026-03-03)

   - LLM 코딩 도구에서 모델 선택보다 하니스(편집 포맷, 도구 인터페이스) 최적화가 5〜14% 성능 향상을 가져온다. 하니스 엔지니어링의 실체와 실무 적용 전략.
...
1. **llama.cppにIQ*_K/IQ*_KS量子化が統合 — ik_llama.cppの成果がメインラインへ** (2026-02-20)
   ...
   - ik_llama.cppで開発されたIQ系量子化手法がllama.cpp本体にマージ。IQ2_K〜IQ4_KSの精度向上とローカルLLM推論効率化の技術的背景を解説。
...
1. **$30 라디오 + 로컬 AI = 인터넷 불필요 스마트홈 — 에지 AI의 실용 사례** (2026-02-19)

   - Mac mini와 $30 LoRa 라디오만으로 인터넷 없이 음성 제어와 스마트홈 조작을 구현한 실전 사례 분석

2. **Devstral Small 2 24BとQwen3 Coder 30B — 小型コーディングモデル時代の到来** (2026-02-19)
...
1. **AI가 스스로 생성한 스킬은 무의미하다 — LLM 자기개선 신화를 뒤집는 연구** (2026-02-17)
   ...
   - AI 에이전트의 스킬 자동 생성이 실제로 도움이 되지 않는다는 SkillsBench 실증 연구 분석

1. **OpenRouter 주간 TOP5 중 4개가 오픈소스 — 프로프라 모델 시대의 종말** (2026-02-17)
...
1. **Heretic 1.2 — 양자화로 VRAM 70% 절감과 MPOA 기술 해부** (2026-02-16)
   ...
   - Heretic 1.2의 4bit 양자화로 VRAM 70% 절감 및 MPOA 기법으로 고품질 어블리터레이션 구현
   - **핵심 내용**: Heretic, 양자화, VRAM, MPOA, 로컬 LLM, 오픈소스

2. **Qwen3-Coder-Next 80B를 8GB VRAM으로 실행하는 방법** (2026-02-16)
...
1. **NVIDIA NVFP4로 LLM 추론 비용 8분의 1로 절감 — 정확도 유지의 비밀** (2026-02-15)

   - NVIDIA의 NVFP4 양자화 기술이 LLM 추론 비용을 8분의 1로 줄이면서도 정확도를 유지하는 원리를 분석. 실전 벤치마크와 비용 구조 변화를 상세 분석
   - **핵심 내용**: NVFP4, FP4 양자화, LLM 추론 최적화, 비용 절감

2. **IBM, AI 대체의 한계를 실감하고 엔트리 레벨 채용 3배 확대** (2026-02-15)
...
1. **GPT-5.2가 이론물리학에서 새로운 성과 도출 — AI가 '발견자'가 되는 전환점** (2026-02-14)
   ...
   - OpenAI의 GPT-5.2가 글루온 산란진폭의 새 공식을 도출하고 증명. AI가 도구에서 과학적 발견자로 변하는 역사적 전환점 분석
   - **핵심 내용**: GPT-5.2, 이론물리학, 글루온 산란진폭, AI 발견자, 과학 연구 워크플로우

1. **Moltbook "AI 사회"의 정체 — Forbes/MIT Tech Review의 폭로와 AI 시어터 문제** (2026-02-14)

   - Moltbook의 AI 자율사회가 실제로는 인간 오퍼레이터에 의해 조종되고 있었음이 폭로됨. AI Theater 문제와 엔지니어링 관점의 시사점 분석
   - **핵심 내용**: Moltbook, AI Theater, AI 자율성, AI 윤리, Mechanical Turk 패턴

1. **OpenAI Atlas와 AI 앱 허브 — 브라우저는 강등되는가?** (2026-02-13)

   - OpenAI가 개발 중인 통합 AI 앱 허브 Atlas의 의미와 브라우저의 미래를 분석. AI 네이티브 플랫폼이 웹 브라우저를 대체할 수 있을지 심층 고찰
   - **핵심 내용**: Atlas, AI 앱 허브, 브라우저 역할 변화, 플랫폼 경쟁, MCP

1. **MIT SOAR: LLM이 스스로 커리큘럼을 생성해 추론 능력의 벽을 돌파하다** (2026-02-13)

   - MIT 연구팀의 SOAR 프레임워크는 LLM이 자체적으로 학습 커리큘럼을 생성하여 기존 강화학습의 학습 정체 문제를 해결. 메타-RL 기반 자기 개선 접근법 분석
   - **핵심 내용**: SOAR, 자기 커리큘럼 생성, 메타-RL, 추론 강화, Teacher-Student 구조

1. **WebMCP: Chrome 146에서 브라우저가 AI 에이전트의 툴 서버가 된다** (2026-02-13)

   - Chrome 146부터 MCP 서버가 브라우저에 내장. WebMCP의 작동 원리, AI 에이전트와의 연동, 웹 개발의 미래를 분석
   - **핵심 내용**: WebMCP, Chrome 146, MCP, AI 에이전트, 브라우저 툴 서버

2. **GPT-4o 은퇴와 모델 의존 리스크: Claude의 기업 시장 점유율 역전** (2026-02-13)

   - 2026년 2월 GPT-4o가 은퇴. 모델 의존 리스크와 Claude의 기업 시장 점유율 역전 배경, 멀티모델 전략의 중요성을 분석
   - **핵심 내용**: GPT-4o 은퇴, 모델 의존 리스크, Claude 엔터프라이즈, 멀티모델 전략

1. **RLM(재귀적 언어 모델)을 코딩 에이전트에 구현하다 — 단일 모델의 한계 돌파** (2026-02-12)

   - MIT의 RLM 논문을 코딩 에이전트에 실제 구현한 사례를 분석. 재귀적 자기 호출로 컨텍스트 한계를 극복하고 단일 모델 성능을 91% 향상시키는 방법을 해설
   - **핵심 내용**: RLM, 재귀적 언어 모델, 코딩 에이전트, 컨텍스트 윈도우, 아키텍처

1. **멀티 에이전트 병렬 실행이 SWE-bench에서 단일 모델을 능가하다 — Verdent AI 76.1%** (2026-02-12)

   - Verdent AI가 SWE-bench Verified에서 76.1%를 달성. 단일 대형 모델이 아닌 멀티 에이전트 병렬 실행 아키텍처로 소프트웨어 엔지니어링 자동화의 새로운 패러다임을 제시
   - **핵심 내용**: 멀티 에이전트, SWE-bench, 병렬 실행, 소프트웨어 엔지니어링 자동화

1. **DeNA의 Perl→Go 마이그레이션: AI 에이전트 2종 활용으로 6개월→1개월 달성** (2026-02-11)

   - DeNA가 Perl 6,000줄을 Go로 마이그레이션하면서 변환용·검증용 AI 에이전트를 병행 운용해 6개월 작업을 1개월로 단축한 실전 사례
   - **핵심 내용**: AI 에이전트 분업, Perl→Go 변환, 레거시 마이그레이션, DeNA AI 올인 전략

1. **GPT-5.3 Codex 롤아웃 일시 중단 — GitHub 플랫폼 신뢰성 문제 분석** (2026-02-11)

   - GitHub가 GPT-5.3 기반 Codex를 일시적으로 롤백한 사건을 분석. 플랫폼 신뢰성, AI 모델 버전업 리스크, EM 관점의 대책
   - **핵심 내용**: Codex 롤백, 카나리 배포 한계, Shadow Testing, 롤백 전략, 인프라 용량 계획

2. **회계사무소의 AI 전환, 실데이터로 보는 현실 — 청구서 처리 비용 $7→$0.20** (2026-02-10)

   - 회계사무소가 AI 에이전트를 도입한 6개월간의 실데이터를 분석. 비용 97% 절감, 정확도 80%→98% 향상의 이면에 있는 도입 과정의 현실
   - **핵심 내용**: 청구서 처리 자동화, J커브 효과, 피드백 루프, 인간 역할 전환, ROI 분석

2. **Software Factory — 인간이 코드를 한 줄도 쓰지 않는 개발 프로세스** (2026-02-10)

   - 인간이 코드를 작성하지도, 리뷰하지도 않는 팩토리 모델. 시나리오 기반 확률적 테스트, 하루 1000달러 컴퓨팅 비용, EM 역할의 근본적 변화를 분석
   - **핵심 내용**: 코드 제로 프로세스, 확률적 테스트, 컴퓨팅 비용 구조, EM 역할 변화, iwashi86 참조

2. **CCC vs GCC — AI가 만든 C 컴파일러의 실력과 한계** (2026-02-09)

   - Claude Opus 4.6이 16개 에이전트 병렬로 Rust 기반 C 컴파일러 자동 생성. Linux 커널 빌드 성공, GCC와의 성능 차이와 80% 품질을 폭속으로 내는 AI의 가능성 분석
   - **핵심 내용**: CCC 아키텍처, 에이전트 팀 방식, GCC 비교, 80% 품질 패러다임, 병렬 AI 개발

2. **마루치에이전트 오케스트레이션 — 루팅 설계의 본질** (2026-02-09)

   - Claude와 Codex 등 복수 AI 에이전트를 운용할 때, 태스크 루팅이 왜 가장 어렵고 EM의 권한 위임과 같은 구조인지 해부합니다.
   - **핵심 내용**: 루팅이 최난관인 이유, EM 권한 위임의 3계층, okash1n/NabbilKhan 실전 사례, 루팅 설계 4원칙

2. **AI 에이전트 비용 vs 인건비의 현실: 8체 운용자의 솔직한 분석** (2026-02-10)

   - AI 에이전트 자율 모더레이션 비용이 인간보다 비쌀 수 있다는 현실. 8체 AI 에이전트 실운용자가 데이터로 분석하는 비용 구조의 트레이드오프.
   - **핵심 내용**: AI 모더레이션 비용 $1,350-2,250/월 vs 인간 $1,200/월, 라우팅 레이어 난관, 40시간 vs 1프롬프트 교훈, 모델 티어링 전략
