...
**IMPORTANT - Minimum Content Requirements (CRITICAL)**:

> 이 기준을 충족하지 못하면 QA 루프에서 FAIL 처리된다. 작성 단계에서부터 충분한 분량을 확보해라.

| 항목 | 최소 기준 | 권장 |
|------|----------|------|
| **총 단어 수 (한국어)** | **2,000단어** | 2,500~3,500단어 |
| **H2 섹션 수** | **5개** | 6~8개 |
| **도입부** | 150단어 | 200~300단어 |
| **본문 섹션당** | 300단어 | 400~600단어 (섹션 간 길이 불균일 허용) |
| **리서치 소스** | **3개** | 5개 이상 (공식 문서, GitHub, 블로그 등) |

**분량 확보 전략**:
- 추상적 설명을 구체적 예시로 풀어라 (코드, 수치, 실제 시나리오)
- "이걸 해보니 이랬다"는 1인칭 경험이 자연스럽게 분량을 늘린다
- 기사 유형별 필수 구조(아래 참조)를 채우면 자동으로 2,000단어를 넘긴다
- **채우기용 뻔한 설명은 금지** — 분량을 늘리되 정보 밀도는 유지
...
- **기사 유형에 따라 적절한 구조 장치를 활용** (아래 "기사 유형별 필수 구조" 참조)
- Tutorial/Comparison 유형은 코드 + 표가 자연스러우므로 3~4개 허용
- Opinion/News 유형은 산문 중심이므로 0~2개가 적절
- 핵심: 장치의 **개수**보다 **맥락 적합성**이 중요. 필요 없는데 넣지 말고, 필요한데 빼지도 마라
...
##### 9. 셀프 체크 & QA 루프 (작성 완료 후 필수 — 최대 10회 재시도)

포스트 작성 후 아래 항목을 반드시 검증. **하나라도 FAIL이면 해당 부분을 수정하고 다시 체크한다** (최대 10회):

**분량 체크 (CRITICAL)**:
- [ ] 한국어 기준 **최소 2,000단어** (약 원고지 8~10매) 이상인가?
- [ ] **H2 섹션이 5개 이상**인가?
- [ ] 도입부가 150~300단어인가?
- [ ] 각 본문 섹션이 300~600단어인가? (섹션 간 길이 변주 허용)
- [ ] 미달 시: 얕게 다룬 섹션을 구체적 예시, 코드, 경험담으로 보강

**팩트 체크 (CRITICAL — 1회차에서 반드시 수행)**:
- [ ] 본문에 언급된 **수치, 통계, 가격, 벤치마크, GitHub 스타 수** 등을 WebSearch로 1~2건 검증했는가?
- [ ] **날짜, 버전 번호, API 엔드포인트** 등 시간에 민감한 정보가 최신인가?
- [ ] 데이터를 **날조하지 않았는가**? (정확한 수치를 모르면 "공식 발표 기준" 또는 "약 ~" 표기)
- [ ] 인용한 출처(공식 문서, 블로그 등)의 URL이 실제로 존재하는가?
- [ ] **직접 인용 축자 대조 (CRITICAL)**: 따옴표나 인용 블록으로 출처를 명시한 문장은 발행 전 WebFetch로 원문과 글자 단위로 대조해 통과해야 한다. 대조가 불가능하거나 원문과 불일치하면 직접 인용을 버리고 의역 + 출처 링크로 강등한다. 원문에 없는 문자열을 직접 인용으로 표기하는 것은 날조 인용이며 페르소나 5절 위반이다. **인용을 나중에 수정·교체할 때도 반드시 원문을 재-fetch해 대조할 것 — 기억으로 재작성하면 축자를 오히려 깨뜨린다(2026-07-27 실측).**
- [ ] **인용 곁 출처 URL 필수 (게이트)**: 축자 주장 블록인용("그대로 옮기면"/verbatim/そのまま引く/原文照录 등)에는 원문 URL이 인용 바로 곁(도입문 또는 블록 직후 출처행)에 인라인으로 있어야 한다. `validate:publishing`가 '축자주장+인접 링크 부재'를 빌드에서 차단한다. 독자와 다음 리뷰어가 한 번의 클릭으로 축자를 검증할 수 있게 하는 것이 목적.
- [ ] **금지**: 존재하지 않는 기능, 미출시 버전, 허구의 벤치마크를 사실처럼 서술
...
- [ ] 비판적 의견이 2개 이상인가?
- [ ] 명확한 입장 표명이 1개 이상인가?
- [ ] 구조 장치가 기사 유형에 맞게 활용되었는가? (Tutorial/Comparison: 3~4개, Opinion/News: 0~2개)
- [ ] 도입부가 직전 포스트와 다른 방식인가?
- [ ] 문장 길이에 변주가 있는가? (메트로놈 아닌가?)
- [ ] "이 글을 AI가 쓴 것 같다"고 느껴지는 부분이 있는가? → 있으면 재작성

**QA 루프**:
- 1회차: 위 체크리스트 전체 검증 + **팩트 체크 WebSearch 수행**
...
4. Write the blog post using the **FACT-CORE TRANSCREATION APPROACH** (2026-07-07 — 번역 구조 폐지):
   ...
   **Stage 0: FACT CORE 작성 (모든 판의 유일한 사실 원천)**
   ...
   - 공식 출처 인용문(원문 그대로) + URL — 직접 인용은 WebFetch로 원문 축자 대조를 통과한 문자열만 FACT CORE에 넣는다. 대조 불가/불일치면 의역 + 출처 링크로 강등
   - 코드/스키마 스니펫
