...
- [ ] **직접 인용 축자 대조 (CRITICAL)**: 따옴표나 인용 블록으로 출처를 명시한 문장은 발행 전 WebFetch로 원문과 글자 단위로 대조해 통과해야 한다. 대조가 불가능하거나 원문과 불일치하면 직접 인용을 버리고 의역 + 출처 링크로 강등한다. 원문에 없는 문자열을 직접 인용으로 표기하는 것은 날조 인용이며 페르소나 5절 위반이다.
